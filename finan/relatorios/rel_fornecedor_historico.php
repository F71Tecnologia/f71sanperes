...
if (isset($_REQUEST['consultar'])) {
    ...
    $query = "SELECT a.id_saida, a.especifica, a.id_projeto, a.data_vencimento, a.valor, d.nome AS nome_projeto,a.valor_bruto,
                b.c_razao,REPLACE(REPLACE(REPLACE(b.c_cnpj, '-', ''), '.', ''), '/', '') AS cnpj, a.n_documento
                FROM saida AS a
                ...
                WHERE a.status = 2 AND estorno = 0 AND a.id_regiao = $id_regiao AND (data_vencimento BETWEEN '$data_ini' AND '$data_fim') $where $projeto
    ...
    while ($row = mysql_fetch_assoc($result)) {
        ...
        $total_liquido += (strpos(',', $row['valor']) !== FALSE) ? $row['valor'] : str_replace(',', '.', $row['valor']);
        echo strpos(',', $row['valor']) !== FALSE;
    }
...
$nome_pagina = 'Relatório de Fornecedores Pagos';
...
<html lang="pt">
    ...
    <body>
        ...
        <div class="container">
            ...
            <form action="rel_fornecedor_historico.php" method="post" class="form-horizontal top-margin1" name="form1" id="form1">
                            ...
                            <tbody>
                                ...
                                foreach ($saidas as $cnpj => $value) {
                                    ...
                                    if ($proj_anterior !== $value['id_projeto'] && $proj_anterior > 0) {
                                        ?>
                                        <tr>
                                            <td colspan="7" class="info">&emsp;</td>
                                        </tr>
                                        <?php
                                    }
