...
if (isset($_REQUEST['method']) && $_REQUEST['method'] == 'excluir') {
    ...
    $objSubGrupo->setIdEntradasaida($id);
    $objSubGrupo->inativa();
    echo json_encode(['status' => $objSubGrupo->inativa()]);
    exit();
}
...
<html lang="pt-br">
    ...
    <body>
        ...
        <div class="container">
            ...
            <?php
            $objSubGrupo->listaSubGrupos();
            if ($objSubGrupo->getNumRows() > 0) {
                ?>
                <div class="panel panel-default">
                    <table class="table valign-middle text-sm">
                        ...
                        <tbody>
                            <?php while ($objSubGrupo->getRow()) { ?>
                                <tr id="p<?= $objSubGrupo->getId() ?>">
                                    <td><?= $objSubGrupo->getIdSubgrupo() ?></td>
                                    <td><?= $objSubGrupo->getNome() ?></td>
                                    <td class="text-right">
                                        <a class="btn btn-xs btn-success" href="form_conta.php?id=<?= $objSubGrupo->getId() ?>"><i class="fa fa-pencil"></i></a>
                                        <button class="btn btn-xs btn-danger btn_excluir" data-id="<?= $objSubGrupo->getId() ?>"><i class="fa fa-trash"></i></button>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
        ...
        <script src="../js/gestao_contas.js" type="text/javascript"></script>
    </body>
</html>
