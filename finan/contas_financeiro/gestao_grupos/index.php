...
if (isset($_REQUEST['method']) && $_REQUEST['method'] == 'excluir') {
    ...
    $objGrupo->inativa();
    echo json_encode(['status' => $objGrupo->inativa()]);
    exit();
}



$nome_pagina = 'Gestão de Grupos Financeiros';
...
<html lang="pt-br">
    ...
    <body>
        ...
        <div class="container">
            ...
            <?php
            $objGrupo->listaGrupos();
            if ($objGrupo->getNumRows() > 0) {
                ?>
            <div class="panel panel-default">
                <table class="table valign-middle text-sm">
                    ...
                    <tbody>
                        <?php while ($objGrupo->getRow()) { ?>
                        <tr id="p<?= $objGrupo->getIdGrupo() ?>">
                                <td><?= $objGrupo->getIdGrupo() ?></td>
                                <td><?= $objGrupo->getNomeGrupo() ?></td>
                                <td class="text-right">
                                    <a class="btn btn-xs btn-success" href="form_conta.php?id=<?= $objGrupo->getIdGrupo() ?>"><i class="fa fa-pencil"></i></a>
                                    <button class="btn btn-xs btn-danger btn_excluir" data-id="<?= $objGrupo->getIdGrupo() ?>"><i class="fa fa-trash"></i></button>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
