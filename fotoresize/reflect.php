<?php
		...
		v2 - 2nd March 2007
        Updates include changes by Monte Ohrt (monte@ohrt.com), Luis Fred
		----------------------------------------------------------------
	...
	*/
	...
	{
		echo 'e preciso PHP v 4.3.2 ou superior para executar isto.';
		exit();
	}
	...
	{
		echo 'Extencao GD nao carregada, impossivel continuar.';
		exit();
	}
	...
	if (isset($_GET['img']))
	{
        ...
        if (file_exists($source_image))
        {
            if ($cache)
            {
                ...
                $cache_file = 'ref_' . md5($_SERVER['REQUEST_URI']) . '_' . $cache_base;
        ...
        {
          echo 'Nao pode encontrar imagem para usar como fonte de reflexo';
          exit();
        }
	}
	else
	{
		echo 'Nao ha imagem para fornecer reflexo!';
		exit();
	}
	...
	*/
	...
	{
		echo 'Imagem nao fornecida, ou nao tem permissao de acesso a ela.';
		exit();
	}
	else
	{
	...
	switch ($type)
	{
		...
		default:
			echo 'formato de imagem nao suportado.';
			exit();
	}
	...
	*/
	...
	if (headers_sent())
	{
		echo 'Headers ja foi enviado, Nao sera possivel exibir imagem agora.';
		exit();
	}
	else
	{
