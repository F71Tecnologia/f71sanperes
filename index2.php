...
//-----------VERIFICANDO SE EXISTE ALGUMA OBRIGAÇÃO VENCIDA-------------------
if ($id_user == '133' OR $id_user == '5') {
    $REObr = mysql_query("
    SELECT id_oscip FROM obrigacoes_oscip WHERE
    status='1' AND id_master='{$usuario[id_master]}' AND periodo <> 'Indeterminado'
    AND 
    (CASE periodo
       ...
       WHEN 'Período' THEN oscip_data_termino
    END) <= NOW()");
    $ContObr = mysql_num_rows($REObr);
    if ($ContObr > "0") {
        print "<script type=\"text/javascript\">alert('..............ATENÇÃO..............\\n\\ $ContObr OBRIGAÇÕES VENCIDA(S)');</script>";
    }
}
...
if ($_GET['webmail'] == 'create') {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $register = mysql_query("INSERT INTO funcionario_email_assoc(id_master, id_funcionario, email, senha) VALUES({$master_id}, {$id_user}, '{$email}', '123456')");
                        ...
                        while ($row_pagina = mysql_fetch_assoc($qr_botoes_pg)):
                            ...
                            <div class="conteudo_aba" style="display:none;">
                                ...
                                ?>
                                    ...
                                    LINK ANTIGO: ver_tudo.php?id=19&regiao=
                                    -->
                                                                    
                                <?php } elseif($row_botoes['botoes_id'] == 17) {  //BOTÃO GESTAO DE FUNÇOES
                                ?>
                                ...
                                <?php } elseif($row_botoes['botoes_id'] == 12) {  //BOTÃO GESTAO DE FUNÇOES
                                ?>
                                ...
                                <?php } elseif($row_botoes['botoes_id'] == 9) {  //BOTÃO GESTAO DE UNIDADES
                                ?>
                                ...
                                <div id="webmail_cadastro">
                                    ...
                                    <form action="?webmail=create" method="post">
                                        <table>
                                            ...
                                            <tr>
                                                <td></td>
                                                <td><input type="submit" value="confirmar" style="width: 75px; height: 22px;" /></td>
                                            </tr>
                                        </table>
                                    </form>
                                </div>
