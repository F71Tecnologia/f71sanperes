                    ...
                    <form id="form1" class="form-horizontal" method="post">
                        <div class="panel panel-default">
                            ...
                            <div class="panel-footer text-right">
                                ...
                                <button type="submit" class="btn btn-primary" name="filtrar" value="Filtrar"><i class="fa fa-filter"></i> Filtrar</button>
                                <?php if ($filtro) { ?>
                ...
                if ($num_rows > 0) { ?>
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <table id="tbRelatorio" class="table table-striped table-condensed table-hover table-bordered">
                                <thead>
                                ...
                                <tbody>
                                    <?php while ($row = mysql_fetch_assoc($rs)) { ?>
                                        <tr class="valign-middle">
                                            <td class="text-center"><?=$row['id_coop']?></td>
                                            <td class="text-center"><?=($row['tipo'] == 1) ? 'Cooperativa' : 'Pessoa Jurídica'?></td>
                                            <td class="text-left"><?=$row['nome']?></td>
                                            <td class="text-left"><?=$row['tel']?></td>
                                            <td class="text-left"><?=$row['contato']?></td>
                                            <td class="text-center"><?=$row['taxa']?></td>
                                            <td class="text-center"><a class="btn btn-xs btn-primary bt-image" data-type="cooperativa" data-key="<?=$row['id_coop']?>"><i title="Ver Cooperativa" class="fa fa-search"></i></a></td>
                                            <td class="text-center"><a class="btn btn-xs btn-warning bt-image" data-type="editar" data-key="<?=$row['id_coop']?>"><i title="Editar" class=" fa fa-pencil"></i></a></td>
                                            <td class="text-center"><a class="btn btn-xs btn-danger bt-image" data-type="excluir" data-key="<?=$row['id_coop']?>"><i title="Excluir" class="fa fa-trash-o"></i></a></td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
