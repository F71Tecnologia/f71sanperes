...
if(validate($_REQUEST['method']) && $_REQUEST['method'] == "calcular"){
    ...
    $funcionariosPorSindicato = getFuncionarioPorSindicato($_REQUEST['sindicato']);
    ...
    while ($row_curso = mysql_fetch_assoc($funcionariosPorSindicato)) {
        ...
        if($qntFaixas == 1){
            ...
            $row_curso['nome'] = utf8_encode($row_curso['nome_clt']);
        }else{
        ...
        $arrayCursosNovo[$row_curso['nome_clt']][$row_curso['letra']][$row_curso['numero']] = $row_curso;
    }
...
//    }
    ...
    $tabelaFuncionarioDissidio = $objFuncoes->montaTabelaFuncionariosNiveis($arrayCursosNovo);
    
    $tabelaFuncionarioDissidio .= "<div class='text-center'>
                <input  type='hidden' name='file_sql' id='file_sql' value='{$fname}' /> 
                <button type='button' href='javascript:;' name='processar' id='processar' class='btn btn-danger'><i class='fa fa-trash'></i> Processar</button>
                </div>";
    
    echo $tabelaFuncionarioDissidio;
...
if(validate($_REQUEST['method']) && $_REQUEST['method'] == "executar"){
    ...
    foreach($sqlEx as $val){
        if(strlen($val) > 3){
            mysql_query($val) or die("Erro: ".mysql_error(). strlen($val));
        }
    }
    
    echo "Dissidio aplicado com sucesso";
...
$arrayCursosNovo = array();
$arrayFuncionariosNovo = array();
while ($row_curso = mysql_fetch_assoc($funcionariosPorSindicato)) {
    $arrayCursosNovo[$row_curso['nome']][$row_curso['letra']][$row_curso['numero']] = $row_curso;
    $arrayFuncionariosNovo[$row_curso['nome_clt']][$row_curso['letra']][$row_curso['numero']] = $row_curso;
}


$tabela = $objFuncoes->montaTabelaFuncoesNiveis($arrayCursosNovo);
$tabelaFuncionarios = $objFuncoes->montaTabelaFuncionariosNiveis($arrayFuncionariosNovo);
                    ...
                    <div class="panel-body">
                        ...
                        <div class="row duplicate">
                            <div class="form-group">
                                ...
                                <div class="col-sm-3">
                                    <div class="input-group">
                                        <input type="text" name="percent[]" id="percent[]" value="" placeholder="0,03" class="form-control" />
...
<!--                                <label for="valor_de" class="col-sm-2 control-label hidden-print" >Faixa salarial de</label>
                                <div class="col-sm-4">
                                    <div class="input-group">
                                        <input type="text" name="valor_de[]" id="valor_de[]" value="" placeholder="1.000,00" class="form-control valor" />
                                        <span class="input-group-addon"> até </span>
                                        <input type="text" name="valor_ate[]" id="valor_ate[]" value="" placeholder="2.000,00" class="form-control valor" />
                                    </div>
                                </div>-->
        ...
        <script>
            $(function(){
                ...
                $("#collapseExample2").on("click","#processar", function(){
                    //executar
                    var url = "dissidio_calc_por_funcionario.php";
                ...
                $("#calcular").click(function(){
                    ...
                    var url = "dissidio_calc_por_funcionario.php?method=calcular&"+ser+"&sindicato="+id_sind;
