                ...
                <?php foreach ($p as $k_data => $p_data) { ?>
                    ...
                    <?php foreach ($p_data as $pedido) { ?>
                        <tr id="tr_vt_pedido_<?= $pedido['id_vt_pedido']; ?>" class="valign-middle">
                            ...
                            <td class="center">
                                <?php if ($pedido['status'] != 2 && $pedido['user'] == $usuario['id_funcionario']) { ?><a href="javascript:;" onclick="if (confirm('Dejesa realmente deletar esse pedido')) {
                                    deletar_pedido(<?= $pedido['id_vt_pedido']; ?>)
                                } else {
                                    return false
                                }" ><img  id="img_del_pedido_<?= $pedido['id_vt_pedido']; ?>" src="../../financeiro/imagensfinanceiro/Delete-32.png" alt="Deletar" border="0" title="Excluir"></a><?php } ?>
                            </td>
                        </tr>
                    <?php } ?>
                <?php } ?>                
