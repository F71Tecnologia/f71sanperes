...
<html>
    ...
    <?php include("../template/navbar_default.php"); ?>
        <div class="container">
            <div class="row">
                <div class="col-xs-12">
                    <div class="page-header box-rh-header"><h2><span class="fa fa-users"></span> - RECURSOS HUMANOS <small> - Cracha Provisório</small></h2></div>
                </div>
            </div>
                    ...
                    <form action="" method="post" name="form1" id="form1" enctype="multipart/form-data" class="form-horizontal" >
                        <div class="panel panel-default">
                            ...
                            <div class="panel-footer text-right">
                                <?php   if(!empty($filtro) && isset($_POST['filtrar'])){ ?>
                                    <button type="button" form="formPdf" name="pdf" data-title="Cracha Provisório" data-id="tbRelatorio" id="pdf" value="Gerar PDF" class="btn btn-danger"><i class="fa fa-file-pdf-o"></i> Gerar PDF</button>
                                <?php } ?>
                                    <button type="submit" class="btn btn-primary" value="Filtrar" name="filtrar" id="filt"><span class="fa fa-filter"></span> Filtrar</button>
                            </div>
                        </div>
                    </form>
            ...
            <?php
            if ($filtro) { ?>
            <form action="cracha_provisorio_imprimir.php" method="post">
                    ...
                    <table id="tbRelatorio" class="table table-striped table-hover table-bordered table-condensed text-sm valign-middle">
                        ...
                        while ($rowClt = mysql_fetch_assoc($queryClt)) { ?>
                            <tr>
                                ...
                                <td class="" width="80%"><?=$rowClt['nome']?></td>
                                <td class="text-center" width="15%"><?=$rowClt['data_entrada']?></td>
                            </tr>
                        <?php } ?>
                        </tbody>
                        <tfoot>
