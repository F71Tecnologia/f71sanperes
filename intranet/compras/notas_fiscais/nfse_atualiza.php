        ...
        <div class="row"> 
            ...
            <div class="col-lg-12">
                <table class="table table-striped">
                    ...
                    <tbody>
                        <tr>
                            <td><?= $nfse->TomadorServico->RazaoSocial ?></td>
                            <td>
                                <?= mascara_string("##.###.###/####-##", $nfse->TomadorServico->IdentificacaoTomador->CpfCnpj->Cnpj) ?>
                                <input type="hidden" name="projeto_cnpj" id="projeto_cnpj" value="<?= $nfse->TomadorServico->IdentificacaoTomador->CpfCnpj->Cnpj ?>" class="onchange_cnpj" onchange="">
                            </td>
...
if (isset($_REQUEST['method']) && $_REQUEST['method'] == 'cosulta_cod_servico') {
    $cod = addslashes($_REQUEST['cod']);
    $query = "SELECT * FROM nfse_codigo_servico WHERE codigo LIKE '%{$cod}%'";
    $result = mysql_query($query) or die(mysql_error());
    while ($row = mysql_fetch_assoc($result)) {
        $servicos['servicos'][] = utf8_encode("{$row['codigo']} - {$row['descricao']}");
    }
