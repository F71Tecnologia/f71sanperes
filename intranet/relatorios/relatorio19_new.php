...
if (isset($_REQUEST['method']) && !empty($_REQUEST['method']) && $_REQUEST['method'] == "getList") {
    ...
    if($projeto > 0){
        $sql_clt .= "AND id_projeto = '{$projeto}' ";
    }
    $sql_clt .= "ORDER BY nome";
    ...
    if($projeto > 0){
        $sql_autonomo .= "AND id_projeto = '{$projeto}' ";
    }
    $sql_autonomo .= "ORDER BY nome";
        ...
        <script>
            $(function() {
                $(".bt-image").on("click", function() {
                    ...
                    thickBoxIframe(nome, "relatorio19_new.php", {id_curso: id_curso, projeto: projeto, regiao: regiao, method: "getList"}, "625-not", "500");
                });
            });
            $(function() {
        ...
        <div id="content">
            <form  name="form" action="" method="post" id="form">
                ...
                <table id="tbRelatorio" border="0" cellpadding="0" cellspacing="0" class="grid" width="100%" style="page-break-after:auto;"> 
                        ...
                        $sql_qtd_clt = "SELECT count(id_clt) FROM rh_clt WHERE id_curso = {$row_rel['id_curso']} AND id_regiao = '{$id_regiao}' AND id_projeto = '{$row_rel['id_projeto']}'";
                        if(!isset($_REQUEST['todos_projetos'])) {
                            $sql_qtd_clt .= "AND id_projeto = '$id_projeto' ";
                        }
                        $result_qtd_clt = mysql_query($sql_qtd_clt);
                        $qtd_clt = mysql_fetch_row($result_qtd_clt);
                        $sql_qtd_autonomo = "SELECT count(id_autonomo) FROM autonomo WHERE id_curso = {$row_rel['id_curso']} AND id_regiao = '{$id_regiao}' AND id_projeto = '{$row_rel['id_projeto']}'";
                        if(!isset($_REQUEST['todos_projetos'])) {
                            $sql_qtd_autonomo .= "AND id_projeto = '$id_projeto' ";
                        }
