...
if (isset($_REQUEST['gerar']) || isset($_REQUEST['todos_projetos'])) {
    ...
    $sql = "SELECT A.id_clt, A.id_curso, A.nome, A.matricula, A.cpf, A.pis, A.tel_cel, DATE_FORMAT(A.data_entrada,'%d/%m/%Y') AS dt_admissao, E.nome as projeto, C.nome as funcao, F.horas_mes,
            IF(A.status < 60 || A.status = 200,'ATIVO','INATIVO') status_nome,
            IF(A.data_saida != 0000-00-00,DATE_FORMAT(A.data_saida,'%d/%m/%Y'),DATE_FORMAT(A.data_demi,'%d/%m/%Y')) dt_saida
                FROM rh_clt AS A
                ...
                <table class="table table-striped table-hover table-condensed table-bordered text-sm valign-middle" id="tbRelatorio">
                    ...
                    <?php while ($row_rel = mysql_fetch_assoc($qr_relatorio)) { 
                        ...
                        <tr style="font-size:11px; ">
                            ...
                            <td align=""><?php echo $row_rel['funcao'] ?></td>
