...
if (isset($_REQUEST['method']) && $_REQUEST['method'] == "visualizarRescisao") {
    ...
    $sql = "SELECT A.id_projeto AS projeto_rescisao, A.nome, A.aviso, C.especifica, A.sal_base, C.codigo, B.id_clt
        ...
        WHERE A.id_recisao_lote = '{$_REQUEST['header']}' AND A.recisao_provisao_de_calculo = 1 AND (B.status < 60 || B.status = 200) GROUP BY A.id_clt ORDER BY C.codigo, B.nome";
...
if ((isset($_REQUEST['confirmar_rescisao']))) {
    $formImprimir = "<form action='../rh/recisao/imprimir_rescisao_lote.php' method='post' id='imprimir_confirmadas'>";
    $clt = $_REQUEST['id_clt'];
    $rescisao = $_REQUEST['id_recisao'];
...
//    foreach ($_REQUEST[id_clt] as $key => $value) {
    for($i=0;$i < count($_REQUEST['id_clt']);$i++) {
        $sqlMotivo = mysql_fetch_assoc(mysql_query("SELECT id_recisao, motivo, data_aviso, data_demi,saldo_salario,total_liquido FROM rh_recisao WHERE id_clt = $clt[$i] AND id_recisao = $rescisao[$i] AND status = 0 LIMIT 1"));

        $sqlClt = mysql_fetch_assoc(mysql_query("SELECT status FROM rh_clt WHERE id_clt = $clt[$i] LIMIT 1"));
        
//        if($_COOKIE['logado'] == 179){
//            echo "<pre>";   
//            print_r($sqlMotivo['id_recisao']);
//            echo "</pre>";
//            exit();
//        }  
        
        if ($sqlClt[status] == 10 || $sqlClt[status] == 200) {
            $contribuicao_sindical = 0;
            $liquido_final = $sqlMotivo['total_liquido'] - $contribuicao_sindical;

            $update_liquido_rescisao = "UPDATE rh_recisao SET total_liquido = '{$liquido_final}' WHERE id_recisao = '{$sqlMotivo['id_recisao']}'";
            mysql_query($update_liquido_rescisao);

            $formImprimir .= '<input type="hidden" name="id_clts[]" value="' . $clt[$i] . '">';
            
            $query = "
                INSERT rh_movimentos_rescisao (id_rescisao, id_mov, id_clt, nome_movimento, tipo_qnt, qnt, qnt_horas, valor, status, incidencia)
                    SELECT tmmrl.id_rescisao, tmmrl.id_movimento AS id_mov, tmmrl.id_clt, rhm.descicao AS nome_movimento,NULL AS tipo_qnt, NULL AS qnt, NULL AS qnt_horas, tmmrl.valor, tmmrl.status, CONCAT(CASE WHEN rhm.incidencia_inss > 0 THEN '5020,' ELSE '' END,CASE rhm.incidencia_irrf WHEN 1 THEN '5021,' ELSE '' END,CASE rhm.incidencia_fgts WHEN 1 THEN '5023' ELSE '' END) AS indidencia 
                    FROM tabela_morta_movimentos_recisao_lote tmmrl INNER JOIN rh_movimentos rhm ON tmmrl.id_movimento=rhm.cod
                    WHERE tmmrl.id_rescisao={$sqlMotivo['id_recisao']} AND tmmrl.id_clt={$clt[$i]}
                    ";    
...
//            }             
            ...
            mysql_query("UPDATE rh_recisao SET status=1, data_proc=NOW() WHERE id_recisao = '{$sqlMotivo['id_recisao']}' LIMIT 1;");
            mysql_query("UPDATE rh_clt SET status=$sqlMotivo[motivo], data_aviso='$sqlMotivo[data_aviso]', data_demi='$sqlMotivo[data_demi]', data_saida='$sqlMotivo[data_demi]' WHERE id_clt = $clt[$i] LIMIT 1;");
        }
    ...
    VALUES ('$usuario[id_funcionario]', '$usuario[id_regiao]', '$usuario[tipo_usuario]', '$usuario[grupo_usuario]', 'Confirmação de Rescisão em Lote', NOW(), '$ip', 'Confirmação de Rescisão em Lote')") or die("Erro Inesperado<br><br>" . mysql_error());
}
                    ...
                    <table id="historico_gerado" border="0" cellpadding="0" cellspacing="0" class="grid" width="100%">
                        ...
                        <tbody>
                            <?php while ($linha_header = mysql_fetch_assoc($sql_historico)) { ?>
                                <?php
                                $total_participantes = "SELECT COUNT(B.id_recisao) AS total 
                                        FROM rh_recisao AS B
                                        LEFT JOIN rh_clt AS C ON(B.id_clt = C.id_clt)
                                        WHERE B.id_recisao_lote = '{$linha_header['id_header']}' AND (C.`status` < 60 || C.`status` = 200)
                                        GROUP BY B.id_projeto";
                                $sql_total_participantes = mysql_query($total_participantes);
                                $rows_total_participantes = mysql_fetch_assoc($sql_total_participantes);
                                ?>
...
//                        echo "R$ 0,00" . number_format($row_rel['a477'], 2, ",", ".");
                                ...
                                <td align="right">
                                    <?php
                                    echo "R$ " . number_format(($row_rel['total_rendimento'] + $total_movimentos[$row_rel['id_recisao']][$row_rel['id_clt']]['CREDITO']) - ($row_rel['total_deducao'] + $total_movimentos[$row_rel['id_recisao']][$row_rel['id_clt']]['DEBITO']), 2, ",", ".");
                    ...
                    <div id='totalizador' style='height: auto; text-align: right; clear: both;'>
                        <input type='button' id='confirmar_rescisao1' value='Confirmar Rescisão'><input type='submit' style='display: none;' id='confirmar_rescisao' name='confirmar_rescisao' value='Confirmar Rescisão'>
                    </div>
...
//                        echo "</pre>";
                            ...
                            if ($status != $row_rel["codigo"]) {
                                ...
                                <thead>
                                    <tr>
                                        <th colspan="3" class="cabecalho_compactar"><?php echo "<p style='text-transform:uppercase; text-align:left' > » RESCISÕES FINALIZADAS </p>"; ?></th>
                                        <th colspan="15">Verbas Rescisórias</th>
                                        <!--<th colspan="6">Deduções</th>-->
                                        <th style="background: #fff; border: 0px;" ></th>
                                        <th colspan="5">EMPRESA</th>
