...
<?php
		  ...
		  switch($tipo) {
             ...
             break;
			 case 2:
					$result = mysql_query("SELECT * , date_format(data_nasci, '%d/%m/%Y') AS data_nasci, date_format(data_entrada, '%d/%m/%Y') AS data_entrada, date_format(data_rg, '%d/%m/%Y') AS data_rg, date_format(data_saida, '%d/%m/%Y') AS data_saida FROM rh_clt WHERE  tipo_contratacao = '2' AND id_regiao = '$regiao' AND id_projeto = '$projeto'");
		...
		while($participante = mysql_fetch_assoc($result)):
			...
			$qr_codigos = mysql_query("SELECT distinct(cod) FROM rh_movimentos WHERE  cod != 9991 AND cod != 0001 ORDER BY cod ASC");
				...
				for($b=1; $b<13; $b++) {
					...
					$qr_folha = mysql_query("SELECT id_folha FROM $banco1 WHERE mes = '$tubarao' AND year(data_inicio) = '$ano_base' $coluna_contratacao AND projeto = '$projeto' AND regiao = '$regiao' AND terceiro != '1' AND ferias != '1' AND status = '3'  ") or die(mysql_error());
				
				
					$folha = mysql_fetch_assoc($qr_folha);
					$qr_folha_individual = mysql_query("SELECT * FROM $banco2 WHERE id_folha = '$folha[id_folha]' AND $coluna_id = '$participante[$coluna_id]' AND status = '3' AND ($codigo != '0.00' OR $codigo != '') ") or die(mysql_error());
							
					if(mysql_num_rows($qr_folha_individual) == 0){
					$valores[] = NULL;
							...
							$valores[] = $folha_individual[ $codigo];
					}
			...
			for($m=1; $m<13; $m++) {
				...
				$qr_folha_individual_13 = mysql_query("SELECT * FROM $banco2 WHERE id_folha = '$folha_13[id_folha]' AND $coluna_id = '$participante[$coluna_id]' AND status = '3'");
				...
				// Rescisão
				$qr_rescisao  = mysql_query("SELECT * FROM rh_recisao WHERE $coluna_id = '$participante[$coluna_id]' AND year(data_demi) = '$ano_base' AND month(data_demi) = '$tubarao' AND motivo IN (60,61,62,80,81,100) AND status = '1'");
...
unset($rendimentos, $total_descontos, $total_rendimentos, $descontos, $movimentos,$soma, $soma_total_descontos, $soma_total_rendimentos, $valor_liquido_total, $salario, $salario13, $salario_ferias, $salario_rescisao,$soma1, $soma2, $soma3, $soma4);
endwhile;
 } ?>