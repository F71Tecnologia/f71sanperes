...
If (isset($_REQUEST['gerar'])) {
    ...
    $qr_relatorio_sql = "
SELECT clt.*, C.id_curso, C.nome as nome_curso,  D.horas_mes, (D.horas_mes - D.horas_semanais) AS horas_trabalhadas
    FROM 
            (
           ...
           (SELECT id_curso_de FROM rh_transferencias WHERE id_clt=B.id_clt AND id_curso_de <> id_curso_para AND data_proc >= A.data_inicio ORDER BY id_transferencia ASC LIMIT 1) AS de,
           (SELECT id_curso_para FROM rh_transferencias WHERE id_clt=B.id_clt AND id_curso_de <> id_curso_para AND data_proc <= A.data_inicio ORDER BY id_transferencia DESC LIMIT 1) AS para,
           (SELECT id_horario_de FROM rh_transferencias WHERE id_clt=B.id_clt AND id_horario_de <> id_horario_para AND data_proc >= A.data_inicio ORDER BY id_transferencia ASC LIMIT 1) AS horario_de,
           (SELECT id_horario_para FROM rh_transferencias WHERE id_clt=B.id_clt AND id_horario_de <> id_horario_para AND data_proc <= A.data_inicio ORDER BY id_transferencia DESC LIMIT 1) AS horario_para,
           C.cpf    
    ...
    $qr_relatorio = mysql_query($qr_relatorio_sql) or die(mysql_error());

    echo "<!-- $qr_relatorio_sql-->";
...
///MOVIMENTOS
    ...
    while ($row_mov = mysql_fetch_assoc($qr_movimentos)) {
        ...
        if ($row_folha['terceiro'] == 1 and $row_mov['id_mov'] == 200) {
            continue;
        }
    ...
    <body class="novaintra" >        
        <div id="content" style="width:3000px;">
            ...
            if (!empty($qr_relatorio) and isset($_POST['gerar'])) {
                ...
                <p style="text-align: left; padding-left: 30px;"><input type="button" onclick="tableToExcel('Table_geral', 'Folha')" value="Exportar para Excel" class="exportarExcel"></p>  
                ...
                <table border="0" cellpadding="0" cellspacing="0" id="Table_geral" name="geral">    
                    <tr>
                        <td>

                            <table border="0" cellpadding="0" cellspacing="0" class="grid essatb" width="100%" style="page-break-after:auto;" > 

                                <tr>
                                    <td align="center" colspan="<?php echo (count($nomes_credito) + ((count($nomes_debito) + 3)) + 7) ?>7"> <?php echo $row_master['razao'] . ' - ' . $row_projeto['nome']; ?>
                                        <strong>  <?php echo $row_master['razao'] . ' - ' . $row_projeto['nome']; ?>  - CLT</strong>
                                    </td>
                                </tr>
                                ...
                                while ($row_rel = mysql_fetch_assoc($qr_relatorio)) {
                                    ...
                                    $salario_base = $row_rel['salario_base']; // + $movimentos[$row_rel['id_clt']]['DEBITO']['(11) FALTA'];   //POG por conta da folha 
                                    $inss = $row_rel['inss'];
                                    $irrf = $row_rel['irrf'];
                                    ...
                                    if (mysql_num_rows($verifica_rescisao) != 0)
                                        continue;
                                    ?>
                                    ...
                                    <tr style="font-size:11px;">                    
                                        ...
                                        <td>
                                            ...
                                            <?php /* ALTERADO A PEDIDO DO RH  echo $row_rel['horas_trabalhadas']; */
                                                    echo $row_rel['horas_mes']; 
                                            ?>
                                        
                                        </td>
                                        <?php
                                        foreach ($nomes_credito as $mov) {
                ...
                <tr>
                    <td>
                        <table border="0" cellpadding="0" cellspacing="0" class="grid essatb" width="100%" style="page-break-after:auto;"> 
                            ...
                            <tr>
                                <td align="center" colspan="<?php echo (count($nomes_credito) + ((count($nomes_debito) + 3)) + 7) ?>7"> <?php echo $row_master['razao'] . ' - ' . $row_projeto['nome']; ?>
                                    <strong>  <?php echo $row_master['razao'] . ' - ' . $row_projeto['nome']; ?> - AUTÔNOMO </strong>
                                </td>
                            </tr>
                            ...
                            <?php
                            $qr_autonomo = mysql_query("
                                                    SELECT A.*, B.nome, C.nome AS nome_projeto,C.id_projeto, B.banco, B.conta, B.agencia, B.tipo_pagamento,B.nome_banco, B.cpf,B.pis,E.nome AS funcao, F.horas_mes
                                                    ...
                                                    LEFT JOIN rh_horarios as F ON(F.funcao = B.id_curso AND F.nome = E.nome)
                                                    WHERE D.id_master = '{$id_master}' AND MONTH(data_geracao) = '{$mes}' AND YEAR(data_geracao) = '{$ano}' AND D.id_regiao = '{$regiao}' AND C.id_projeto = '{$projeto}'
                                                    ORDER BY B.id_projeto,B.nome");

                            while ($linhaRpa = mysql_fetch_assoc($qr_autonomo)) {
                                ?>

                                <tr style="font-size:11px;">   
                                    <td><?php echo $linhaRpa['nome']; ?></td>
                                    <td><?php echo $linhaRpa['cpf']; ?></td>
                                    <td><?php echo $linhaRpa['funcao']; ?></td>
                                    <td></td>    
                                    <td><?php echo "R$ " . number_format($linhaRpa['valor'], '2', ',', '.'); ?></td>
                                    <td><?php echo $linhaRpa['horas_mes']?></td>
                                    <td><?php echo $linhaRpa['horas_mes']?></td>
                                    <td align="center"></td>
                                    <?php
                                    foreach ($nomes_credito as $titulo_mov) {
                                    ...
                                    ?>
                                    <td align="center"><?php echo "R$ " . $linhaRpa['valor_inss']; ?></td>
                                    <td align="center"><?php echo "R$ " . $linhaRpa['valor_ir']; ?></td>
                                    <td align="center"><?php echo "R$ " . $linhaRpa['valor_liquido']; ?></td>
