...
if (isset($_REQUEST['gerar']) || isset($_REQUEST['todos_projetos'])) {
    ...
    if ($tipo_contratacao == 2) {
        $str_qr_relatorio = "SELECT *, date_format(data_ctps, '%d/%m/%Y') as data_ctpsbr
            FROM rh_clt
...
<html lang="pt">
    <head>
        ...
        <title>:: Intranet :: Relatório de Participantes por CTPS </title>
    ...
    <body>
        ...
        <div class="container">
            <div class="page-header box-rh-header"><h2><span class="fa fa-users"></span> - RECURSOS HUMANOS <small> - Relatório de Participantes por CTPS</small></h2></div>
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-footer text-right hidden-print controls">
                        ...
                        <?php if (!empty($qr_relatorio) && (isset($_POST['gerar'])) || isset($_REQUEST['todos_projetos'])) { ?>
                            <button type="button" onclick="tableToExcel('tabela', 'Participantes por CTPS')" value="Exportar para Excel" class="btn btn-success" ><span class="fa fa-file-excel-o"></span>Exportar para Excel</button>
                        <?php } ?>
                        <?php
                            ...
                            while ($row_rel = mysql_fetch_assoc($qr_relatorio)) {
                                ...
                                <tr class="<?php echo $class ?>">
                                    ...
                                    <td><?php
                                        ...
                                        ?></td>
                                    <td><?php
                                        if (!empty($row_rel['serie_ctps'])) {
                                            echo $row_rel['serie_ctps'];
                                        } else {
                                            echo "NÃO TEM";
                                        }
                                        ?></td>
                                    <td><?php
                                        if (!empty($row_rel['data_ctpsbr'])) {
                                            echo $row_rel['data_ctpsbr'];
                                        } else {
                                            echo "NÃO TEM";
                                        }
                                        ?></td>
                                    <td><?php
                                        if (!empty($row_rel['uf_ctps'])) {
                                            echo $row_rel['uf_ctps'];
                                        } else {
                                            echo "NÃO TEM";
                                        }
                                        ?></td>
