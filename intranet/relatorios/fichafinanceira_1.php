        ...
        <div id="content">
            <div id="head">
                ...
                <div class="fleft" style="width:86.3%">
                    ...
                    <p></p>
                    <table class="grid" border="1" cellspacing="0" cellpadding="0" width="95%" style="margin-left:80px; border: 1px solid #ccc; background: #f1f1f1;"> 
                        <tr>
            ...
            if (isset($_POST['gerar'])) {
                ...
                $qr_folha = mysql_query("select A.id_folha,A.terceiro,A.mes, A.ids_movimentos_estatisticas, B.*,
                                                IF(MONTH(C.data_entrada) = B.mes AND  YEAR(C.data_entrada) = B.ano ,sallimpo_real, sallimpo) as salario_base
                                                from rh_folha as A 
                ...
                while ($row_folha = mysql_fetch_assoc($qr_folha)) {
                    ...
                    while ($row_mov = mysql_fetch_assoc($qr_mov)) {
                        ...
                        if ($row_folha['terceiro'] == 1 and $row_mov['cod'] == 5029) {
                            $valor_movimento = $row_folha['salliquido'];
                        } else {
                            $valor_movimento = $row_folha[$nome_campo];
                        }

                        if ($valor_movimento != '0.00' and $valor_movimento != '') {

                            $salario[(int) $row_folha['mes']] = $row_folha['salario_base'];
                    ...
                    $qr_mov = mysql_query("SELECT * FROM rh_movimentos_clt WHERE id_movimento IN($row_folha[ids_movimentos_estatisticas]) AND id_clt = '$id_clt'  ") or die(mysql_error());
                    while ($row_mov = mysql_fetch_assoc($qr_mov)) {
                ...
                <table cellspacing="0" cellpadding="0" class="grid" border="1" width="100%">
                    ...
                    for ($i = -1; $i < 13; $i++) {
                        if ($i < 1) {
                            ...
                            if ($salario[$i] == 0) {
                                ...
                                echo '<td>R$ ' . number_format($salario[$i], "2", ",", ".") . '</td>';
                            }
                        }
                    }

                    echo '<td>R$ ' . number_format(array_sum(array_slice($salario, 2, 12)), 2, ',', '.') . '</td>';
                    ...
                    foreach ($mov_tipos as $chave => $tipo) {
                            ...
                            for ($i = -1; $i <= 13; $i++) {
                                if ($i < 1) {
                                    if ($i < 13) {
                                        ...
                                        echo '<td>' . array_sum(array_slice($array, 2, 12)) . '</td>';
                                    }
                                } else {
                                    ...
                                    if ($i < 13) {
                                        if ($array[$i] == 0) {
                                            ...
                                            echo '<td >R$ ' . number_format($array[$i], "2", ",", ".") . '</td>';
                                        }
                                    } else {
                                        if ($array[$i] == 0) {
                                            echo '<td > - </td>';
                                        } else {
                                            echo '<td>R$ ' . number_format((array_sum(array_slice($array, 2, 12))), 2, ',', '.') . '</td>';
                                        }
                                    }
                                }
                                if ($i > 0) {
                        ...
                        if ($tipo == 'CREDITO') {
                            ...
                            for ($i = 0; $i <= 13; $i++) {
                                ...
                                switch ($i) {
                                    case 0: echo '<td align="right" colspan="2" height="40" style="background: #eee; height:30px"><b>Total de   rendimentos</b></td>';
                                        break;
                                    case 13:
                                        $total_rend[$i] = array_sum($total_rend);
                                        if ($total_rend[$i] == 0) {
                                            echo '<td style="background: #eee; height:30px"> - </td>';
                                        } else {
                                            echo '<td style="background: #eee; height:30px">R$ ' . number_format(array_sum($total_rend),"2",",",".") . '</td>';
                                        }
                                        break;
                                    default :
                                        if ($rendimentos == 0) {
                                            ...
                                            echo '<td style="background: #eee; height:30px"> R$ ' . number_format($rendimentos,"2",",",".") . '</td>';
                                        }
                                }
                            }
                            echo '</tr>';
                            ...
                            for ($i = 0; $i <= 13; $i++) {
                                ...
                                switch ($i) {
                                    case 0:
                                        echo '<td align="right" colspan="2" style="background: #eee; height:30px"><b>Total de descontos</b></td>';
                                        break;
                                    case 13: $total_desco[$i] = array_sum(array_slice($total_desco, 2, 12));
                                        if ($total_desco[$i] == 0) {
                                            echo '<td style="background: #eee; height:30px"> - </td>';
                                        } else {
                                            echo '<td style="background: #eee; height:30px">R$ ' . number_format(array_sum(array_slice($total_desco, 2, 12)), "2", ",", ".") . '</td>';
                                        }
                                        break;
                                    default :
                                        if($descontos == 0){
                                            ...
                                            echo '<td style="background: #eee; height:30px">R$ ' . number_format(formata_numero($descontos), "2", ",", ".") . '</td>';
                                        }
                                }
                            }
                            echo '</tr>';
                    ...
                    echo '<tr><td align="right" colspan="2" style="background: #eee; height:30px"><b>Valor líquido</b></td>';
                    ...
                    for ($i = 1; $i <= 13; $i++) {
                        ...
                        if ($soma == 0) {
                            ...
                            echo '<td style="background: #eee; height:30px">R$ ' . number_format($soma, "2", ",", ".") . '</td>';
                        }
                    }
                    echo '</tr>';
