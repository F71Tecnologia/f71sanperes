...
$qr_trab = mysql_query("SELECT * FROM autonomo WHERE id_autonomo = '$id_autonomo'");
...
$('#documento').uploadify({
			...
			'queueID'        : '<barra_proc></barra_proc>esso',
...
<div id="corpo">
   ...
   <form name="form" method="post"  enctype="multipart/form-data" action="ver_bolsista.php?bol=<?php echo $id_autonomo?>&pro=<?php echo $row_trab['id_projeto'] ?>&reg=<?php echo $row_trab['id_regiao'];?> ">
     ...
     <div>
        <fieldset id="fildset_documento">
         <legend>Anexo</legend>
        	<hr>
        	<?php 
			$qr_anexos = mysql_query("SELECT * FROM documento_autonomo_anexo WHERE id_autonomo = '$id_autonomo' AND id_upload = '$id_documento'  AND anexo_status = 1 ORDER BY ordem ASC") or die(mysql_error());
			...
			<?php			
				...
				?>
                  <img src="<?php echo 'documentos_autonomo/'.$row_anexos['anexo_nome'].'.'.$row_anexos['anexo_extensao']?>" height="250" width="200" >
                <?php
			}
            ...
            <tr>
            <td><input type="text" name="muda_ordem" class="muda_ordem_documento" value="<?=$row_anexos['id_upload']?>" alt="<?=$row_anexos['anexo_id']?>"/> </td>
            <td>
