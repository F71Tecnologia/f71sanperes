...
while($row_grupo = mysql_fetch_assoc($qr_grupo)):
		...
		while($row_subgrupo = mysql_fetch_assoc($qr_subgrupo)):
			...
			while($row_tipo = mysql_fetch_assoc($qr_tipo)):
				...
				$qr_contas = mysql_query("SELECT * FROM c_contas WHERE c_conta_status = '1' AND c_subtipo_id = '$row_subtipo[c_subtipo_id]' ");				
					
					if(mysql_num_rows($qr_contas) != 0 ) {
								
								if($row_grupo['c_grupo_id'] != $grupo_anterior) { 
									echo '<tr><td colspan="7" style="text-align:left;"> <h3>'.$row_grupo['c_grupo_nome']; 	
								}
								...
								echo $row_tipo['c_tipo_nome']; 
									}
									
								
								if($row_subtipo['c_subtipo_id'] != $tipo_anterior) { 
								echo $row_subtipo['c_subtipo_nome'].'</h3></td></tr>'; 							
								?>							
										...
										<tr>	
											...
											<td style="background-color:#BBB;">EDITAR</td>
											<td style="background-color:#BBB;">EXCLUIR</td>
										</tr>
										
							<?php	}
							
							while($row_contas = mysql_fetch_assoc($qr_contas)):
						
							?>
									<tr>
										<td><?php echo $row_contas['c_conta_id'];?></td>
										<td><?php echo $row_contas['c_conta_nome'];?></td>
										<td><?php echo $row_contas['c_conta_T'];?></td>										
										<td><?php echo $row_contas['c_conta_grau'];?></td>
										<td><a href="editar.php?tb=5&id=<?php echo $row_contas['c_conta_id'];?>">Editar</a></td>
										<td><a href="excluir.php?tb=5&id=<?php echo $row_contas['c_conta_id'];?>">Excluir</a></td>
									<?php
							endwhile;
				...
				$subtipo_anterior = $row_subtipo['c_subtipo_id'];		
			endwhile;
