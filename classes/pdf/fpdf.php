...
//EDITADO POR RENATO
function AliasSelPage($alias='{snb}')
{
        //Define an alias for total number of pages
        $this->AliasSelPage=$alias;
}

function AliasSelPageText($AliasSelPageText='')
{
        //Define an alias for total number of pages
        $this->AliasSelPageText=$AliasSelPageText;
}
//FIM EDIÇÃO
...
function _putpages()
{
	...
	if(!empty($this->AliasNbPages))
	{
        ...
        //EDITADO POR RENATO
        if(!empty($this->AliasSelPage))
        { 
            foreach ($this->pages as $key => $value)
            {
                $pos = strripos($value, $this->AliasSelPageText);
                if($pos)
                    $sel = $key;
            }
            ...
            if($sel)
                for($n=1;$n<=$nb;$n++)
                    $this->pages[$n] = str_replace($this->AliasSelPage,$sel,$this->pages[$n]);
        }
        //FIM EDIÇÂO
