...
class FuncoesClass {
    ...
    function alteraStatusCurso($id_curso, $usuario) {
        $sql = "UPDATE curso SET status = 0, status_reg = 0  WHERE id_curso = '{$id_curso}'";
        $qry = mysql_query($sql);
    ...
    function alteraStatusCursoByNome($nome, $usuario) {
        $sql = "UPDATE curso SET status = 0, status_reg = 0  WHERE nome = '{$nome}'";
        $qry = mysql_query($sql);
        $res = mysql_fetch_assoc($qry);
    ...
    public static function cadastraFuncao($usuario, $id_regiao, $id_usuario) {
        ...
        if (isset($_REQUEST['cadastrar'])) {
            ...
            $valor_hora_cooperado = $_REQUEST['valor_hora'];
            ...
            if ($total_cursos != 0) {
                ...
                $insere_curso = mysql_query("INSERT INTO curso
                                        ...
                                        mes_abono, id_user, data_cad, tipo, hora_mes, qnt_maxima, tipo_insalubridade, periculosidade_30, qnt_salminimo_insalu, quota, num_quota, valor_hora) VALUES
                                        ...
                                        '{$data_cad}', '{$contratacao}', '{$horas}', '{$qtd_contratacao}', '{$insalubridade}', '{$periculosidade}', '{$qtd_salarios}', '{$quota}', '{$parcela_quotas}', '{$valor_hora_cooperado}')") or die(mysql_error());
                ...
                if($contratacao == 2){
                    ...
                    foreach($nome_horario as $k => $valor) {
                        ...
                        $insere_horario = mysql_query("INSERT INTO rh_horarios (id_regiao,nome,obs,entrada_1,saida_1,entrada_2,saida_2,dias_semana,horas_mes,horas_semanais,salario,funcao,valor_dia,valor_hora,folga, dias_mes) VALUES
                          ...
                          '{$horas_mes[$k]}', '{$horas_semana[$k]}' ,'{$salario}', '{$id_curso}', '{$diaria_f}','{$hora_f}','{$folga}', '{$dias_mes[$k]}')") or die(mysql_error());
                    }
        ...
        if (isset($_REQUEST['atualizar'])) {
            ...
            if(($cbo == '') || ($nome_funcao == '')){
                ...
                $altera_curso = mysql_query("UPDATE curso SET nome = '{$nome_funcao}', campo2 = '{$nome_funcao}', cbo_nome = '{$cbo_nome}', cbo_codigo = '{$cbo_cod}', area = '{$area}', local = '{$local_fun}', parcelas = '{$parcelas}',
                                        descricao = '{$descricao}', data_alter = '{$data_cad}', hora_mes = '{$horas}', user_alter = '{$id_usuario}', qnt_maxima = '{$qtd_contratacao}', tipo_insalubridade = '{$insalubridade}',  qnt_salminimo_insalu = '{$qtd_salarios}', periculosidade_30 = '{$periculosidade}' WHERE id_curso = {$id_curso}") or die(mysql_error());
