<?php
/*
 * PHO-DOC - FeriasClass.php
 * 
 * ??-??-????
 * 
 * Classe de processamento de férias
 * 
 * Versão: 3.0.4667 - 07/12/2015 - Jacques - Foi suprimida a condição "...AND (status < 60 OR status = 200)" na query do método getFuncionarioFerias por não estár 
 *                                           aparecendo na listagem [Lista de Funcionários] alegado por Gimenes. 
 * Versão: 3.0.4965 - 15/12/2015 - Jacques - Revisão .4667 disfeita por solicitação de Junior à Ramon "...AND (status < 60 OR status = 200)" na query do método getFuncionarioFerias 
 ...
 * Versão: 3.0.6315 - 11/02/2016 - Jacques - Incluído no método getFuncionarioFerias o campo last_id_ferias que guardo o ID da última férias válida do Clt para impressão em lote.
 * Versão: 3.0.7828 - 14/03/2016 - Ramon   - Adicionando valor default para quarto parametro do metodo listaFuncionariosFerias
 * @author 
 *  
 */  
...
class Ferias {
     ...
     * @param type $debug
     */
    public function montaArrayFeriasByClt($clt, $ano) {
        
        $qryFerias = "SELECT A.*, B.data_demi
                        FROM rh_ferias AS A
                            LEFT JOIN rh_clt AS B ON(A.id_clt = B.id_clt) 
                        WHERE A.id_clt = '{$clt}' AND A.`status` = 1 AND YEAR(A.data_ini) = '{$ano}'";
        $sqlFerias = mysql_query($qryFerias) or die("Erro ao selecionar ferias");
        
        
        $dataPrimeiraParcelaDecimo = $ano.'-11-01';                                
        while ($d = mysql_fetch_assoc($sqlFerias)) {
            
            if($d['data_demi'] < $dataPrimeiraParcelaDecimo){
                $verificaMovAdiantamento = "SELECT A.id_clt, A.id_mov, A.nome_movimento, A.valor_movimento  
                        FROM rh_movimentos_clt AS A WHERE A.id_clt = '{$clt}' 
                        AND A.mes_mov = 17 AND A.cod_movimento = 80030 AND A.status = 1";
                $sqlVerificaAdiantamento = mysql_query($verificaMovAdiantamento);
                while($rows = mysql_fetch_assoc($sqlVerificaAdiantamento)){
                    $this->dadosNovo[$d['mes']][5027]['movimento']  = "ADIANTAMENTO DE 13 º SALÁRIO";
                    $this->dadosNovo[$d['mes']][5027]['tipo']       = "CREDITO";
                    $this->dadosNovo[$d['mes']][5027]['valor']      = $rows['valor_movimento'];
                }
            }else if (!empty($d['valor_total_ferias']) && $d['valor_total_ferias'] != 0.00) {
                $this->dadosNovo[$d['mes']][5027]['movimento']  = "ADIANTAMENTO DE 13 º SALÁRIO";
                $this->dadosNovo[$d['mes']][5027]['tipo']       = "CREDITO";
                $this->dadosNovo[$d['mes']][5027]['valor']      = $d['adiantamento13'];
            }
            
            if (!empty($d['valor_total_ferias']) && $d['valor_total_ferias'] != 0.00) {
                $this->dadosNovo[$d['mes']][5037]['movimento']  = "FERIAS";
                $this->dadosNovo[$d['mes']][5037]['tipo']       = "CREDITO";
                $this->dadosNovo[$d['mes']][5037]['valor']      = $d['salario'];
            }
            
            if (!empty($d['salario_variavel']) && $d['salario_variavel'] != 0.00) {
                $this->dadosNovo[$d['mes']][50258]['movimento']  = "SALARIO VARIAVEL FÉRIAS";
                $this->dadosNovo[$d['mes']][50258]['tipo']       = "CREDITO";
                $this->dadosNovo[$d['mes']][50258]['valor']      = $d['salario_variavel'];
            }
            
            if (!empty($d['umterco']) && $d['umterco'] != 0.00) {
                $this->dadosNovo[$d['mes']][5039]['movimento']  = "1/3 ACRESCIMO CONSTITUCIONAL FÉRIAS";
                $this->dadosNovo[$d['mes']][5039]['tipo']       = "CREDITO";
                $this->dadosNovo[$d['mes']][5039]['valor']      = $d['umterco'];
            }
            
            if (!empty($d['abono_pecuniario']) && $d['abono_pecuniario'] != 0.00) {
                $this->dadosNovo[$d['mes']][5041]['movimento']  = "ABONO PECURIÁRIO FÉRIAS";
                $this->dadosNovo[$d['mes']][5041]['tipo']       = "CREDITO";
                $this->dadosNovo[$d['mes']][5041]['valor']      = $d['abono_pecuniario'];
            }
            
            if (!empty($d['umterco_abono_pecuniario']) && $d['umterco_abono_pecuniario'] != 0.00) {
                $this->dadosNovo[$d['mes']][5051]['movimento']  = "1/3 SOBRE ABONO PECURIÁRIO FÉRIAS";
                $this->dadosNovo[$d['mes']][5051]['tipo']       = "CREDITO";
                $this->dadosNovo[$d['mes']][5051]['valor']      = $d['umterco_abono_pecuniario'];
            }
            
            if (!empty($d['pensao_alimenticia']) && $d['pensao_alimenticia'] != 0.00) {
                $this->dadosNovo[$d['mes']][80026]['movimento']  = "PENSÃO ALIMENTICIAS SOBRE FÉRIAS";
                $this->dadosNovo[$d['mes']][80026]['tipo']       = "DEBITO";
                $this->dadosNovo[$d['mes']][80026]['valor']      = $d['pensao_alimenticia'];
            }
            
            if (!empty($d['inss']) && $d['inss'] != 0.00) {
                $this->dadosNovo[$d['mes']][5035]['movimento']  = "INSS SOBRE FÉRIAS";
                $this->dadosNovo[$d['mes']][5035]['tipo']       = "DEBITO";
                $this->dadosNovo[$d['mes']][5035]['valor']      = $d['inss'];
            }
            
            if (!empty($d['ir']) && $d['ir'] != 0.00) {
                $this->dadosNovo[$d['mes']][5036]['movimento']  = "IR SOBRE FÉRIAS";
                $this->dadosNovo[$d['mes']][5036]['tipo']       = "DEBITO";
                $this->dadosNovo[$d['mes']][5036]['valor']      = $d['ir'];
            }               
        }

        return $this->dadosNovo;
    }
...
//            echo "Dados CLT: INI {$row_ferias['data_ini']} - FIM {$row_ferias['data_retorno']}<br/>";
        ...
        if (!empty($num_ferias)) {
            ...
            if ($row_ferias['data_ini'] >= $data_inicio and $row_ferias['data_ini'] <= $data_fim) {
                ...
                $fim = ($row_ferias['data_fim'] < $data_fim) ? $row_ferias['data_fim'] : $data_fim; //aki + 1 day
...
//                    $dias_ferias = abs((int) floor((strtotime($inicio) - strtotime($fim)) / 86400));
                    ...
                    $dias_ferias = $countDiasFerias;
                    
                    if($_COOKIE['logado'] == 179){
                        echo " - Inicio {$inicio}<br/>";
                        echo " - Fim {$fim}<br/>";
                        echo " - DIAS dias_feriass {$dias_ferias}<br/>";
                    }
...
//                    }
    ...
    public function listaFuncionariosFerias($id_regiao, $id_projeto = null, $pesquisa = NULL, $data_ini_fmt = NULL, $id_unidade = null) {
       ...
       //echo $query_projeto . "<br>";
        ...
        $array_clt = $this->getFuncionarioFerias($id_regiao, $id_projeto, $pesquisa, TRUE, $id_unidade);
    ...
    public function getFuncionarioFerias($id_regiao, $id_projeto = null, $pesquisa = NULL, $type_id_projeto = FALSE, $id_unidade = null) {
        ...
        if (!empty($pesquisa)) {
            ...
            $filtro_data = (!empty($data_ini_fmt)) ? " AND r.id_clt IN (SELECT id_clt FROM rh_ferias WHERE DATE_FORMAT(data_ini,'%d/%m/%Y')='{$data_ini_fmt}')" : '';
            
        }
        
        $consulta_unidade = ($id_unidade) ? "AND r.id_unidade = '{$id_unidade}'" : false;
        
        $auxStatus = (!in_array($_COOKIE['logado'], [158,179,276,9,347,346,345,348,349,350,353,354,355,371,379,381])) ? " AND (r.status < 60 OR r.status = 200) " : null;
        ...
        $query = "
                ...
                WHERE r.id_regiao = {$id_regiao} $cond_projeto $auxPesquisa $filtro_data $consulta_unidade $auxStatus
    ...
    public function relatorioFerias($id_regiao, $id_projeto = null, $funcao = null, $mesIni = null, $mesFim = null, $anoIni = null, $anoFim = null) {
        if (!empty($funcao) && $funcao > 0) {
            $auxFuncao = " AND A.id_curso = {$funcao} ";
        }
        if (!empty($id_projeto) && $id_projeto > 0) {
            $auxProjeto = " AND A.id_projeto = {$id_projeto} ";
        }

        $sqlClts = "SELECT A.nome, A.id_clt, data_entrada, id_projeto, date_format(A.data_entrada, '%d/%m/%Y') data_entrada2, DATE_FORMAT(C.inicio, '%d/%m/%Y') inicioBR, 
            ADDDATE(ADDDATE(A.data_entrada, INTERVAL + 1 YEAR), INTERVAL -1 DAY) pAno,
            FLOOR(DATEDIFF(NOW(),A.data_entrada) / 365) tempoEmpresa,
            B.nome funcao, B.valor salario,A.id_unidade
            FROM rh_clt A LEFT JOIN curso B ON (A.id_curso = B.id_curso)
            LEFT JOIN rh_ferias_programadas AS C ON(A.id_clt = C.id_clt AND C.status = 1)
            WHERE A.id_regiao = '{$id_regiao}' AND (A.status < '60' OR A.status = '200') $auxMes $auxFuncao $auxProjeto
            ORDER BY A.nome ASC";

        $rsClts = mysql_query($sqlClts) or die(mysql_error());

        $numero_clts = mysql_num_rows($rsClts);

        if ($numero_clts > 0) {
            while ($row_clt = mysql_fetch_array($rsClts)) {
                ...
                $sql_ferias = "SELECT 
                       ...
                       FROM rh_ferias
                       WHERE id_clt = {$row_clt['id_clt']} AND status = '1' ORDER BY data_fim DESC";
                $qr_ferias = mysql_query($sql_ferias) or die(mysql_error());
//            $numFerias = mysql_num_rows($qr_ferias);
                $ferias = mysql_fetch_assoc($qr_ferias);

                for ($i = 0; $i <= $row_clt['tempoEmpresa']; $i++) {
                    $dataEntrada = explode('-', $row_clt['data_entrada']);
                    $dataEntrada = $dataEntrada[0] + $i . '-' . $dataEntrada[1] . '-' . $dataEntrada[2];

                    $pAno = explode('-', $row_clt['pAno']);
                    ...
                    if ($dataEntrada >= $ferias['periodoGozadoIni'] AND $pAno <= $ferias['periodoGozadoFim'] AND ! empty($ferias['periodoGozadoIni'])) {
                        ...
                        $periodoGozado .= "$periodoGozadoIni &agrave; $periodoGozadoFim, ";
                    } else if ($pAno < (date('Y') + 1) . '-01-01') {
                        //} else {
                        if ($dataEntrada <= date("Y-m-d") AND $pAno >= date("Y-m-d") AND $row_clt['tempoEmpresa'] > 0) {
                            ...
                            $periodoNaoGozado .= "$periodoNaoGozadoIni &agrave; $periodoNaoGozadoFim, ";
                        }
                        if ($pAno < date("Y-m-d")) {
                            $arrayPeriodos[] = $pAno;
                            ...
                            $periodoVencido .= "$periodoVencidoIni &agrave; $periodoVencidoFim, ";
                        }
                    }
                }
                ...
                if (empty($periodoGozado)) {
                    ...
                    $periodoGozado = substr($periodoGozado, 0, -2);
                }
                if (empty($periodoVencido)) {
                    $periodoVencido = '---';
                } else {
                    $periodoVencido = substr($periodoVencido, 0, -2);
                }
                if (empty($periodoNaoGozado)) {
                    $periodoNaoGozado = '---';
                } else {
                    $periodoNaoGozado = substr($periodoNaoGozado, 0, -2);
                }

                $result_pro = mysql_query("SELECT * FROM projeto WHERE id_projeto = '$row_clt[id_projeto]' AND status_reg = '1' ");
                $row_pro = mysql_fetch_array($result_pro);

                if (empty($mesIni) || $mesIni <= 0) {
                    $retorno[] = array(
                        'projeto_nome' => $row_pro['nome'],
                        'id_clt' => $row_clt['id_clt'],
                        'clt_nome' => $row_clt['nome'],
                        'funcao' => $row_clt['funcao'],
                        'salario' => number_format($row_clt['salario'], 2, ',', '.'),
                        'data_entrada' => $row_clt['data_entrada2'],
                        'periodoGozado' => $periodoGozado,
                        'periodoVencido' => $periodoVencido,
                        'periodoNaoGozado' => $periodoNaoGozado,
                        'inicioBR' => $row_clt['inicioBR']
                    );
                } else {
                    $exibe = 0;
                    if (is_array($arrayPeriodos)) {
                        foreach ($arrayPeriodos as $final) {
                            if ($anoIni . sprintf("%02d", $mesIni) . "01" <= str_replace('-', '', $final) AND $anoFim . sprintf("%02d", $mesFim) . "31" >= str_replace('-', '', $final)) {
                                $exibe = 1;
                                break;
                            }
                        }
...
//                    "prazo_primerio_periodo"    => 
    ...
    public function getAvisosFerias($id_master, $arrayDiasAviso, $id_regiao = null, $diasLicenca = true, $tipoPeriodo = null){
        ...
        $sqlClt = mysql_query("
        SELECT A.id_clt, A.id_regiao, A.nome, A.data_entrada, B.nome nomeProjeto, C.regiao nomeRegiao, D.unidade nomeUnidade
        FROM rh_clt A LEFT JOIN projeto B ON (A.id_projeto = B.id_projeto) LEFT JOIN regioes C ON (A.id_regiao = C.id_regiao) LEFT JOIN unidade D ON (A.id_unidade = D.id_unidade)
        ...
        while($rowClt = mysql_fetch_assoc($sqlClt)){
            ...
            $sqlFerias = mysql_query("SELECT data_aquisitivo_ini, data_aquisitivo_fim FROM rh_ferias WHERE status = 1 AND id_clt = {$rowClt['id_clt']} ORDER BY data_aquisitivo_fim DESC LIMIT 1;");
...
//        echo $sql."<br><br>";
    ...
    /**
     * Lista de solicitações de férias executadas pelo PORTAL
     */
    public function listaSolicitacoesFerias($projeto, $mes, $ano, $clt = null, $status = null){
       
        $criteria = "";
        if($clt != null){
            $criteria .= " AND B.id_clt = '{$clt}' ";
        }
        
        if($status != null){
            $criteria .= " AND A.status IN($status) ";
        }else{
            $criteria .= " AND A.status IN(1,2,3,4,5) ";
        }
