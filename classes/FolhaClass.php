 ...
 * Versão: 3.0.0301 - 14/12/2015 - Jacques - O método getMultaFgts não estava fazendo o select com verificação do status na query
 * Versão: 3.0.0302 - 14/12/2015 - Jacques - O método getMultaFgts também estava retornando valor NULL na consulta
 * Versão: 3.0.0551 - 14/12/2015 - Jacques - No método getMultaFgts foi suprimido a substituição do valor do recolhimento pelo campo a5031 (INSS sobre 13º).
 *                                           Ninguém soube explicar a origem dessa substituição e a razão de se existir.
 *
 * @author Sinésio
 */ 
...
class Folha extends Calculo_Folha{
    ...
    public function getDadosClt($clt, $mes = null, $ano = null) {
        $sql = mysql_query("SELECT A.id_clt, A.id_regiao, A.nome,CONCAT(A.endereco,A.numero,', ',A.bairro,', ',A.cidade,' - ',A.uf) as endereco,
            ...
            F.especifica AS status_especifica, A.status AS status_clt, G.nome AS nome_projeto
            FROM rh_clt as A 
...
//        }
        ...
        $sqls = "SELECT A.id_folha, C.id_projeto, B.terceiro, A.id_clt, A.mes, A.ano, C.nome,
                ...
                A.t_imprenda, if(A.status_clt = 70,A.base_inss * 0.08,A.base_inss * 0.08) AS fgts, 
...
//                LEFT JOIN rh_recisao AS E ON(A.id_clt = E.id_clt AND A.mes = date_format(E.data_demi,'%m') AND E.`status` = 1)
//                {$ferias}
//                WHERE A.id_clt = '{$clt}' AND A.status = 3 
            ...
            if ($rows['terceiro'] == 2) {
                if (!empty($rows['salario_base'])) {
                    ...
                    if ($rows['salario_base'] != 0.00 ) {
                        if ($rows['status_clt'] > 60 && $rows['status_clt'] != 80 && $rows['status_clt'] != 70 && $rows['status_clt'] != 69 && $rows['status_clt'] != 68 && $rows['status_clt'] != 67 && $rows['status_clt'] != 200) {
...
//                    $this->dados["5029"][$mes] = $rows['decimo_terceiro'] + $rows['rend'] - $rows['inss_dt'] - $rows['ir_dt'];
            ...
            if ($rows['status_clt'] > 60 && $rows['status_clt'] != 80 && $rows['status_clt'] != 70 && $rows['status_clt'] != 200) {
...
//                    }
            ...
            if($_COOKIE['debug'] == 666){
                echo "AAAAAAAA"; print_array($this->dados["5020"][$mes]);
            }
...
//                    WHERE id_movimento IN(" . $rows['ids_movimentos_estatisticas'] . ") AND id_clt = '{$clt}'";
            ...
            $sql_movs = "SELECT A.tipo_qnt,A.qnt, A.qnt_horas, A.mes_mov, A.ano_mov, A.lancamento, A.id_folha, 
                    ...
                    FROM rh_movimentos_clt AS A
                    LEFT JOIN rh_movimentos AS B ON(A.cod_movimento = B.cod)    
                    WHERE id_movimento IN(" . $ids_movimentos . ") AND id_clt = '{$clt}' ORDER BY lancamento ASC";
            if($_REQUEST['debug'] == 666){
...
//                }        
            ...
            while ($d = mysql_fetch_assoc($sql_movs)) {
                ...
                /* Leonardo 2017-07-11
                 * array com todos os movimentos de hora extra 
                 * para exibir corretamente a quantidade de horas da hora extra
                 */
                $arr_mov_horas_dias = [
                    80024,8080,80025,80086,80051,80052,80057,80058,80059,80060,80061,
                    80062,80063,80064,80065,80066,80067,80068,80069,80070,80071,80072,
                    80073,80074,80075,80076,80095,80096,80097,80098,80099,80094,80100,
                    80101,80102,80103,80104,80105,80106,80107,80108,80109,80110,80111,
                    80112,80113,80114,80115,80116,80117,80118,80119,80120,80121,80122,
                    80123,80124,80125,80126,80127,80128,80129,80130,80276,80277,80278,
                    80279,80280,80281,80282,80283,80284,80285,80286,80287,80288,80289,
                    80290,80291,80292,80293,80294,80295,80296,80297
                ];
                
                if($d['cod_movimento'] == '9997'){
                    $ref = "5";
                }else if(in_array($d['cod_movimento'],$arr_mov_horas_dias)){
                    
                    if($d['tipo_qnt'] == 1){
                        
                       $d['percentual'] =  substr($d['qnt_horas'], 0, -3);
                       $tipagem = "Horas";
...
//                }
                ...
                if ($d['lancamento'] == '2') {
                    ...
                    $this->dados[$d['cod_movimento']]['qnt'] = $d['percentual'];
     ...
     * @return type
     */
    public function getCltsByIdFolha($id_folha, $id_clt = null, $ini = null, $id_funcao = null, $so_ativos = false){
        $auxClt = (!empty($id_clt) && $id_clt != 'todos') ? " AND id_clt = $id_clt " : '';
        $auxIni = (!empty($ini)) ? " LIMIT $ini, 50 " : '';
        $auxAti = ($so_ativos) ? " AND status_clt < 60 " : '';
        $auxFuncao = (!empty($id_funcao)) ? " AND id_curso = $id_funcao " : '';
        $query = "SELECT id_clt FROM rh_folha_proc WHERE id_folha IN({$id_folha}) $auxAti $auxClt $auxFuncao ORDER BY nome $auxIni";
        //if($_COOKIE['logado'] == 35) echo $query;
    ...
    public function getDadosFolhaById($folha, $status = null, $clt = null) {
        ...
        $query = "SELECT A.id_folha, A.id_clt, D.data_inicio, D.data_fim, A.id_projeto, A.nome, A.salbase, A.sallimpo, A.salliquido, A.base_inss, A.valor_ferias, A.rend, A.base_irrf, if(dias_trab < 30, A.sallimpo_real,A.sallimpo) AS salario, C.nome AS funcao, DATE_FORMAT(B.data_entrada,'%d/%m/%Y') AS data_admissao, A.mes AS mes_proc, A.ano AS ano_proc,
...
//        }           
     ...
     */
    public function setArrayLicenca( array $arrayLicenca){
    ...
    /**
     * 
     * @param type $arrayFerias
     */
    public function setArrayFerias(array $arrayFerias){
        $this->arrayFerias = $arrayFerias;
    }
    
    /***
     * 
     */
    public function getArrayFerias(){
        return $this->arrayFerias;
    }
     ...
     * @param type $projeto
     */
    public function setProjetoFolha($projeto){
     ...
     * @return type
     */
    public function getProjetoFolha(){
        return $this->projeto;
    }
    
    /**
     * 
     * @param type $objFerias
     */
    public function setObjFerias($objFerias){
     ...
     * @return type
     */
    public function getObjFerias(){
        return $this->objFerias;
    }
     ...
     * @param type $regiao
     */
    public function setRegiaoFolha($regiao){
        $this->regiao = $regiao;
    }
     ...
     * @return type
     */
    public function getRegiaoFolha(){
        return $this->regiao;
    }
    
    /**
     * 
     * @param type $inssFerias
     */
    public function setInssFerias($inssFerias){
     ...
     * 
     */
    public function getInssFerias(){
        return $this->inssFerias;
    }
    
    /**
     * 
     * @param type $irrfFerias
     */
    public function setIrrfFerias($irrfFerias){
     ...
     * 
     */
    public function getIrrfFerias(){
        return $this->irrfFerias;
    }
    ...
    public function getResumoPorMovimentoClt($id_folha, $id_clt){
        
        $sql_folha = "SELECT A.*, B.ids_movimentos_estatisticas FROM rh_folha_proc A LEFT JOIN rh_folha B ON A.id_folha = B.id_folha WHERE A.id_folha = '{$id_folha}' AND A.status = '3' AND id_clt = $id_clt AND A.status_clt NOT IN (60,61,62,63,64,65)";
        $qr_folha = mysql_query($sql_folha);
        $row_folha = mysql_fetch_assoc($qr_folha);
        
        $movimento['base']['dt'] = $row_folha['valor_dt'];
        
        $sqlMov = "SELECT A.cod_movimento, A.tipo_movimento, A.nome_movimento, valor_movimento as valor,
            qnt,SEC_TO_TIME(SUM(TIME_TO_SEC(qnt_horas))) as qnt_horas, B.percentual
            FROM rh_movimentos_clt as A
            INNER JOIN rh_movimentos as B ON A.id_mov = B.id_mov
            WHERE id_movimento IN({$row_folha['ids_movimentos_estatisticas']}) AND id_clt = $id_clt GROUP BY A.id_mov";
        ...
        while($mov = mysql_fetch_assoc($qrMov)){
           $horas = explode(':', $mov['qnt_horas']); 
           $horas = ($mov['qnt_horas'] != '00:00:00')? $horas[0].':'.$horas[1].' horas' : '' ;
           $movimento['movs'][$mov['cod_movimento']]['nome'] = $mov['nome_movimento'];
           $movimento['movs'][$mov['cod_movimento']]['tipo'] = $mov['tipo_movimento'];
           $movimento['movs'][$mov['cod_movimento']]['qnt']   = $mov['qnt'];
           $movimento['movs'][$mov['cod_movimento']]['qnt_horas'] = $horas;
           $movimento['movs'][$mov['cod_movimento']]['percentual'] = $percent;
           $movimento['movs'][$mov['cod_movimento']]['valor'] = $mov['valor'];
        }
        
        $totalRendimento = $movimento['base']['dt'];
        foreach ($movimento['movs'] as $value) {
        ...
        $sql_base_nao_incide_fgts = "SELECT A1.id_clt, SUM(A1.base_inss - IFNULL(C1.valor_movimento,0) - IFNULL(E.valor_movimento,0)) decimo_terceiro, A1.base_inss
                ...
                WHERE A1.status = 3 AND B1.status = 3 AND B1.terceiro = 1 AND B1.tipo_terceiro IN (2,3) AND B1.projeto = '{$row_folha['id_projeto']}'
                AND MONTH(R.data_demi) = {$row_folha['mes']} AND YEAR(R.data_demi) = {$row_folha['ano']} AND R.rescisao_complementar = 0 AND R.status = 1 AND R.motivo IN (61,64,66) AND A1.id_clt = $id_clt";//"(61,64,66);";
        
//                                  
        $qry_base_nao_incide_fgts = mysql_fetch_assoc(mysql_query($sql_base_nao_incide_fgts));
        
//        if ($_COOKIE['debug'] == 666) {
//            print_array($sql_base_nao_incide_fgts);
//        }
        
        $base_fgts_total = $totalRendimento - $movimento['movs'][5050]['valor'] - $movimento['movs'][80019]['valor'] - $movimento['movs'][80031]['valor'] - $qry_base_nao_incide_fgts['decimo_terceiro'];
        
        /**
         * GAMBI PQ NÃO TEM JEITO
         */
//        if ($id_folha == 105) {
//            $base_fgts_total -= 2.94
//        }
        //FIM DA GAMBI
        $fgts['rendimento'] = $totalRendimento;
        $fgts['5050'] = $movimento['movs'][5050]['valor'];
        $fgts['80019'] = $movimento['movs'][80019]['valor'];
        $fgts['80031'] = $movimento['movs'][80031]['valor'];
        $fgts['n_incide'] = $qry_base_nao_incide_fgts['decimo_terceiro'];
        $fgts['base'] = $base_fgts_total;
        $fgts['fgts'] = $base_fgts_total * 0.08;
        return $fgts;
    }
     ...
     * @param type $folha
     */
    public function totalizadorBaseFolha($tipo = 'individual'){
        ...
        try{
            //BUSCA DADOS DO PROJETO (RAT)
            $proj = $this->getProjetoFolha();
            if(isset($proj) && !empty($proj)){
                $qryEmp = "SELECT aliquotaRat FROM rhempresa WHERE id_projeto = '{$proj}'";
                $sql = mysql_query($qryEmp) or die('Erro ao montar totalizador "empresa"');
            ...
            if($tipo == 'totalizador'){
                ...
                $qry = "SELECT 
                        COUNT(A.id_clt) as totalParticipantes,
                        SUM(A.salliquido) as totalLiquido,
                        SUM(A.base_inss) as totalBaseInss,
                        SUM(A.inss) as totalInss,
                        SUM(A.imprenda) as totalIrrf,
                        -- SUM(A.base_inss_ferias) as totalBaseInssFerias,
                        SUM(A.base_inss) as totalBaseFgts,
                        SUM(A.base_inss * 0.20) as totalInssEmpresa,
                        SUM(A.base_inss * {$rat}) as totalInssRat,
                        SUM(A.base_inss * 0.01) as totalInssTerceiro,
                        SUM(A.base_irrf) as totalBaseIrrf 
                    FROM rh_folha_proc AS A 
                    WHERE A.id_folha = '{$folha}' AND A.status > 0";

                $sql = mysql_query($qry) or die('Erro ao montar totalizador: '.$qry);        
                ...
                while($rows = mysql_fetch_assoc($sql)){
                     ...
                     * 
                     */
                    $dados['participantes']['nome'] = 'Participantes';
                    $dados['participantes']['valor'] = $rows['totalParticipantes'];
                    
                    /**
                     * 
                     */
                    $dados['liquido']['nome'] = 'Total Liquido';
                    $dados['liquido']['valor'] = $rows['totalLiquido'];

                    /**
                     * 
                     */
                    $dados['total_inss']['nome'] = 'INSS Recolhido';
                    $dados['total_inss']['valor'] = $rows['totalInss'];
                    
                    /**
                     * 
                     */
                    $dados['total_irrf']['nome'] = 'IRRF Recolhido';
                    $dados['total_irrf']['valor'] = $rows['totalIrrf'];
                    
                    /**
                     * 
                     */
                    $dados['base_inss']['nome'] = 'Base INSS';
                    $dados['base_inss']['valor'] = $rows['totalBaseInss'];

                    /**
                     * 
                     */
                    $dados['base_inss_ferias']['nome'] = 'Base INSS Férias';
                    $dados['base_inss_ferias']['valor'] = $rows['totalBaseInssFerias'];

                    /**
                     * 
                     */
                    $dados['base_fgts']['nome'] = 'Base FGTS';
                    $dados['base_fgts']['valor'] = $rows['totalBaseFgts'];

                    /**
                     * 
                     */
                    $dados['inss_empresa']['nome'] = 'INSS (Empresa)';
                    $dados['inss_empresa']['valor'] = $rows['totalInssEmpresa'];

                    /**
                     * 
                     */
                    $dados['inss_rat']['nome'] = 'INSS (RAT)';
                    $dados['inss_rat']['valor'] = $rows['totalInssRat'];

                    /**
                     * 
                     */
                    $dados['inss_terceiro']['nome'] = 'INSS (Terceiro)';
                    $dados['inss_terceiro']['valor'] = $rows['totalInssTerceiro'];
                    
                    /**
                     * 
                     */
                    $dados['base_irrf']['nome'] = "BASE IRRF";
                    $dados['base_irrf']['valor'] = $rows['totalBaseIrrf'];
                ...
                $qry = "SELECT 
                        A.id_clt,
                        A.nome,
                        (A.base_inss) as totalBaseInss,
                        (A.base_inss_ferias) as totalBaseInssFerias,
                        (A.base_inss) as totalBaseFgts,
                        (A.base_inss * 0.20) as totalInssEmpresa,
                        (A.base_inss * 0.05) as totalInssRat,
                        (A.base_inss * 0.01) as totalInssTerceiro,
                        (A.base_irrf) as totalBaseIrrf 
                    FROM rh_folha_proc AS A 
                    WHERE A.id_folha = '{$folha}' AND A.status > 0";

                $sql = mysql_query($qry) or die('Erro ao montar totalizador');        
     ...
     * @param type $mes
     * @param type $ano
     * @param type $tipo Verifica se é folha de 13 ou não 1=folha normal, 2=folha de 13
     * @param type $projeto
     */
    public function getDadosFolhaApi($mes,$ano,$tipo=1,$projeto=false){
        $dadosFolha = false;
        if($mes > 0 && $mes <=12){
            
            $sql = "SELECT C.cnpj,'1' as tipo_salario,A.sallimpo,A.inss,A.imprenda,A.salliquido,A.fgts,'' as inssEmp, 
                    A.desco AS outrodesconto,'' as obs,D.nome as cargo, B.mes, A.nome, A.cpf
                    FROM rh_folha_proc AS A
                    LEFT JOIN rh_folha AS B ON (A.id_folha = B.id_folha)
                    LEFT JOIN rhempresa AS C ON (B.regiao = C.id_regiao)
                    LEFT JOIN curso AS D ON (A.id_curso = D.id_curso)
                    WHERE B.mes = '{$mes}' AND B.ano = '{$ano}'";
                    
            $rs = mysql_query($sql);
            $dadosFolha = $this->api->montaRetorno($rs);
            
        }else{
            return array("Erro: mês inválido");
        }
