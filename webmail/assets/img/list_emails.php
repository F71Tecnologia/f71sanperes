...
if ($emails) {
    ...
    for ($c=($page * MAX_EMAILS_FOR_PAGE) - MAX_EMAILS_FOR_PAGE;
         $c <= $page * MAX_EMAILS_FOR_PAGE;
         ++$c) {

        $email_uid = $emails[$c];
        if (! $email_uid) continue;
	...
	$qr_fav = mysql_query("SELECT id FROM webmail_msg_favorite WHERE id_message = '{$email_uid}' LIMIT 1");
        ...
        $forwarded = ($count_rw == 1) ? 'est_02' : 'est_01'; // Imagem com a estrela.
	$favorite = ($count_fav == 1) ? true : false;
