...
//    }
    ...
    $verifica_recisao = "SELECT * FROM rh_recisao AS A WHERE {$criteria} AND A.`status` = 0";
...
if ((isset($_REQUEST['confirmar_rescisao']))) {
    foreach ($_REQUEST[id_clt] as $key => $value) {
        $sqlMotivo = mysql_fetch_assoc(mysql_query("SELECT motivo, data_aviso, data_demi FROM rh_recisao WHERE id_clt = $value AND status = 0 LIMIT 1"));
        
        mysql_query("UPDATE rh_recisao SET status=1 WHERE id_clt = $value LIMIT 1;");
        
        mysql_query("UPDATE rh_clt SET status=$sqlMotivo[motivo], data_aviso='$sqlMotivo[data_aviso]', data_demi='$sqlMotivo[data_demi]', data_saida='$sqlMotivo[data_demi]' WHERE id_clt = $value LIMIT 1;");
    }
    
    mysql_query("INSERT INTO log (id_user, id_regiao, tipo_user, grupo_user, local, horario, ip, acao)
    VALUES ('$usuario[id_funcionario]', '$usuario[id_regiao]', '$usuario[tipo_usuario]', '$usuario[grupo_usuario]', 'Confirmação de Rescisão em Lote', NOW(), '$ip', 'Confirmação de Rescisão em Lote id $_REQUEST[header_lote]')") or die("Erro Inesperado<br><br>" . mysql_error());
        ...
        <script>
            $(function () {
                ...
                $('#confirmar_rescisao1').click(function(){
                    thickBoxConfirm("Confirmar Rescisão", "Você deseja realmente realizar esta rescisão?", 300, 200, function(data){
                        if(data){                                       
                            if(data == true){
...
//                            echo "<pre>"; 
                        ...
                        $total_reflexo_dsr += 0;
                        ?></td> <!-- 59 -->
                                <td align="left" class="esconder"><?php echo "R$ 0,00";
...
<!--                    <div class="totalizador">
                    ...
                    <?php if($_COOKIE[logado] == 257){
                        echo "<div class='totalizador' style='height: auto; text-align: center;'>";
                        echo "<input type='button' id='confirmar_rescisao1' value='Confirmar Rescisão'><input type='submit' style='display: none;' id='confirmar_rescisao' name='confirmar_rescisao' value='Confirmar Rescisão'>";
                        echo "</div>";
                    } ?>
                    <?php } ?>
