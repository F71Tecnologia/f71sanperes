...
if (isset($_REQUEST['gerar']) || isset($_REQUEST['todos_projetos'])) {
    ...
    if ($tipo_contratacao == 2) {
        $str_qr_relatorio = "SELECT A.nome, B.nome1, B.nome2, B.nome3,
                            ...
                            date_format(B.data1, '%d/%m/%Y') AS data1br,
                            date_format(B.data2, '%d/%m/%Y') AS data2br,
                            date_format(B.data3, '%d/%m/%Y') AS data3br,
                            date_format(B.data4, '%d/%m/%Y') AS data4br,
                            date_format(B.data5, '%d/%m/%Y') AS data5br,
                            date_format(B.data6, '%d/%m/%Y') AS data6br
                            FROM rh_clt AS A
...
<html lang="pt">
    <head>
        ...
        <title>:: Intranet :: Relatório de Participantes com Dependentes</title>
    ...
    <body>
        ...
        <div class="container">
            <div class="page-header box-rh-header"><h2><span class="fa fa-users"></span> - RECURSOS HUMANOS <small> - Relatório de Participantes com Dependentes</small></h2></div>
            <form action="" method="post" class="form-horizontal top-margin1" name="form" id="form">  
                    ...
                    <div class="panel-footer text-right hidden-print controls">
                        <span class="fleft erro"><?php if ($verifica_dirf != 0) echo 'Arquivo já existente!'; ?></span>
                         <?php if (!empty($qr_relatorio) && (isset($_POST['gerar'])) || isset($_REQUEST['todos_projetos'])) { ?>
                            <button type="button" onclick="tableToExcel('tabela', 'Participantes com Dependentes')" value="Exportar para Excel" class="btn btn-success" ><span class="fa fa-file-excel-o"></span> Exportar para Excel</button>
                        <?php } ?>
                        <?php
                ...
                <?php if (!empty($qr_relatorio) && (isset($_POST['gerar'])) || isset($_REQUEST['todos_projetos'])) { ?>
                    <table class="table table-striped table-bordered table-condensed table-hover text-sm valign-middle" id="tabela">
                        <?php while ($row_rel = mysql_fetch_assoc($qr_relatorio)) { ?>
                            <?php
                            if ($nome_anterior != $row_rel['nome']) {
                                $cont = 0;
                                ?>
                                <thead>
                                    <tr>
                                        <th align="center" style="background:#d7d7d7" colspan="2"><?php echo $row_rel['nome']; ?></th>
                                    </tr>
                                    <tr>
                                ...
                                <tbody>
                                    <?php
                                    for ($num_dependente = 1; !empty($row_rel['nome' . $num_dependente]); $num_dependente++) {
                        ...
                        <?php } $nome_anterior = $row_rel['nome']; ?>
                    <?php } ?>
                </table>
            <?php } ?>
        </form>
