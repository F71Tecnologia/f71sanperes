...
//	$sql = "SELECT * FROM curso WHERE campo3 = '$projeto' AND id_regiao='$id_regiao' AND tipo = '3' ORDER BY campo3 ASC";
	$sql = "SELECT * FROM curso WHERE campo3 = '{$row['id_projeto']}' AND id_regiao='$id_regiao' AND tipo = '3' ORDER BY campo3 ASC";
	$result_curso = mysql_query($sql);
		  ...
		  if(!empty($verifica_curso)) {
			  ...
			  while($row_curso = mysql_fetch_array($result_curso)) {
				    ...
				    echo ">$row_curso[0] - {$row_curso['nome']} - {$row_curso['salario']}</option>\n";
			  }
			       print "</select>";
		  } else {
  ...
  <tr>
    ...
    <td>
        <?php 
        $sql_prestador =  "SELECT * FROM prestadorservico WHERE id_projeto = '$projeto' AND id_regiao='$id_regiao'";
...
header("Location: ver_terceiro.php?id=$id&reg=$regiao&bol=$row_id_participante&pro=$id_projeto&sucesso=cadastro&tipo=$tipo_contratacao");
exit;
}
?>