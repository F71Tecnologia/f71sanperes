...
if (isset($_REQUEST['gerar']) || isset($_REQUEST['todos_projetos'])) {
    ...
    if($tipo_contratacao == 2) {
        $str_qr_relatorio = "SELECT A.id_clt AS id, A.nome, B.nome AS atividade, B.salario AS valor, A.cpf, A.rg, A.locacao, IF(A.data_demi IS NOT NULL, date_format(A.data_demi, '%d/%m/%Y'), '00/00/0000') AS data_saida, date_format(A.data_entrada, '%d/%m/%Y') AS data_entrada
            FROM rh_clt AS A
