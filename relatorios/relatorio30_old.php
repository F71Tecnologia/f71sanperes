...
$result_clt = mysql_query("SELECT *, date_format(dada_pis, '%d/%m/%Y') as dada_pis FROM rh_clt WHERE tipo_contratacao = '2'  AND id_regiao = '$regiao' AND id_projeto = '$projeto' ORDER BY nome ASC");
...
<title>Relat&oacute;rio de Participantes do Projeto por PIS</title>
...
<table cellspacing="0" cellpadding="0" class="relacao" style="width:920px; border:0px; page-break-after:always;">
 <tr> 
    ...
    <td width="80%" align="center" colspan="2">
         <strong>RELAT&Oacute;RIO DE PARTICIPANTES DO PROJETO POR PIS</strong><br>
         <?=$row_master['razao']?>
      ...
      <tr class="secao">
		...
		<?php while ($row_autonomo = mysql_fetch_array($result_autonomo)) { ?>
			   <tr>
				...
				<td><?=$row_autonomo['matricula']?></td>
				<td><?=$row_autonomo['nome']?></td>
				<td><?php if(!empty($row_autonomo['pis'])) { echo $row_autonomo['pis']; } else { echo "NÃO TEM"; } ?></td>
				<td><?=$row_autonomo['dada_pis']?></td>
  ...
  <tr> 
    ...
    <tr class="secao">
	    ...
	    <tr class="<?php if($alternateColor++%2==0) { echo "linha_um"; } else { echo "linha_dois"; } ?>">
		   ...
		   <td><?=$row_clt['matricula']?></td>
		   <td><?=$row_clt['nome']?></td>
		   <td><?php if(!empty($row_clt['pis'])) { echo $row_clt['pis']; } else { echo "NÃO TEM"; } ?></td>
		   <td><?=$row_clt['dada_pis']?></td>
