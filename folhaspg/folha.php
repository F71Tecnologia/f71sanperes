...
$id     = (isset($_REQUEST['id'])) ? $_REQUEST['id'] : 9;
$regiao = (isset($_SESSION['id_regiao'])) ? $_SESSION['id_regiao'] : $usuario['id_regiao'];
                  ...
                  <table width="100%" cellspacing="0" cellpadding="8" style="border:solid 1px #DDD; background-color:#EEE; margin-top:10px;" class="relacao2">
                    <tr>
                      ...
                      <select name="id_projeto" id="id_projeto" class='campotexto'>
                        <?php $result_pro = mysql_query("SELECT * FROM projeto WHERE id_regiao = '$regiao' ");
                              $i = "0";
   ...
   <fieldset style="text-align:left; width:910px;">         
	  ...
	  <?php $cores = array('#E3ECE3','#ECF2EC','#ECF3F6','#F5F9FA','#FEF9EB','#FFFDF7');
		  ...
		  if($ultimo_ano != $row_folhas['ano']) { ?>
            ...
            <a href="#" class="titulo_ano" onClick="return false;"> <?php echo $row_folhas['ano'];?></a>
           ...
           <table width="100%" border="0" cellspacing="0"  style="width:100%; font-size:12px; line-height:32px; <?php echo $display;?>;" class="folhas"> 
			...
			if($row_folhas['status'] == 1) {
				...
				$bt2 = "<a href='folha.php?id=12&regiao=$regiao&folha=$row_folhas[0]' onClick='return confirm(\"Excluir esta folha?\")'>
            ...
            <tr bgcolor="<?php if($cor++%2==0) { echo "$cores[$cor_um]"; } else { echo "$cores[$cor_dois]"; } ?>" height="34">
			  ...
			  <?php 			   
					...
					$verifica_acoes  = mysql_num_rows($qr_acoes_assoc = mysql_query("SELECT * FROM funcionario_acoes_assoc WHERE id_funcionario = '$_COOKIE[logado]' AND acoes_id = '2' AND id_regiao = '$regiao'"));
					if($verifica_acoes != 0) {
					 echo $bt1;
					}	  
			  ...
			  <td  align="center"  width="70">
               <?php 
						...
						$verifica_acoes  = mysql_num_rows($qr_acoes_assoc = mysql_query("SELECT * FROM funcionario_acoes_assoc WHERE id_funcionario = '$_COOKIE[logado]' AND acoes_id = '3'  AND id_regiao = '$regiao'"));
						if($verifica_acoes != 0) {
						 echo $bt2;
						}					
						...
						if ($row_folhas['status'] == 3){
								...
								$verifica_acoes  = mysql_num_rows($qr_acoes_assoc = mysql_query("SELECT * FROM funcionario_acoes_assoc WHERE id_funcionario = '$_COOKIE[logado]' AND acoes_id = '7'  AND id_regiao = '$regiao'"));
								if($verifica_acoes != 0) {
								?>	
...
$qnt_dias1 = cal_days_in_month(CAL_GREGORIAN, $mes_pagamento, $ano_pagamento);
...
$RSverificaAUT = mysql_query("SELECT * FROM autonomo where
        status = '0' and YEAR(data_saida) = 2016 and id_projeto = 3331 and
...
//            print_r("Sinesio: " . $row_verificaAUT);
//            print_r("Sinesio 2: " . $row_verificaAUTProc);
//            exit();
//        }    
            
	$result_max = mysql_query("SELECT MAX(parte) FROM folhas where projeto = '$id_projeto' and mes = '$mes_pagamento' AND ano = '$ano_pagamento' and status = '3' AND terceiro != '1'
