...
function confirm_entry(a,b){
	...
	if (input_box==true){ 
		...
		location.href="folha_new.php?id=12&regiao=" + Regiao + "&folha=" + Folha;
		}else{
...
<table width="90%" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" align="center" style="border:1px solid #BBB;">
  ...
  <form action="folha_new.php" method="post" name="form1" onSubmit="return validaForm()" target="iframe1">
  <table width="90%" cellspacing="0" cellpadding="8" style="border:solid 1px #DDD; background-color:#EEE; margin-top:10px;">
       ...
       <td>
        <select name="mes" id="mes" class='campotexto' >
          <?php
			...
			$link = "folha_new.php?id=10&id_projeto=$id_projeto&regiao=$regiao"; ?>
    ...
    <tr>
      <td class="secao">D&eacute;cimo Terceiro:</td>
      <td>
      <label>
    ...
    <tr>
	  ...
	  <?php while($row_folhas = mysql_fetch_array($result_folhas)) {
			...
			if($row_folhas['status'] == 1) {
				$bt1 = "<a href='folha_new2.php?m=1&enc=$linkreg'>
				...
				$bt1 = "<a href='ver_folha_new.php?enc=$linkreg'>
					   	    <img src='../rh/folha/imagens/verfolha.gif' border='0' alt='VER'>
					    </a>";
				$bt2 = NULL;
			}
			...
			if($row_folhas['terceiro'] == 1) {
				...
				$mensagem = '13º '.$tipo_terceiro[$row_folhas['tipo_terceiro']];
			} else {
            ...
            <tr bgcolor="<?php if($cor++%2==0) { echo "$cores[$cor_um]"; } else { echo "$cores[$cor_dois]"; } ?>" height="34">
			  <td width="5%" align="center"><?=$bt1?></td>
			  ...
			  <td width="4%" align="center"><?=$bt2?></td>
            </tr>
    ...
    <tr>
	  ...
	  <?php $cores = array('#E3ECE3','#ECF2EC','#ECF3F6','#F5F9FA','#FEF9EB','#FFFDF7');
			...
			//while($row_pro = mysql_fetch_assoc($result_pro)) {
				...
				$result_folhas = mysql_query("SELECT *, date_format(data_inicio, '%d/%m/%Y') AS data_inicio,
													 	date_format(data_fim, '%d/%m/%Y') AS data_fim
													 	FROM folhas WHERE regiao = '$regiao' AND status != '0' 
													 	AND contratacao = '3' ORDER BY projeto, ano, mes ASC");
	  			$numero_folhas = mysql_num_rows($result_folhas);
				if(!empty($numero_folhas)) { ?>
  ...
  <table width="90%" align="center" style="border:solid 1px #777; margin-top:50px;">
    <tr>
      <td class="secao_pai">FOLHAS AUT&Ocirc;NOMOS / PJ</td>
    </tr>
    <tr>
      <td align="center">
	  ...
	  <?php $cores = array('#E3ECE3','#ECF2EC','#ECF3F6','#F5F9FA','#FEF9EB','#FFFDF7');
			...
			//while($row_pro = mysql_fetch_assoc($result_pro)) {
				...
				$result_folhas = mysql_query("SELECT * , date_format(data_inicio, '%d/%m/%Y')as data_inicio, 				 
														 date_format(data_fim, '%d/%m/%Y')as data_fim
														 FROM folhas WHERE regiao = '$regiao' AND status != '0' 
														 AND contratacao = '4' ORDER BY projeto, ano, mes ASC");
	  			$numero_folhas = mysql_num_rows($result_folhas);
				if(!empty($numero_folhas)) { ?>
...
<form action='folha_new.php' method='post' name='form1' target='_parent'>
...
if($contratacao == 3){
	$result = mysql_query("SELECT * FROM folhas where projeto = '$id_projeto' and mes = '$mes_pagamento' and contratacao = '$contratacao'
...
if($con_result >= 1){
	...
	location.href=\"folha_new.php?id=9&enc=$linkVolta\"
	</script>";
	exit;
}else{
...
if($contratacao == 1){
	...
	$result_max = mysql_query("SELECT MAX(parte) FROM folhas where projeto = '$id_projeto' and mes = '$mes_pagamento' and status = '3' 
	...
	print "<script>
	location.href=\"folha_new2.php?m=1&enc=$linkcontinue\"
	</script>";
