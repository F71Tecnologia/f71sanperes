...
if(!empty($_REQUEST['agencia'])){
	...
	$RE_clt = mysql_query("SELECT * FROM folha_autonomo WHERE id_autonomo = '$id' AND status = 3") or die (mysql_error());
	$RowCLT = mysql_fetch_array($RE_clt);
	
	mysql_query("UPDATE autonomo       SET nome='$nome', cpf='$cpf', agencia='$ag', conta='$cc', tipo_conta='$tipo_conta' WHERE id_autonomo = '$RowCLT[id_autonomo]' LIMIT 1") or die (mysql_error());
	mysql_query("UPDATE folha_autonomo SET nome='$nome', cpf='$cpf', agencia='$ag', conta='$cc'                           WHERE id_autonomo = '$id' LIMIT 1") or die (mysql_error());
      ...
      <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td width="18%" align="center" valign="middle" bgcolor="#CCCCCC" class="style23">Nome </td>
             <td width="8%" align="right" valign="middle" bgcolor="#CCCCCC" class="style23">CPF</td>
             <td width="8%" align="right" valign="middle" bgcolor="#CCCCCC" class="style23">Agência</td>
       ...
       <?php 
			...
			$rowTipoPg = mysql_fetch_array($tiposDePagamentos);
	   		...
	   		if ($row_banco['id_nacional'] == '237'){
				...
				$resultContas = mysql_query("SELECT * FROM folha_autonomo where id_folha = '$folha' and status = '3' and banco = '$banco_participante' and tipo_pg='$rowTipoPg[id_tipopg]' AND salario_liq != '0.00'");				
...
// FIM SANTANDER--------------------------------------------------------------------------------
				...
				//VERIFICA QUAIS OS TIPOS DE CONTAS QUE O FECHAMENTO POSSUI E PREENCHE UMA VARIÁVEL ESPECÍFICA COM O TIPO DE CONTRA ENCONTRADO
				$resultContas = mysql_query("SELECT * FROM folha_autonomo where id_folha = '$folha' and status = '3' and banco = '$banco_participante' and tipo_pg='$rowTipoPg[id_tipopg]' AND salario_liq != '0.00'") or die(mysql_error());				
				while ($rowContas = mysql_fetch_array($resultContas)){	
						$resultTiposDeConta = mysql_query("SELECT tipo_conta FROM autonomo WHERE id_autonomo = '$rowContas[id_autonomo]'");
		  				$rowTiposDeConta = mysql_fetch_array($resultTiposDeConta);
					if ($rowTiposDeConta['tipo_conta'] == 'corrente'){
				...
				//VERIFICA QUAIS OS TIPOS DE CONTAS QUE O FECHAMENTO POSSUI E PREENCHE UMA VARIÁVEL ESPECÍFICA COM O TIPO DE CONTRA ENCONTRADO				
				$resultContas = mysql_query("SELECT * FROM folha_autonomo where id_folha = '$folha' and status = '3' and banco = '$banco_participante' and tipo_pg='$rowTipoPg[id_tipopg]' AND salario_liq != '0.00'") or die(mysql_error());				
				//$resultContas = mysql_query("SELECT * FROM folha_autonomo where id_folha = '$folha' and status = '3' and banco = '$banco_participante'") or die(mysql_error());	
				while ($rowContas = mysql_fetch_array($resultContas)){
						$resultTiposDeConta = mysql_query("SELECT tipo_conta FROM autonomo WHERE id_autonomo = '$rowContas[id_autonomo]'");
		  				$rowTiposDeConta = mysql_fetch_array($resultTiposDeConta);
					if ($rowTiposDeConta['tipo_conta'] == 'corrente'){
       ...
       <?php
		  ...
		  $resultClt = mysql_query("SELECT * FROM folha_autonomo where (id_folha = '$folha' and status = '3' and banco = '$banco_participante' 
		  and tipo_pg='$rowTipoPg[id_tipopg]') ORDER BY nome");
		 ...
		 @$tipo_pg    = mysql_result($qr_tipo_pg,0);
		  ...
		  print"
		  <tr bgcolor=$color height='20' class='TextoDados'>
		  <td align='center' valign='middle' $bord> $rowAutonomo[campo3] </td>
          <td align='lefth' valign='middle' $bord>$alink $nomeT</a> $divTT</td>
		  <td align='right' valign='middle' $bord>$row_clt[cpf]</td>
		  <td align='right' valign='middle' $bord>$row_clt[agencia]</td>
		  <td align='right' valign='middle' $bord>$row_clt[conta]</td>
...
?>
    ...
    SELECIONAR FUNCION&Aacute;RIOS A SEREM PAGOS</a></td> 
    </tr>
    <tr>
    </table>
...
function Confirm(a,b){
    ...
    <tr>
    	<td align="center" valign="middle" bgcolor="#CCCCCC">
        <span class="igreja"> <strong>DOWNLOAD DO ARQUIVO TEXTO <span style="color:#F00">COMPLETO</span></strong></span></td>
    </tr>
    <tr>
    <td align='center'>&nbsp;</td>
    <tr>
