...
$result_master = mysql_query("SELECT * FROM master where id_master = '$row_user[id_master]'")or die(mysql_error());
...
$titulo = "Lista Banco: Projeto $row_projeto[nome] mês de $mes_da_folha";
...
if(!empty($_REQUEST['vai'])){
	...
	$maximo = count($clt);
	for($i=0; $i < $maximo; $i ++){
		mysql_query("UPDATE folha_cooperado SET arquivo = '1' WHERE id_folha = '".$folha."' and id_autonomo = '$clt[$i]' LIMIT 1");
		$Ids .= $clt[$i].",";
	}
		...
		$local = "GERANDO ARQUIVO TXT INDIVIDUAL COOP";
		$horario = date('Y-m-d H:i:s');
		$acao = "GERANDO ARQUIVO TXT INDIVIDUAL COOP ($folha- $Ids )";
		...
		print '
		<script>
		location.href= "folha_bancocoo_a.php?enc='.$linkvolt.'";
		</script>
		';
...
if(!empty($_REQUEST['sel'])){
	...
	print "<br><center>SELECIONE APENAS QUEM VAI SER PAGO!</center><br><br><form action='' method=\"post\" name=\"Form\" id=\"Form\">";
	
	$resultClt = mysql_query("SELECT * FROM folha_cooperado where (id_folha = '$folha' and status = '3' and banco = '$banco') ORDER BY nome");
	...
	while($row = mysql_fetch_array($resultClt)){
		...
		mysql_query("UPDATE folha_cooperado SET arquivo = '0' WHERE id_folha = '".$folha."' and id_autonomo = '$row[id_autonomo]' LIMIT 1");
      ...
      <tr>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td width="55%" bgcolor="#E2E2E2"><span class="texto10">
              <?=$Gnome?>
              <br>
CNPJ :
<?=$Gcnpj?>
       ...
       <?php 
			...
			$rowTipoPg = mysql_fetch_array($tiposDePagamentos);
	   		...
	   		if ($row_banco['id_nacional'] == '237'){
				...
				$resultContas = mysql_query("SELECT * FROM folha_cooperado where id_folha = '$folha' and status = '3' and banco = '$banco' and tipo_pg='$rowTipoPg[id_tipopg]'");				
				...
				//VERIFICA QUAIS OS TIPOS DE CONTAS QUE O FECHAMENTO POSSUI E PREENCHE UMA VARIÁVEL ESPECÍFICA COM O TIPO DE CONTRA ENCONTRADO
				$resultContas = mysql_query("SELECT * FROM folha_cooperado where id_folha = '$folha' and status = '3' and banco = '$banco' and tipo_pg='$rowTipoPg[id_tipopg]'") or die(mysql_error());				
				while ($rowContas = mysql_fetch_array($resultContas)){	
						$resultTiposDeConta = mysql_query("SELECT tipo_conta FROM autonomo WHERE id_autonomo = '$rowContas[id_autonomo]'");
		  				$rowTiposDeConta = mysql_fetch_array($resultTiposDeConta);
					if ($rowTiposDeConta['tipo_conta'] == 'corrente'){
...
?>
    ...
    &nbsp;&nbsp;&nbsp;
    <a href='ver_folhacoop.php?<?="enc=".$linkvolt."&tela=1"?>' style="text-decoration:none; color:#000">VOLTAR</a>
