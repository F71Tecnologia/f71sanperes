...
<table cellpadding="0" cellspacing="1" class="secao">
  ...
  <td>
  <?php $qr_curso = mysql_query("SELECT * FROM curso WHERE id_regiao = '$id_regiao' AND campo3 = '$projeto' AND tipo = '1' ORDER BY nome ASC");
...
<label><input type="radio" id="assinatura3" name="assinatura3" value="1" $selected_ass_sim2> Sim </label>
<label><input type="radio" id="assinatura3" name="assinatura3" value="0" $selected_ass_nao2> Não</label>
<p>&nbsp;</p>
...
(id_projeto, id_regiao, localpagamento, locacao, nome, sexo, 
 ...
 rh_vinculo, rh_status, rh_horario, rh_sindicato, rh_cbo, email)
    VALUES
('$id_projeto', '$regiao', '$localpagamento', '$locacao', '$nome', '$sexo', 
 ...
 '$rh_vinculo', '$rh_status', '$rh_horario', '$rh_sindicato', '$rh_cbo', '$email')") or die (mysql_error());
