<html>
            <head>
                ...
                <link href="relatorios/css/estrutura.css" rel="stylesheet" type="text/css" media="all">
                <style>
                    #content{width: 800px; margin: 0 auto; margin-top: 50px; }
...
if (empty($_COOKIE['logado'])) {
    ...
    $pro = 3304;//$_REQUEST['pro'];
    $id_reg = 45;//$_REQUEST['id_reg'];
    //$clt = $_REQUEST['clt'];
    ...
    //$WHERE = "(data_demi BETWEEN '".date('Y-m')."-01' AND LAST_DAY('".date('Y-m')."-01') OR data_saida BETWEEN '".date('Y-m')."-01' AND LAST_DAY('".date('Y-m')."-01'))";
    //$WHERE = " AND (data_demi >= '2015-10-03' OR data_saida >= '2015-10-03')";
    
    $result_bol = "SELECT A.id_clt, A.nome, A.endereco, A.numero, A.complemento, A.bairro, A.cidade, A.uf, A.nacionalidade, A.tel_fixo, A.tel_cel, A.tel_rec, A.foto, A.sexo, date_format(A.data_nasci, '%d/%m/%Y') AS data_nasci, A.naturalidade, A.civil,
                                ...
                                WHERE A.id_projeto = $pro $WHERE
                                ORDER BY A.nome LIMIT 205,205";
    echo "<!--$result_bol-->";
    $result_bol = mysql_query($result_bol);
    
    $quy_total_participantes = mysql_num_rows($result_bol);
    //echo "Total de Participantes encontrado: " . $quy_total_participantes;
    
    while($row = mysql_fetch_array($result_bol)){
        $data_entrada = implode('-', array_reverse(explode('/',$row['data_entrada'])));

        $result_reci = mysql_query("SELECT *,date_format(data_demi, '%d/%m/%Y') AS data_demibr FROM rh_recisao WHERE id_clt = {$row['id_clt']}  AND status = 1");
        ...
        $qr_depen = mysql_query("SELECT *, date_format(data1, '%d/%m/%Y') AS data1br, date_format(data1, '%d/%m/%Y') AS data1br, date_format(data2, '%d/%m/%Y') AS data2br, date_format(data3, '%d/%m/%Y') AS data3br, date_format(data4, '%d/%m/%Y') AS data4br, date_format(data5, '%d/%m/%Y') AS data5br, date_format(data6, '%d/%m/%Y') AS data6br FROM dependentes 
                                    WHERE id_bolsista = {{$row['id_clt']}} AND (nome1!='' or nome2!='' or nome3!= '' or nome4 != '' or nome5 != '' or nome6 != '')");
        ...
        $qr_exame = mysql_query("SELECT documento,date_format(data, '%d/%m/%Y') AS data FROM rh_doc_status AS A
                                    ...
                                    WHERE A.id_clt = {{$row['id_clt']}} AND A.tipo IN (1,13) ORDER BY data DESC");
        ...
        $qr_transf = mysql_query("SELECT A.id_transferencia, B.nome AS de, C.nome AS para, A.motivo, date_format(A.data_proc, '%d/%m/%Y') AS data FROM rh_transferencias AS A 
                                    ...
                                    WHERE id_clt = {{$row['id_clt']}} AND id_projeto_de <> id_projeto_para");
        ...
        $qr_alt_funcao = mysql_query("SELECT A.id_transferencia, B.nome AS de, C.nome AS para,D.cod AS cbo_de,E.cod AS cbo_para, A.motivo, date_format(A.data_proc, '%d/%m/%Y') AS data FROM rh_transferencias AS A 
                                        ...
                                        WHERE id_clt = {{$row['id_clt']}} AND B.nome <> C.nome");

        /* ALTERAÇÃO DE SALÁRIO */
        $qr_transf_sal = "SELECT id_curso_de, id_curso_para, data_proc FROM rh_transferencias WHERE id_curso_de <> id_curso_para AND id_clt = '{$row['id_clt']}' AND status = 1";
    ...
    //    print_r($qr_alt_salario); exit;
        ...
        $result_verifica = mysql_query("SELECT * FROM rh_doc_status WHERE tipo = '2' and id_clt = '{$row['id_clt']}'");
        $num_row_verifica = mysql_num_rows($result_verifica);
        if ($num_row_verifica == "0") {
            // mysql_query("INSERT INTO rh_doc_status(tipo,id_clt,data,id_user) VALUES ('2','{$row['id_clt']}','$data_cad', '$user_cad')");
        } else {
            //  mysql_query("UPDATE rh_doc_status SET data = '$data_cad', id_user = '$user_cad' WHERE id_clt = '{$row['id_clt']}' and tipo = '2'");
        }
        //-------------GRAVANDO NA TABELA DOCUMENTOS GERADOS
                ...
                <div id="content" style="page-break-after: always;">
                    ...
                    <div id="pessoal" class="box">
                        ...
                        <div id="foto">
                            ...
                            //echo "<img src='fotosclt/$nome_imagem' width='100' height='130' border=1 align=''>";
                            ?>
                        </div>
                        ...
                        <table border="0" cellspacing="0" cellpadding="0" width="100%" class="grid">
                            ...
                            <tbody>
                                ...
                                <?php if(!empty($p) && isset($p)){ ?>    
                                        <tr>
                                            <th colspan="3">PERÍODOS PERDIDOS </th>
                                        </tr>
