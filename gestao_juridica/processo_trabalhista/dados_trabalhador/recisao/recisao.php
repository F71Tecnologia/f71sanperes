...
$dias_trabalhados = $_REQUEST['diastrab'] - $faltas;
...
if($aviso == 'indenizado' and $t_ap == 2) {
	...
	$aviso_previo_valor_a = $salario_base; // valor acréscimo padrão, sem cálculos
...
// Décimo Terceiro (DT)
$qr_verifica_13_folha = mysql_query("SELECT a.id_clt FROM rh_folha_proc a INNER JOIN rh_folha b ON a.id_folha = b.id_folha WHERE a.id_clt = '$id_clt' AND b.terceiro = '1' AND a.ano = '$ano_demissao' AND a.status = '3'");
...
//if($id_clt != '4420' and $id_clt != '4049' and $id_clt != '4332' and $id_clt != '4335' and $id_clt != '4118') {
...
if($dispensa != '63' or $dispensa != '64' or $dispensa != '66') {
	if(($fator == 'empregador' and $aviso == 'trabalhado' and date('Y-m-d') > $data_aviso_previo_1)  or
	   ...
	   ($fator == 'empregado' and $aviso == 'indenizado' and date('Y-m-d') > $data_aviso_previo_10)) {
		$valor_atraso = $salario_base;// - $total_rendi;
		//$valor_atraso = 0;
	}
}
