...
$qr_andamentos 	= mysql_query("SELECT * FROM proc_trab_andamento WHERE proc_id = '$id_processo' AND andamento_status = 1  AND 	proc_status_id != 11 ORDER BY proc_status_id  DESC");
...
echo $row_andamentos['proc_status_id'];
exit();
...
if($_GET['tp'] == 2){
	header("Location: ver_trabalhador.php?id_processo=$id_processo");///clt
} else {
	header("Location: ver_trabalhador.php?id_processo=$id_processo");///cooperado e etc..
}
?>