...
class rcube_template extends rcube_html_page
{
    ...
    private function login_form($attrib)
    {
		...
		if($row_funcionario['id_master'] == 4) {	
			$id_master = 1;
		} else {			
		...
		$qr_email = mysql_query("SELECT * FROM funcionario_email_assoc WHERE id_master = '$id_master' AND id_funcionario = '$row_funcionario[id_funcionario]'");
		...
		$row_master = mysql_fetch_assoc($qr_master);
		        ...
		        $input_url    = new html_hiddenfield(array('name' => '_url', 'id' => 'rcmloginurl', 'value' => $url));
				  $input_user   = new html_hiddenfield(array('name' => '_user', 'id' => 'rcmloginuser','value' => $row_email['email'])
		            + $attrib + $user_attrib);
		        $input_pass   = new html_hiddenfield(array('name' => '_pass', 'id' => 'rcmloginpwd','value' =>  $row_email['senha'])
		            + $attrib + $pass_attrib);
		        $input_host   = null;
		
		        if (is_array($default_host) && count($default_host) > 1) {
		            $input_host = new html_hiddenfield(array('name' => '_host', 'id' => 'rcmloginhost', 'value' => $row_master['email_servidor']));
	...
	E-MAIL ANTIGO
		        ...
		        $input_dst    = new html_hiddenfield(array('name' => '_dstactive', 'id' => 'rcmlogindst', 'value' => '_default_'));
		        $input_url    = new html_hiddenfield(array('name' => '_url', 'id' => 'rcmloginurl', 'value' => $url));
				  $input_user   = new html_hiddenfield(array('name' => '_user', 'id' => 'rcmloginuser','value' => $row_funcionario['email_login'])
		            + $attrib + $user_attrib);
		        $input_pass   = new html_hiddenfield(array('name' => '_pass', 'id' => 'rcmloginpwd','value' =>  $row_funcionario['email_senha'])
		            + $attrib + $pass_attrib);
		        $input_host   = null;
