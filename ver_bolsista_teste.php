...
<table align="center" width="100%" cellspacing="0" cellpadding="12" style="font-size:13px; line-height:22px;">
  ...
  <?php // Consulta para Links
      ...
      <p><a href="rh/solicitapis_pdf.php?pro=<?=$id_pro?>&id_reg=<?=$id_reg?>&bol=<?=$row[0]?>" class="botao" target="_blank">Gerar PIS</a>
      	 <a href="cooperativas/devolucao_quotas.php?coop=<?=$row[0]?>&pro=<?=$id_pro?>&id_reg=<?=$id_reg?>" class="botao" target="_blank">Devolu&ccedil;&atilde;o de Quotas</a></p>
...
function Confirm(a){
  ...
  <table cellpadding="4" cellspacing="0" style="font-size:13px; border:0px; width:100%;">
    ...
    <tr>
        <td colspan="2">
        	<h1><span>&nbsp;RECIBO DE PAGAMENTO &Agrave; AUT&Ocirc;NOMO - R.P.A</span></h1>
        </td>
    </tr>
    <tr>
    	<td colspan="2">
        ...
        <form action="rh/rpa/actions/gerar.rpa.php" method="post" name="form_rpa" id="form_rpa" >
        	<table width="100%">
            	<tr>
                	<td>
                    	Data pagamento
                	...
                	<td>
                    	<input type="text" value="" name="data_pg" class="date">
                    </td>
                    <td>
                    	Valor
                    </td>
                    <td>
                    	<input type="text" value="0,00" name="valor" onKeyDown="FormataValor(this,event,17,2)" >
                  </td>
                    <td >
                    <input type="hidden" name="id" value="<?=$id_bol?>">
                    <input type="hidden" name="projeto" value="<?=$id_pro?>">
                  ...
                  <td colspan="5" align="center">
                  	<div>
                    	<table width="100%">
                            ...
                            <?php 
							$qr_rpa = mysql_query("SELECT * FROM rh_rpa WHERE id_autonomo = '$id_bol'");
							while($row_rpa = mysql_fetch_assoc($qr_rpa)):
							?>
                            <tr class="<? if($alternateColor++%2==0) { ?>linha_um<? } else { ?>linha_dois<? } ?>">
                            	
                                <td><?=implode('/',array_reverse(explode('-',$row_rpa['data'])));?></td>
                                <td>R$
                                <?=number_format($row_rpa['valor'],2,',','.')?></td>
                                <td>R$ <?=number_format($row_rpa['irrf'],2,',','.')?></td>
                                <td>R$ <?=number_format($row_rpa['inss'],2,',','.')?></td>
                                <td>R$ <?=number_format($row_rpa['valor']-($row_rpa['irrf']+$row_rpa['inss']),2,',','.')?></td>
                                <td><?php if(!empty($row_rpa['recibo'])):?>
                                    <a target="_blank" title="Comprovante" href="<?php echo "rh/rpa/arquivos/".$row_rpa['id_rpa'].".pdf"; ?>">
                                    	Ver
                                    </a>
                                    <?php else:?>
                                    <a target="_blank" title="Comprovante" href="rh/rpa/recibo.php?ID=<?=$row_rpa['id_rpa']?>">
                                    	Gerar comprovante
                                    </a>
                                    <?php endif;?>
                                </td>
                                <td>
                                	<?php 
									if(!empty($row_rpa['id_saida'])){
										$qr_saida 	= mysql_query("SELECT * FROM saida WHERE id_saida = '$row_rpa[id_saida]' AND status != '0'");
										$num_saida 	= mysql_num_rows($qr_saida);
										$row_saida 	= mysql_fetch_assoc($qr_saida);
										if($row_saida['status'] == '1'){
											echo "Saida não paga!";
										}else if($row_saida['status'] == '2'){
											echo "Saida Paga em ".implode('/',array_reverse(explode('-',$row_saida['data_pg'])));
										}
									}elseif(!empty($row_rpa['recibo'])){?>
                                    <a href="rh/rpa/Gerar.saida.php?ID=<?=$row_rpa['id_rpa']?>" 
                                    	onclick="return hs.htmlExpand(this, { objectType: 'iframe' } )" >Gerar saida</a>
									<?php }?>
                                </td>
