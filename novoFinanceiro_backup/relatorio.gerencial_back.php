...
if(empty($_COOKIE['logado2'])){
	echo "Acesso restrito, por favor faça login!";
	exit;
}
include "../conn.php";
...
<div id="base">
  ...
  ?>
	...
	SELECT SUM(ent.valor) FROM entradaesaida AS tip INNER JOIN entrada AS ent 
	...
	?>
      <tr class="<? if($alternateColor++%2==0) { ?>linha_um<? } else { ?>linha_dois<? } ?>">
        <td width="68%"><?=$row_tipo_entradas['id_entradasaida'].' - '.$row_tipo_entradas['nome'];?>
        	<a href="view/detalhes.relarorio.gerencial.php?grupo=5&mes=<?=$mes?>&ano=<?=$ano?>&projeto=<?=$projeto?>&id_entradasaida=<?=$row_tipo_entradas['id_entradasaida']?>&ordem=<?=$campo_ordem?>" onclick="return hs.htmlExpand(this, { contentId: 'highslide-html-ajax', wrapperClassName: 'highslide-white', outlineType: 'rounded-white', outlineWhileAnimating: true, objectType: 'ajax', preserveContent: true, width: 700 } )" class="detalhar"  title="VER DETALHES">
...
<table width="100%" border="0"  cellpadding="5" cellspacing="1">
    ...
    <?php 
		$sql = "SELECT SUM(sai.valor + sai.adicional) FROM entradaesaida AS tip INNER JOIN saida AS sai
		ON tip.id_entradasaida = sai.tipo
		WHERE tip.grupo = '1'
		AND MONTH(sai.$campo_ordem) = '$mes' 
		AND YEAR(sai.$campo_ordem) = '$ano'
		AND sai.id_projeto = '$projeto'
		AND tip.id_entradasaida = '$row_folha_tipo[id_entradasaida]'
		AND sai.status = '2'
		ORDER BY sai.$campo_ordem;";
	...
	?>
       <tr class="<? if($alternateColor++%2==0) { ?>linha_um_bloco1<? } else { ?>linha_dois_bloco1<? } ?>">
          <td width="70%"><?=$row_folha_tipo['id_entradasaida'].' - '.$row_folha_tipo['nome']?>
          		<a href="view/detalhes.relarorio.gerencial.php?grupo=1&mes=<?=$mes?>&ano=<?=$ano?>&projeto=<?=$projeto?>&id_entradasaida=<?=$row_folha_tipo['id_entradasaida']?>&ordem=<?=$campo_ordem?>" onclick="return hs.htmlExpand(this, { contentId: 'highslide-html-ajax', wrapperClassName: 'highslide-white', outlineType: 'rounded-white', outlineWhileAnimating: true, objectType: 'ajax', preserveContent: true, width: 700 } )" class="detalhar"  title="VER DETALHES">
          		<img src="image/seta.gif" width="9" height="9" border="0" />
                </a>
                </td>
 ...
 <?php $query_taxa_tipo = mysql_query("SELECT * FROM entradaesaida WHERE id_entradasaida = '65';");?>
	...
	?>
    	<tr class="<? if($alternateColor++%2==0) { ?>linha_um_bloco3<? } else { ?>linha_dois_bloco3<? } ?>">
          <td width="70%"><?=$row_taxa_tipo['id_entradasaida'].' - '.$row_taxa_tipo['nome']?>
          	<a href="view/detalhes.relarorio.gerencial.php?grupo=3&mes=<?=$mes?>&ano=<?=$ano?>&projeto=<?=$projeto?>&id_entradasaida=<?=$row_taxa_tipo['id_entradasaida']?>&ordem=<?=$campo_ordem?>" onclick="return hs.htmlExpand(this, { contentId: 'highslide-html-ajax', wrapperClassName: 'highslide-white', outlineType: 'rounded-white', outlineWhileAnimating: true, objectType: 'ajax', preserveContent: true, width: 700 } )" class="detalhar"  title="VER DETALHES">
          <img src="image/seta.gif" width="9" height="9" border="0"/>
          	</a>
          </td>
	...
	?>
       <tr class="<? if($alternateColor++%2==0) { ?>linha_um_bloco2<? } else { ?>linha_dois_bloco2<? } ?>">
            ...
            <a href="view/detalhes.relarorio.gerencial.php?grupo=2&mes=<?=$mes?>&ano=<?=$ano?>&projeto=<?=$projeto?>&id_entradasaida=<?=$row_reserva_tipo['id_entradasaida']?>&ordem=<?=$campo_ordem?>" onclick="return hs.htmlExpand(this, { contentId: 'highslide-html-ajax', wrapperClassName: 'highslide-white', outlineType: 'rounded-white', outlineWhileAnimating: true, objectType: 'ajax', preserveContent: true, width: 700 } )" class="detalhar"  title="VER DETALHES">
            <img src="image/seta.gif" width="9" height="9" border="0" />
            </a>
            </td>
