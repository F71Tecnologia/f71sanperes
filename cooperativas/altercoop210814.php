...
<table cellpadding="0" cellspacing="1" class="secao">
		  ...
		  if(!empty($verifica_curso)) {
			  ...
			  while($row_curso = mysql_fetch_array($result_curso)) {
				   if($Row['id_curso'] == $row_curso['0']) {
					   print "<option value='$row_curso[0]' selected>$row_curso[0] - $row_curso[nome]</option>";
				   } else {
  ...
  <tr>  
    ...
    <select name='locacao' id='locacao'>
        <?php while($row_unidade = mysql_fetch_assoc($result_unidade)) { ?> 
        <option value="<?php echo $row_unidade['unidade']."//".$row_unidade['id_unidade'];?>" <?php echo selected($row_unidade['id_unidade'], $Row['id_unidade']); ?>>
