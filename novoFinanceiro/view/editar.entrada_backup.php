...
$(function(){
	...
	$('#parceiros_notas').change(function (){
		...
		$.ajax({
			...
			url : '../../financeiro/actions/combo.entradas.php',
			data : { 'notas' : true, 'id_parceiro' :  id_parceiro, 'id_notas' : '<?php echo $row_notas['id_notas'];  ?>'},
			dataType : 'json',
              		...
              		<?php if(!empty($row_notas)):?>
						...
						<?php while($row_nota_selecionada = mysql_fetch_assoc($qr_nota_selecionada)):?>
                        	<tr class="linha_<?php if($alternateColor++%2==0) { echo 'um'; } else { echo 'dois'; } ?>">
                            	<td><input type="radio" <?=($row_notas['id_notas'] == $row_nota_selecionada['id_notas']) ? 'checked="checked"' : ''; ?> name="radio_nota" value="<?=$row_nota_selecionada['id_notas']?>"/></td>
