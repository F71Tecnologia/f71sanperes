...
if(!empty($id)):
	...
	if(empty($num_id)){
		...
		if(!empty($quanti)){
			...
			$row['comprovante1'] = '<a href="..view/comprovantes.php?'.$comprovante1.'" target="_blank">ver</a>';
		}else{
			$row['comprovante1'] = ' ';
		}
		if(!empty($quanti2)){
			$comprovante2 = $link_encryptado_pg;
			$row['comprovante2'] = '<a href="../view/comprovantes.php?'.$comprovante2.'" target="_blank">ver</a>';
		}else{
			$row['comprovante2'] = ' ';
		}		
		if($row['comprovante'] == '1'){
			$comprovante1 = $link_encryptado;
			$row['comprovante1'] = '<a href="../view/comprovantes.php?'.$link_encryptado.'" target="_blank">Ver</a>';
		}else{
			$row['comprovante1'] = ' ';
		}
...
while($row_saida = mysql_fetch_assoc($query_saida)){
	...
	if(!empty($quanti)){
		$comprovante1 = $link_encryptado;
		$row_saida['comprovante1'] = '<a href="../view/comprovantes.php?'.$comprovante1.'" target="_blank">ver</a>';
	}else{
	...
	if(!empty($quanti2)){
		$comprovante2 = $link_encryptado_pg;
		$row_saida['comprovante2'] = '<a href="../view/comprovantes.php?'.$comprovante2.'" target="_blank">ver</a>';
	}else{
	...
	/*if($row_saida['comprovante'] == '1'){
		$row_saida['comprovante1'] = '<a href="../view/comprovantes.php?'.$link_encryptado.'" target="_blank">Ver</a>';
	}else{
		$row_saida['comprovante1'] = ' ';
	}*/
...
while($row_id = mysql_fetch_assoc($query_id)){
	...
	if(!empty($quanti)){
		$comprovante1 = $link_encryptado;
		$row_id['comprovante1'] = '<a href="../view/comprovantes.php?'.$comprovante1.'" target="_blank">ver</a>';
	}else{
		$row_id['comprovante1'] = ' ';
	}
	if(!empty($quanti2)){
		$comprovante2 = $link_encryptado_pg;
		$row_id['comprovante2'] = '<a href="../view/comprovantes.php?'.$comprovante2.'" target="_blank">ver</a>';
	}else{
		$row_id['comprovante2'] = ' ';
	}	
	if($row_id['comprovante'] == '1'){
		$row_id['comprovante1'] = '<a href="../view/comprovantes.php?'.$link_encryptado.'" target="_blank">Ver</a>';
	}else{
		$row_id['comprovante1'] = ' ';
	}
	# FIM Buscando comprovantes e adicionando no json
