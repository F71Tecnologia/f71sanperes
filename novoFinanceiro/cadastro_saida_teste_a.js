$(function() {
    ...
    $('#Projeto-prestador').change(function() {
        ...
        ////CARREGANDO OS OUTROS PRESTADORES
        $.post('actions/combo.prestador_outros.json.php', {regiao: regiao_id, projeto: projeto_id}, function(retorno) {
            $('#prestador_outros').html('<option value="" selected="selected" >Selecione</option>');
            $.each(retorno, function(i, valor) {
                if (prestador_id == valor.id_prestador) {
                    selected = 'selected="selected"';
                } else {
                ...
                $('#prestador_outros').append('<option value="' + valor.id_prestador + '" ' + selected + ' >' + valor.id_prestador + ' - ' + valor.numero + ' - ' + valor.c_fantasia + ' - ' + valor.c_cnpj + '</option>');
                $("select[name=prestador_outros]").val($("#prestador_id").val());
            });
            
            $('#prestador_outros').focus();
        }, 'json' );
