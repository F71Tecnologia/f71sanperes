...
function ret_func($data, $id_curso) {
    global $id_regiao;
	...
	select * from
		...
		where 
			...
			and a.tipo_colaborador = 1 and t.id_curso = '{$id_curso}' and p.regiao_id = {$id_regiao}
		union
		...
		where 
			'{$data}' BETWEEN DATE_FORMAT(p.data_inicio,'%Y-%m-%d') and DATE_FORMAT(p.data_fim,'%Y-%m-%d')
			and a.tipo_colaborador = 2 and t.id_curso = '{$id_curso}' and p.regiao_id = {$id_regiao}
		union
		...
		where 
			'{$data}' BETWEEN DATE_FORMAT(p.data_inicio,'%Y-%m-%d') and DATE_FORMAT(p.data_fim,'%Y-%m-%d')
			and a.tipo_colaborador = 3 and t.id_curso = '{$id_curso}' and p.regiao_id = {$id_regiao}
		union
		select p.acesso_plantao_id as id_escala, 0 as id, '' as nome, 0 as id_curso, '' as curso, 0 as tipo, p.hora_entrada, p.hora_saida
		from 
			acesso_plantao p
		where 
			'{$data}' BETWEEN DATE_FORMAT(p.data_inicio,'%Y-%m-%d') and DATE_FORMAT(p.data_fim,'%Y-%m-%d') and p.regiao_id = {$id_regiao}
...
if($_REQUEST['acao'] == 'editar') {
    ...
    $sql = "
        ...
        where
            a.tipo_colaborador = 1 and p.acesso_plantao_id = {$id} and p.regiao_id = {$id_regiao}
    union
        select 
        ...
        where
            a.tipo_colaborador = 3 and p.acesso_plantao_id = {$id} and p.regiao_id = {$id_regiao}
    union
        select 
        ...
        where
            a.tipo_colaborador = 2 and p.acesso_plantao_id = {$id} and p.regiao_id = {$id_regiao}
    union
        select 
...
if (isset($_REQUEST['method']) && $_REQUEST['method'] == 'funcionarios') {
    if($_REQUEST['contratacao'] == 'clt') {
	$select_clt = "
	SELECT concat(t.id_clt, ';', '1') as id, concat(t.nome, ' - ', '(',lower(c.nome),')') as nome
	FROM rh_clt t inner join curso c on t.id_curso = c.id_curso
	WHERE id_projeto = '{$_REQUEST['pro']}' 
	ORDER BY nome";
	...
	$select_ter = "
	SELECT concat(t.id_terceirizado, ';', '3') as id, concat(t.nome, ' - ', '(',lower(c.nome),')') as nome
	FROM terceirizado t inner join curso c on t.id_curso = c.id_curso
	WHERE id_projeto = '{$_REQUEST['pro']}' 
	ORDER BY nome";
	...
	$select_aut = "
	SELECT concat(t.id_autonomo, ';', '2') as id, concat(t.nome, ' - ', '(',lower(c.nome),')') as nome
	FROM autonomo t inner join curso c on t.id_curso = c.id_curso
	WHERE id_projeto = '{$_REQUEST['pro']}' 
	ORDER BY nome";	
    ...
    <body>
        ...
        <div class="container">
                            ...
                            <div class="form-group">
                                ...
                                select * from
                                    ...
                                    from 
                                        ...
                                        rh_clt t on t.id_clt = a.colaborador_id inner join 
                                        curso c on c.id_curso = t.id_curso
                                    where a.tipo_colaborador = 1 and p.regiao_id in ({$regiao_in})
                                    union
                                    select c.id_curso, c.nome as curso
                                    from 
                                        acesso_plantao p inner join 
                                        ...
                                        autonomo t on t.id_autonomo = a.colaborador_id inner join 
                                        curso c on c.id_curso = t.id_curso
                                    where a.tipo_colaborador = 2 and p.regiao_id in ({$regiao_in})
                                    union
                                    select c.id_curso, c.nome as curso
                                    from 
                                        acesso_plantao p inner join 
                                        ...
                                        terceirizado t on t.id_terceirizado = a.colaborador_id inner join 
                                        curso c on c.id_curso = t.id_curso
                                    where a.tipo_colaborador = 3 and p.regiao_id in ({$regiao_in})) tab
            ...
            <div class="row">
                <div class="col-md-2 text-left">
                    <a class="btn btn-info" href="acesso_escala2.php?data_uso=<?=$data_uso?>&inc=-1&curso=<?=$id_curso?>"><i class="fa fa-angle-double-left"></i> Mês Anterior</a>
                </div>
                <div class="col-md-8 text-center">
                    <label class="control-label"><?=$data_uso?></label>
                </div>
                <div class="col-md-2 text-right">
                    <a class="btn btn-info" href="acesso_escala2.php?data_uso=<?=$data_uso?>&inc=1&curso=<?=$id_curso?>">Próximo Mês <i class="fa fa-angle-double-right"></i></a>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
	    ...
	    */
		...
		if(acao == 'editar') {
		    ...
		    $('#id').val(id);
		    $.ajax({
			url: "acesso_escala2.php",
			data: '&acao='+acao+'&id='+id+'&data='+data+'&curso='+id_curso,
			dataType: "json",
			type: "POST",
		        beforeSend: function() {
			    showWaiting(1);
