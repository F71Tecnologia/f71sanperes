...
$qr_subprojeto=mysql_query("SELECT * FROM subprojeto WHERE id_subprojeto='$projeto'");
$row_sub=mysql_fetch_assoc($qr_subprojeto);
$inicio_sub= implode('/',array_reverse(explode('-',$row_sub['inicio'])));
$termino_sub= implode('/',array_reverse(explode('-',$row_sub['termino'])));
?>
<html>
<head>
...
<table align="center" width="100%" cellspacing="0" cellpadding="12" style="font-size:13px; line-height:22px;">
  ...
  <div style="border-bottom:2px solid #F3F3F3; margin-top:10px;">
       <h2 style="float:left; font-size:18px;">PROJETOS CADASTRADOS</h2>
...
<?php $qr_projetos = mysql_query("SELECT * FROM projeto WHERE id_regiao = '$regiao' AND status_reg = '1'");
...
<?php $qr_projetos = mysql_query("SELECT *, date_format(inicio, '%d/%m/%Y') AS data_ini, date_format(termino, '%d/%m/%Y') AS data_fim FROM projeto WHERE id_projeto = '$row_sub[id_projeto]' AND status_reg IN ('1','0')");
	  ...
	  $qr_clt_ativos = mysql_query("SELECT * FROM rh_clt WHERE (status < '60' OR status = '200') AND id_regiao ='$regiao' AND tipo_contratacao = '2' AND id_projeto = '$row_sub[id_projeto]'");
	  $num_clt_ativos = mysql_num_rows($qr_clt_ativos);
	  $qr_cooperado_ativos = mysql_query("SELECT * FROM autonomo WHERE status = '1' AND tipo_contratacao = '3' AND id_regiao ='$regiao' AND id_projeto = '$row_sub[id_projeto]'");
	  $num_cooperado_ativos = mysql_num_rows($qr_cooperado_ativos);
	  $qr_autonomo_ativos = mysql_query("SELECT * FROM autonomo WHERE status = '1' AND tipo_contratacao = '1' AND id_regiao ='$regiao' AND id_projeto = '$row_sub[id_projeto]'");
	  $num_autonomo_ativos = mysql_num_rows($qr_autonomo_ativos);
	  $qr_autonomo_pj_ativos = mysql_query("SELECT * FROM autonomo WHERE status = '1' AND tipo_contratacao = '4' AND id_regiao ='$regiao' AND id_projeto = '$row_sub[id_projeto]'");
	  $num_autonomo_pj_ativos = mysql_num_rows($qr_autonomo_pj_ativos);
	  
	  // Participantes Inativos
	  $qr_clt_inativos = mysql_query("SELECT * FROM rh_clt WHERE (status >= '60' AND status != '200') AND tipo_contratacao = '2' AND id_regiao = '$regiao' AND id_projeto = '$row_sub[id_projeto]'");
	  $num_clt_inativos = mysql_num_rows($qr_clt_inativos);
	  $qr_cooperado_inativos = mysql_query("SELECT * FROM autonomo WHERE status != '1' AND tipo_contratacao = '3' AND id_regiao = '$regiao' AND id_projeto = '$row_sub[id_projeto]'");
	  $num_cooperado_inativos = mysql_num_rows($qr_cooperado_inativos);
	  $qr_autonomo_inativos = mysql_query("SELECT * FROM autonomo WHERE status != '1' AND tipo_contratacao = '1' AND id_regiao = '$regiao' AND id_projeto = '$row_sub[id_projeto]'");
	  $num_autonomo_inativos = mysql_num_rows($qr_autonomo_inativos);
	  $qr_autonomo_pj_inativos = mysql_query("SELECT * FROM autonomo WHERE status != '1' AND tipo_contratacao ='4' AND id_regiao = '$regiao' AND id_projeto = '$row_sub[id_projeto]'");
...
<table align="center" width="100%" cellspacing="0" cellpadding="12" style="font-size:13px; line-height:22px;">
  ...
  <td>
  <div style="border-bottom:2px solid #F3F3F3; margin-top:10px;">
       <h2 style="float:left; font-size:18px;"><?=$row['id_projeto']?> - <?=$row['nome']?>- RENOVA&Ccedil;&Atilde;O</h2>
  ...
  <tr>
    <td bgcolor="#F3F3F3" style="padding-left:20px;">
        ...
        <b>Descri&ccedil;&atilde;o:</b> <?=$row_sub['descricao']?><br>
        <b>Data de In&iacute;cio:</b> <?=$inicio_sub;?><br>
        <b>Previs&aacute;o de T&eacute;rmino:</b> <?=$termino_sub;?><br>
        <b>Tipo de renovação :</b><?=$row_sub['tipo_contrato']?><br>
    </td>
  </tr>
  <?php if($funcionario['grupo_usuario'] <= 3 or $funcionario['grupo_usuario'] == 5) { ?> 
  <tr>
    <td><h1><span>EDI&Ccedil;&Atilde;O DE PARTICIPANTES</span></h1></td>
  </tr>
  <tr>
    <td class="menu">
    <?php if($funcionario['grupo_usuario'] <= 2 or $funcionario['grupo_usuario'] == 5) { ?> 
    ...
    <p><a href="cadastro_bolsista.php?regiao=<?=$regiao?>&pro=<?=$projeto?>" class="botao">Cadastrar <span class="aut">Aut&oacute;nomo</span></a>
	...
	<?php } elseif($funcionario['grupo_usuario'] <= 3) { ?> 
 ...
 <tr>
  ...
  <?php } ?>
  <tr>
    <td><h1 style="margin-top:0px;"><span>ESTAT&Iacute;STICA DOS PARTICIPANTES DO PROJETO</span></h1></td>
  </tr>
  <tr>
    <td>
        ...
        <?php /* <div class="right" style="width:50%;">
           <?php if(!empty($total)) { ?>
             <div class="left">
	              <iframe src="pizza.php?valores=<?=$total_ativos?>-<?=$total_inativos?>" height="100" width="130" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe>
             </div>
  ...
  <tr>
    <td><h1 style="margin-top:0px;"><span>RELAT&Oacute;RIOS DO PROJETO</span></h1></td>
  </tr>
  <tr>
    <td>
