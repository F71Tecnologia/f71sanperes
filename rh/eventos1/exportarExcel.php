...
//    list($regiao, $evento) = explode('&', decrypt(str_replace('--', '+', $_REQUEST['enc'])));
    ...
    // LISTANDO FUNCIONARIOS  
            $participantes2 = mysql_query("SELECT b.nome AS projeto, d.especifica AS nome_status, a.nome, a.cpf, c.nome AS funcao
                                    FROM rh_clt a
                                    LEFT JOIN projeto b ON a.id_projeto = b.id_projeto
                                    LEFT JOIN curso c ON a.id_curso = c.id_curso
                                    LEFT JOIN rhstatus d ON a.status = d.codigo
                                    WHERE a.id_regiao = '$regiao' AND b.status_reg = '1' AND a.status NOT IN(40,90,60,101,81,62,61,64,66,65,63,10)
                                    ORDER BY b.id_projeto, d.especifica, a.nome"); ?>
                ...
                <p style="text-align: left; margin-top: 20px"><button type="button" value="Exportar" class="btn btn-success" id="exportarExcel"><span class="fa fa-file-excel-o"></span> Exportar para Excel</button></p>
                        ...
                        <div id="tbRelatoriob">
                            <table id="tbRelatorio" style="border-collapse: collapse; width:100%; page-break-after:auto; border:0;" class="grid"> 
                                ...
                                <tbody>
                                    <?php
                                    while ($row = mysql_fetch_assoc($participantes2)) { 
                                        $class = ($cont++ % 2 == 0) ? "even" : "odd";
                                        ?>
                                        <tr class="<?php echo $class ?>">
                                            <td style="text-align:center;"><?= $row['cpf']  ?></td>
                                            <td style="text-align:left;"><?= $row['nome']  ?>
                                            </td>
                                            <td style="text-align:left;"><?= $row['funcao']  ?></td>
                                            <td style="text-align:center;"><?= $row['nome_status']  ?></td>
                                            <td style="text-align:center;"><?= $row['projeto']  ?></td>
...
// excluir evento
    if (isset($_POST['set_status']) && ($_POST['set_status'] == 'zerar')) {
         ...
         * FEITO EM: 14/03/2016
         * ATUALIZANDO A DATA DE ULTIMO PROCESSAMENTO O CLT 
         */
//        $rh->Clt->setDefault()->setIdClt($id_clt)->onUpdate();
        ...
        if ($retorno) {
            ...
            $log->gravaLog('Eventos', "Evento ID{$id_evento} excluido do funcionário ID{$id_clt}");
...
// Ação do Formulário
    if (isset($_POST['pronto'])) {
         ...
         * ATUALIZANDO A DATA DE ULTIMO PROCESSAMENTO O CLT 
         */
//        $rh->Clt->setDefault()->setIdClt($_POST['id_clt'])->onUpdate();
        onUpdate($_POST['id_clt']);
...
//                    AND status = 1;";
////        echo $query_teste;
//        $return = mysql_query($query_teste);
...
//        } else {
            ...
            if ($result['status']) {
                $sucesso = true;
                $log->gravaLog('Eventos', "Evento ID{$ultimo_evento} cadastrado para o funcionário ID{$_POST['id_clt']}");
            }else{
                $resp = array(
                    'class' => 'back-red',
                    'msg' => $result['msg']
...
//        }
    ...
    $qr_status = mysql_query("SELECT * FROM rhstatus WHERE status_reg = '1' AND codigo NOT IN (10,90,40,200) /*AND tipo = 'licenca'*/ AND tipo IS NULL ORDER BY tipo,especifica ASC");
...
//        $data_calculo = ($pericia['pericia'] == 1 && $dados_evento['data_retorno_final'] == NULL) ? $dados_evento['data_retorno'] : $dados_evento['data_retorno_final']; //para saber qual data usar
        ...
        if ($dados_evento['dias'] != $dif && $dados_evento['dias'] != 0 && $dados_evento['dados_retorno'] != '0000-00-00') {
            ...
            * ATUALIZANDO A DATA DE ULTIMO PROCESSAMENTO O CLT 
            */
//            $rh->Clt->setDefault()->setIdClt($clt)->onUpdate();
            onUpdate($clt);
