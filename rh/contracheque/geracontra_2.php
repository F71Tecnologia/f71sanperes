...
$array_outros_movimentos = array(9998, 5060,5060,9999,5913,5912, 9997 );
...
for($i=0; $i<$qnt; $i++) {
		...
		//
		//var_dump(in_array($ARcodigosVER[$i], $array_outros_movimentos));
		
		
	
		echo $ARcodigosVER[$i].'<br>';
		if(in_array($ARcodigosVER[$i], $array_outros_movimentos)){	
			
			$qr_movimento_clt = mysql_query("SELECT * FROM rh_movimentos_clt WHERE id_clt = '$RowFolhaPro[id_clt]' AND id_folha = '$id_folha' AND cod_movimento = '$ARcodigosVER[$i]'") or die(mysql_error());
			$row_mov = mysql_fetch_assoc($qr_movimento_clt);
			$Valor_F = number_format($row_mov['valor_movimento'],2,",",".");
			echo $Valor_F.'<br>';
