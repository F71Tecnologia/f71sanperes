...
while ($RowFolhaPro = mysql_fetch_array($REfolhaproc)) {
    ...
    $qr_clt = mysql_query("SELECT *,DATE_FORMAT(data_entrada, '%d/%m/%Y')as databr FROM rh_clt WHERE id_clt = '$RowFolhaPro[id_clt]'");
    ...
    //salário base
    //verifica_falta
    $sql_movimento_clt = "SELECT * FROM rh_movimentos_clt WHERE  id_movimento IN($RowFolha[ids_movimentos_estatisticas]) AND id_clt = '$RowFolhaPro[id_clt]' AND id_mov  IN(62)";
...
//    var_dump($row_falta);
    ...
    $pdf->Rect(1, 15.9, 13, 0.8);
    $pdf->Line(4, 0.8 + $altura_ag, 4, $altura_ag + 1.6);
    $pdf->Line(8.5 , 0.8 + $altura_ag, 8.5, $altura_ag + 1.6);
    $pdf->Line(11.3 , 0.8 + $altura_ag, 11.3, $altura_ag + 1.6);
    $pdf->Text(1.2, 1.1 + $altura_ag, 'Salário Base:');
    $pdf->Text(1.5, 1.5 + $altura_ag, "R$ ".number_format($RowFolhaPro['salbase'], 2, ',', '.'));
    $pdf->Text(4.3, 1.1 + $altura_ag, 'Salário:');
    $pdf->Text(4.6, 1.5 + $altura_ag, "R$ ".number_format($salario_base, 2, ',', '.'));
    $pdf->Text(8.7, 1.1 + $altura_ag, 'FGTS:');
    $pdf->Text(9.2, 1.5 + $altura_ag, "R$ ".number_format($fgts, 2, ',', '.'));
    $pdf->Text(11.5, 1.1 + $altura_ag, 'Base Calculo IRRF:');
    $pdf->Text(11.8, 1.5 + $altura_ag, "R$ ".number_format($ir, 2, ",", "."));

    $pdf->Rect(16, 15.9, 13, 0.8);
    $pdf->Line(8.5 + $largura_celula + $distancia, 0.8 + $altura_ag, 8.5 + $largura_celula + $distancia, $altura_ag + 1.6);
    $pdf->Line(11.3 + $largura_celula + $distancia, 0.8 + $altura_ag, 11.3 + $largura_celula + $distancia, $altura_ag + 1.6);
    $pdf->Text(1.2 + $largura_celula + $distancia, 1.1 + $altura_ag, 'Salário Base:');
    $pdf->Text(1.5 + $largura_celula + $distancia, 1.5 + $altura_ag, "R$ ".number_format($RowFolhaPro['salbase'], 2, ',', '.'));
    $pdf->Text(4.3 + $largura_celula + $distancia, 1.1 + $altura_ag, 'Salário:');
    $pdf->Text(4.6 + $largura_celula + $distancia, 1.5 + $altura_ag, "R$ ".number_format($salario_base, 2, ',', '.'));
    $pdf->Text(8.7 + $largura_celula + $distancia, 1.1 + $altura_ag, 'FGTS:');
    $pdf->Text(9.2 + $largura_celula + $distancia, 1.5 + $altura_ag, "R$ ".number_format($fgts, 2, ',', '.'));
    $pdf->Text(11.5 + $largura_celula + $distancia, 1.1 + $altura_ag, 'Base Calculo IRRF:');
    $pdf->Text(11.8 + $largura_celula + $distancia, 1.5 + $altura_ag, "R$ ".number_format($ir, 2, ",", "."));
    ...
    $y = 18.7;
    $pdf->Text(1.3, 17.8, 'Declaro ter recebido a importância líquida discriminada acima');
    ...
    $pdf->Text(16.2, 17.8, 'Declaro ter recebido a importância líquida discriminada acima');
