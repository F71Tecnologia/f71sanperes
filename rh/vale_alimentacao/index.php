...
// teste leonardo 27/02/2015
// funcao que retorna proximo mes e ano
function vt_data(){
    $data = date('Y-m',  mktime(0, 0, 0, date('m')+1, 1, date('Y')));
    $data = explode('-', $data);
    return array('ano'=>$data[0],'mes'=>$data[1]);
}
...
function getRegioesFuncionario($usuario) {
...
function getFuncionariosByProjeto(Array $dados) {
	...
	WHERE A.id_projeto = '$projeto' $cpf $nome $mes $ano AND (A.status = '10' OR A.status = '40') AND (A.status_demi=0 OR A.status_demi IS NULL)";
...
function relacao_clt_pedido(Array $dados, $gravar=FALSE) {
    ...
    $sql = "SELECT A.id_clt, A.nome AS nome_funcionario, B.nome AS nome_projeto, D.valor_diario FROM rh_clt AS A LEFT JOIN projeto AS B ON(A.id_projeto=B.id_projeto) 
            ...
            WHERE A.id_projeto=$projeto AND A.vale_alimentacao=1  AND (A.status = '10' OR A.status= '40') AND (A.status_demi=0 OR A.status_demi IS NULL) GROUP BY A.id_clt";
//    echo $sql."<br>";
    $result = mysql_query($sql);
    $relacao_funcionarios = array();
...
function get_relacao_clt_pedido($id_projeto){
    $sql = "SELECT D.id_clt, A.id_va_pedido, A.mes, A.ano, B.nome AS projeto, C.nome AS usuario, E.nome AS nome_funcionario, "
            ...
            . "WHERE A.`status`='1' AND A.id_va_pedido='$id_projeto'  AND E.status = '10'";
    //echo $sql."<br>";
    $resp = mysql_query($sql);
...
function criar_csv_aelo($relacao_funcionarios){
    ...
    foreach($relacao_funcionarios as $funcionario){
        $body .= "%;".RemoveAcentos(substr($funcionario[nome_funcionario], 0, 40)).";"
    ...
    fclose($file);
    $dados = utf8_encode($header.$body);
    return json_encode(array('name_file'=>$name,'dados'=>$dados));
}
...
$data = vt_data(); // pega mes seguinte e ano

$ano = isset($_POST['ano']) ? $_POST['ano'] : $data['ano'];
//$ano = isset($_POST['ano']) ? $_POST['ano'] : date('Y');
//$ano = isset($_POST['ano']) ? $_POST['ano'] : date('Y');
$mes = str_pad(isset($_POST['mes']) ? $_POST['mes'] : $data['mes'], 2, "0", STR_PAD_LEFT);
...
//            include_once 'includes/table_aelo.php';
        ...
        case 'form2' :
            ...
            $relacao_funcionarios = getFuncionariosByProjeto($dados);
