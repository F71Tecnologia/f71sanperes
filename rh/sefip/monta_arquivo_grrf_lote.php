...
$idCltArray = isset($_REQUEST['ativo']) ? $_REQUEST['ativo'] : NULL;
...
$sql = mysql_query("SELECT A.id_clt, A.pis, B.id_recisao
    FROM rh_clt AS A
    LEFT JOIN rh_recisao AS B ON(A.id_clt = B.id_clt)
    WHERE A.id_clt IN(".implode(',',$idCltArray).") AND B.status = 1 AND B.rescisao_complementar = 0 AND B.motivo NOT IN(60,63,65)
    ORDER BY A.pis ASC");
while($row = mysql_fetch_assoc($sql)){
//    echo $row[pis].'<br>';
    $row[pis] = str_replace(array('.','-'), '', $row[pis]);
    $idCltArrayOrdenado[$row[id_recisao]] = $row[id_clt];
}
...
//}
$contador = 0;
foreach ($idCltArrayOrdenado as $idResc => $idClt) {
    
//    print_r($idCltArrayOrdenado);exit;
    $contador++;
    $daoClt = new DaoClt();
    $valorBaseInformado = $_REQUEST['valor'][$idClt];
    $clt = new Clt();
    ...
    $row_clt = $daoClt->buscaClt($clt, $idResc);
...
//    print_helper($row_rescisao);
    ...
    $res_fpas_empresa = $dao->getFPAS($row_empresa['fpas']);
    $row_empresa['fpas'] = $res_fpas_empresa['codigo'];
