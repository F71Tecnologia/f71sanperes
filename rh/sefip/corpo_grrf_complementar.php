...
// inicio da linha 1
    ...
    $obj->dados($row_empresa['cnpj_matriz_vasco']); // INSCRIÃ‡ÃƒO DO RESPONSÃ?VEL (1 - CNPJ)
...
// inicio de linha 4
    ...
    $obj->dados($obj->completa($dados['trabalhador']['avos_dt'], 2, '0', 'antes')); // 13 salário 12 avos
    $obj->dados($obj->completa($dados['trabalhador']['dt_salario'], 15, '0', 'antes')); // 13 salário 
    $obj->dados($obj->completa('', 2, '0', 'antes')); // 13 salário indenizado 12 avos
    $obj->dados($obj->completa('', 15, '0', 'antes')); // 13 salário indenizado
    $obj->dados($obj->completa($dados['trabalhador']['ferias_vencidas'], 15, '0', 'antes')); // Férias Vencidas
    $obj->dados($obj->completa($dados['trabalhador']['avos_fp'], 2, '0', 'antes')); // Férias Proporcionais
    $obj->dados($obj->completa($dados['trabalhador']['ferias_pr'], 15, '0', 'antes')); // Férias Proporcionais
    $obj->dados($obj->completa($dados['trabalhador']['umterco_fp'], 15, '0', 'antes')); // 1/3 Férias 
    $obj->dados($obj->completa('', 2, '0', 'antes')); // Salário Família Dias
    $obj->dados($obj->completa($dados['trabalhador']['sal_familia'], 15, '0', 'antes')); // Salário Família Valor
    $obj->dados($obj->completa($arrayMovimentos[66]['valor'], 15, '0', 'antes')); // Adicional Noturno
    $obj->dados($obj->completa('', 15, '0', 'antes')); // Comissões
    $obj->dados($obj->completa($arrayMovimentos[192]['valor'], 15, '0', 'antes')); // Gratificação
    ...
    $obj->dados($obj->completa($dados['trabalhador']['inss_ss'], 15, '0', 'antes')); // Dedução Previdência
    $obj->dados($obj->completa($dados['trabalhador']['inss_dt'], 15, '0', 'antes')); // Dedução 13º salário
    $obj->dados($obj->completa('', 15, '0', 'antes')); // Dedução adiantamentos
    $obj->dados($obj->completa($dados['trabalhador']['ir'], 15, '0', 'antes')); // Dedução imposto de renda
...
//    echo $sql_grrf.'<br>';
    ...
    $log->gravaLog('Relatório e Impostos', "GRRF Gerado: ID{$grrf_id}");
