...
//                    for ($ano = 2009; $ano <= date('Y'); $ano++) {
                                            ...
                                            <td>
                                                <div class="folha_mes" title="<span style='color:#c30;'><?= $nome_mes ?></span><br><span style='line-height:normal;'>
                                                     ...
                                                     while ($folha = mysql_fetch_assoc($qr_folha)) {
                                                         ...
                                                         $dados_folha = "{$nome_projeto} - {$nome_regiao} ({$total_participantes})";
                                                         if (!empty($verifica_sefip)) {
                                                                
                                                             $title        = 'Visualizar SEFIP';
                                                             $link_arquivo = "arquivos/{$regiao}_{$projeto}_{$mes}_{$ano}{$parte_folha}.re";
                                                             $link_excluir = "<a href='index.php?excluir=true&regiao={$regiao}&projeto={$projeto}&mes={$mes}&ano={$ano}&folha={$folha['id_folha']}' 
                                                                               title='Excluir SEFIP' style='color:#C30'>excluir</a><br>";
                                                             
                                                                echo "<a href='{$link_arquivo}' target='_blank' title='{$title}'>{$dados_folha}</a>";
                                                                echo $link_excluir;
                                                                
                                                                } else {
                                                                    
                                                                    $title          = 'Gerar SEFIP';
                                                                    $link_arquivo   = "sefiptexto2.php?mes={$mes}&ano={$ano}&regiao={$regiao}&projeto={$projeto}&folha={$folha['id_folha']}";
                                                                    $link_excluir   = '';
                                                             echo "<a href='{$link_arquivo}' onClick='return false' class='dataSefip' title='{$title}'>{$dados_folha}</a>";
            ...
            <tr>
                <td>
                    ...
                    <?php for ($ano = date('Y'); $ano >= '2009'; $ano--): ?>
                        ...
                        <div class="meses_recisao" style="display:none;">
                            <?php foreach ($meses as $nome_mes => $num_mes): ?>
                                ...
                                $sql_rescisao = "
                                                    ...
                                                    INNER JOIN rhstatus ON rhstatus.id_status=rh_recisao.fgts_cod
