...
$obj->dados($obj->completa($obj->limpar($row_empresa['responsavel']),20)); // NOME RESPONSAVEL 
...
$obj->dados($obj->completa($row_empresa['razao'],40)); // NOME EMPRESA / RAZÃƒO
$obj->dados($obj->completa('RUA JOAO CAETANO 359',50)); // RUA , NÂº
$obj->dados($obj->completa('CENTRO',20)); // BAIRRO
$obj->dados($obj->completa($obj->limpar($row_empresa['cep']),8)); // CEP
$obj->dados($obj->completa($obj->limpar('ITABORAI'),20)); // CIDADE
$obj->dados($obj->completa('RJ',2)); // UNIDADE DA FEDERAÃ‡ÃƒO 
$obj->dados($obj->completa($obj->limpar($row_empresa['tel']),12,'0','antes'));// TELEFONE
$obj->dados($obj->completa($row_empresa['cnae'],7)); // NOME EMPRESA / RAZÃƒO
$obj->dados('1'); // SIMPLES, NÃƒO OPTANTE
$obj->dados($obj->completa($row_empresa['fpas'],3)); // SIMPLES, NÃƒO OPTANTE
$obj->filler(143); // BRANCOS
$obj->fechalinha('*');// FECHA LINHA

$qr_recisao = mysql_query("SELECT * FROM rh_recisao WHERE MONTH(data_demi) = '$row_folha[mes]' AND YEAR(data_demi) = '$row_folha[ano]' AND id_regiao = '$row_folha[regiao]' AND id_projeto = '$row_folha[projeto]' AND status != '0'");
...
while($row_recisao  = mysql_fetch_assoc($qr_recisao)):
	...
	$matrizDeControle[] = $row_clt['id_clt'];
	...
	$obj->dados($obj->completa($row_clt['nome'],70)); // Nome do trabalhador
	$obj->dados($obj->completa($obj->limpar($row_clt['campo1']),7)); // CTPS
	$obj->dados($obj->completa($obj->limpar($row_clt['serie_ctps']),5)); // SERIE CTPS
	...
	$obj->dados($obj->completa($horasPorSemanas,2,'0','antes')); // quantidade de horas trabalhadas por semana
	...
	//
	$obj->dados($obj->completa($obj->limpar($cbo),6,'0','antes')); // CODIGO CBO
