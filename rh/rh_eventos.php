...
<html>
    ...
    <body>
        <div id="corpo">
            ...
            <table cellspacing="4" cellpadding="0" id="topo">
                ...
                <tr>
                    <td width="15%" valign="middle" style="font-size:11px; text-align:center;">
                        ...
                        $img = new empresa();
                        $img->imagemCNPJ();
                        ?><!--<img src="../imagens/logomaster1.gif" width="110" height="79">--><br>
                    ...
                    <td valign="middle" style="font-size:12px;">
                        ...
                        $query = "SELECT A.especifica, A.codigo, A.tipo, COUNT(A.codigo) AS total
                                    FROM rhstatus AS A 
                                    INNER JOIN rh_clt AS B ON(A.codigo = B.status)
                                    WHERE B.id_regiao = '{$regiao}' AND A.status_reg = 1 AND A.codigo NOT IN(90,60,101,81,62,61,64,66,65,63)
                                    GROUP BY A.codigo
                                    ORDER BY A.especifica";
                        $sql = mysql_query($query) or die("Erro ao selecionar clt com eventos");
                        while ($row_status = mysql_fetch_assoc($sql)) {
                            $id++;
                            ...
                            echo '<a href="#' . $row_status['codigo'] . '" class="botao' . $ativo . '" title="Visualizar Participantes em ' . $row_status['especifica'] . '" onclick="return false">' . abreviacao($row_status['especifica'], 5) . ' (' . $row_status['total'] . ')</a>';
            ...
            <?php
            $qr_status = mysql_query("SELECT * FROM rhstatus WHERE status_reg = '1' ORDER BY especifica ASC");
            while ($row_status_old = mysql_fetch_assoc($qr_status)) {
                
                $id++;
                ?>

                <div class="status" id="<?= $row_status_old['codigo'] ?>" <?php
                if (($evento == true and $row_status_old['codigo'] == $evento) or ( $evento == false and $row_status_old['codigo'] == 10)) {
                ...
                ?>>
                    <table cellpadding="0" cellspacing="1" id="folha">
                        ...
                        <?php
                        $qr_participantes = mysql_query("SELECT b.id_projeto, b.nome AS nome_projeto, a.id_curso, a.id_clt, a.status, a.nome
	   												  FROM rh_clt a
											     LEFT JOIN projeto b
												        ON a.id_projeto = b.id_projeto
												     WHERE a.id_regiao = '$regiao'
													   AND b.status_reg = '1'
                                                                                                           AND a.status = '{$row_status_old['codigo']}'
                        ...
                        while ($row_participante = mysql_fetch_assoc($qr_participantes)) {
                            ...
                            $qr_evento = mysql_query("SELECT date_format(data, '%d/%m/%Y') AS data, date_format(data_retorno, '%d/%m/%Y') AS data_retorno FROM rh_eventos WHERE id_clt = '{$row_participante["id_clt"]}' AND cod_status = '{$row_participante["status"]}'  AND status=1 ORDER BY id_evento DESC");
                            ...
                            ?>">
                                ...
                                <td align="left">
                                    <a href="acao_evento.php?clt=<?= $row_participante['id_clt'] ?>&regiao=<?= $regiao ?>" onClick="return hs.htmlExpand(this, {objectType: 'iframe'})" class="participante" title="Inserir evento para <?= $row_participante['nome'] ?>">
                                        <?= abreviacao($row_participante['nome'], 4, 1) ?> <img src="folha/sintetica/seta_<?php
                                        if ($seta++ % 2 == 0) {
                                            echo 'um';
                                        } else {
                                            echo 'dois';
                            ...
                            <?php
                            $ultimo_projeto = $row_participante['id_projeto'];
                        } 
