...
<title>RAIS - Relat&oacute;rio para sistema</title>
</head>
<body>
<?php
$qr_empresa = mysql_query("SELECT REPLACE(REPLACE(REPLACE(cnpj, '.', ''),'/',''),'-','') as cnpj, endereco, bairro, cidade, uf, cod_municipio, email, 
                              ...
                              responsavel,
                              SUBSTR(tel,2,2) as ddd,natureza,proprietarios,cnae,
                              DATE_FORMAT(data_nasc, '%d%m%Y') as data_nasc,
                              TRIM(REPLACE(SUBSTR(tel,5,10),'-','')) as telefone
                                  FROM rhempresa WHERE id_regiao = '$row_regiao[id_regiao]' ") or die(mysql_error());
$empresa    = mysql_fetch_assoc($qr_empresa);


$qr_projetos_ativos = mysql_query("SELECT * FROM projeto WHERE status_reg = '1' AND id_regiao = '$row_regiao[id_regiao]' ");
...
echo "SELECT DISTINCT(rh_clt .id_clt), rh_clt.nome, rh_folha_proc.ano 							
							FROM rh_clt 
							INNER JOIN rh_folha_proc ON rh_folha_proc.id_clt = rh_clt.id_clt	
                                                        WHERE rh_folha_proc.ano = '$ano_base'  AND rh_clt.id_regiao NOT IN ('2','4','6','7','11','13','20','26','36') 
                                                        AND rh_clt.id_projeto IN($projetos_ativos) ORDER BY `rh_clt`.`nome`  ASC";
...
$linha1['NUMERO'] = $numero = explode(',', $empresa['endereco']);
$linha1['NUMERO'] = explode('-', $linha1['NUMERO'][1]);
$linha1['NUMERO'] = substr(RemoveAcentos(RemoveCaracteres(RemoveEspacos($linha1['NUMERO'][0]))), 0, 6);
...
$linha2['NUMERO'] = substr(RemoveAcentos(RemoveCaracteres(RemoveEspacos($linha2['NUMERO'][0]))), 0, 6);
...
$linha2['BAIRRO']      = substr($empresa['bairro'],0,19);
...
$linha2['NATUREZA']     = substr($empresa['natureza'],0,4);
...
$qr_empregado  = mysql_query("SELECT DISTINCT(rh_clt .id_clt), rh_clt.nome, rh_folha_proc.ano, rh_clt.id_curso, rh_clt.data_nasci, rh_clt.nacionalidade, rh_clt.escolaridade, rh_clt.cpf, rh_clt.campo1, rh_clt.serie_ctps, rh_clt.data_entrada,rh_clt.etnia, rh_clt.deficiencia, rh_clt.sexo, rh_clt.rh_sindicato,rh_clt.id_regiao, rh_clt.pis
FROM rh_clt 
INNER JOIN rh_folha_proc ON rh_folha_proc.id_clt = rh_clt.id_clt	
 WHERE rh_folha_proc.ano = '$ano_base'  AND rh_clt.id_regiao NOT IN ('2','4','6','7','11','13','20','26','36') AND rh_clt.id_projeto IN($projetos_ativos) ORDER BY `rh_clt`.`nome`  ASC");
