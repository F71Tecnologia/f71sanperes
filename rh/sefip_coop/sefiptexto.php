...
$data_comparacao = implode('-', array_reverse(explode('/', $_REQUEST['data'])));
$data            = str_replace('/','',$_REQUEST['data']);

// Buscando Ids
$qr_folha    = mysql_query("SELECT * FROM folhas WHERE status = '3' AND mes = '$mes' AND ano = '$ano' AND regiao != '36'");
$total_folha = mysql_num_rows($qr_folha);

if(!empty($total_folha)) {

	while($row_folha = mysql_fetch_assoc($qr_folha)) {
		$qr_participantes     = mysql_query("SELECT * FROM folha_cooperado WHERE id_folha = '$row_folha[id_folha]'  AND status = '3'");
		$total_participantes += mysql_num_rows($qr_participantes);
		while($row_participantes = mysql_fetch_assoc($qr_participantes)) {			
			
			$ids[]      = $row_participantes['id_folha_pro'];
...
//if($parte_sefip == 7) {
	...
	mysql_query("INSERT INTO sefip (mes, ano, regiao, projeto, folha, tipo_sefip, data, autor) VALUES ('$mes', '$ano', '', '', '', '3', NOW(), '$_COOKIE[logado]')");
	// Mensagem Final
