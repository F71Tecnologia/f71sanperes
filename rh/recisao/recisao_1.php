...
$total_aguardo = mysql_num_rows($qr_aguardo);
    ...
    <?php while($row_aguardo = mysql_fetch_array($qr_aguardo)) {
            ...
            <tr style="background-color:<?php if($cor++%2!=0) { echo '#F0F0F0'; } else { echo '#FDFDFD'; } ?>">
                ...
                <td><a href="recisao_1.php?tela=2&enc=<?=$link?>"><?=$row_aguardo['nome']?></a></td>
                <td><?=$NomeProjeto?></td>
            ...
            <tr style="background-color:<?php if($cor++%2!=0) { echo '#F0F0F0'; } else { echo '#FDFDFD'; } ?>">
                ...
                <td align="center">
                    <?php if(!empty($total_rescisao_complementar)) { ?>
                          <a href="../arquivos/recisaopdf/rescisao_complementar_<?=$row_demissao[0]?>_1.pdf" class="link" target="_blank" title="Visualizar Rescisão Complementar"><img src="../../imagens/pdf.gif" border="0"></a>
                	<?php } else {
						  $link = str_replace('+','--',encrypt("$regiao&$row_demissao[0]&$row_rescisao[0]")); ?>
                          <a href="recisao_complementar.php?enc=<?=$link?>" class="link" target="_blank" title="Gerar Rescisão Complementar"><img src="../../imagens/pdf.gif" border="0"></a>
                	<?php } ?>
                </td>
                <td>R$<?php 
...
$fgts = number_format(mysql_result($qr_liquido,0) * 0.08, 2, ',', '.'); ?>
    	...
    	<form action="recisao_1.php" name="form1" method="post" onsubmit="return validaForm()">
    	<table cellpadding="4" cellspacing="0" style="width:80%; margin:0px auto; border:0; line-height:30px;">
          ...
          <tr>
            ...
            <td><input name="diastrab" type="text" id="diastrab" value="<?=$dias_trabalhados?>" size="1" maxlength="2"> dias (data para demissão: <?=$data_demissaoF?>)</td>
          </tr>
          <tr>
...
if($dispensa == 60) {
	...
	$cod_saque_fgts = '02';
        ...
        //VERIFICA SE TEM MAIS DE UM ANO
        if($um_ano == 1){
            ...
            $msg = "Trabalhou menos de 1 ano. Não tem direito a FÉRIAS VENCIDAS E 1/3 DE FÉRIAS";
        }
...
if($verifica_insalu_prop > $data_demissao) {


	
			        ////INSALUBRIDADE PROPORCINAL	
						$qr_mov = mysql_query("SELECT * FROM rh_movimentos WHERE anobase = '$ano_demissao' AND cod = '0001' ");
						$row_mov = mysql_fetch_assoc($qr_mov);
						$valor_insalubridade = ($row_mov['fixo'] * 0.20) / 30;
						$valor_insalubridade = $valor_insalubridade * $dias_trabalhados;
						
						

}elseif($clt_insalubridade == 1){ 
	
	$qr_mov = mysql_query("SELECT * FROM rh_movimentos WHERE cod = '0001' AND anobase = 2012") or die(mysql_error());
	$row_mov = mysql_fetch_assoc($qr_mov);
	
	$valor_insalubridade = $row_mov['fixo'] * 0.20;	
} else {
...
/////PEGANDO A QUANTIDADE DE DIAS RESTANTES PARA O TÉRMINO DO CONTRATO ( TIPO DISPENSA 63,64,66)
		...
		$dias_restantes   =  45 - $dias_restantes;
		...
		$dias_restantes   =  ( $data_hoje - mktime(0,0,0,$mes_admissao, $dia_admissao, $ano_admissao)) / 86400; ////quantidade de dias trabalhados desde a data de admissão
		$dias_restantes   =  90 - $dias_restantes;	
...
if($aviso == 'indenizado' and $t_ap == 2) {
	...
	$aviso_previo_valor_a = $salario_base - $valor_insalubridade; //+ $valor_diario_3 - $valor_insalubridade;  valor acréscimo padrão, sem cálculos
	
	 if($dispensa == 63  or $dispensa == 66) {
		
		// Valor acréscimo
	    $art_479   = ($valor_salario_dia * $dias_restantes) / 2 ;	
...
if($t_13 == 1) {
	...
	if($fator == 'empregado'){
		...
		$meses_ativo_dt = $meses_ativo_dt - 1;
	}
...
$data_aviso_previo_    =  date('Y-m-d', strtotime("$data_aviso"));
...
<?php $cont = 0;
      ...
      while($row_evento = mysql_fetch_array($result_eventos)) {
        ...
        if(!empty($total_evento)) {
			...
			switch($row_evento['cod']){
				...
				case '5061':   $gratificacao_fds =  $row_total_evento['valor'];
				break;
				
				case '5060': $gratificacao_upa = $row_total_evento['valor'];
				break;
				
				case '5912': $gratificacao = $row_total_evento['valor'];
				break;
    ...
    <table width="95%" cellpadding="0" cellspacing="0">
	 ...
	 $valor_grativicacao = $gratificacao_upa + $gratificacao_fds + $gratificacao;
	  ?>
	...
	if($re_tot_rendimento != 0 or $re_tot_desconto != 0) {  ?>
      ...
      <tr>
        <td colspan="4" style="font-size:14px; text-align:center; font-weight:bold;">
            R$ <?=number_format($valor_sal_familia+$valor_adnoturno+$valor_comissao+$valor_grativicacao+$hora_extra+$valor_atraso + $valor_outro+$aviso_previo_valor_a+$terceiro_ss+$re_tot_rendimento-$re_tot_desconto+$vale_refeicao-$debito_vale_refeicao +$valor_insalubridade,2,',','.'); ?>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <?php
// Arquivo TXT
$conteudo = "INSERT INTO rh_recisao(id_clt,ajuda_custo, nome, id_regiao, id_projeto, id_curso, data_adm, data_demi, data_proc, dias_saldo, um_ano, meses_ativo, motivo, fator, aviso, aviso_valor, dias_aviso, data_fim_aviso, fgts8, fgts40, fgts_anterior, fgts_cod, fgts_saque, sal_base, saldo_salario, inss_ss, ir_ss, terceiro_ss, previdencia_ss, dt_salario, inss_dt, ir_dt, previdencia_dt, ferias_vencidas, umterco_fv, ferias_pr, umterco_fp, inss_ferias, ir_ferias, sal_familia, to_sal_fami, ad_noturno, insalubridade, vale_refeicao, debito_vale_refeicao, a479, a477, comissao, gratificacao, extra, outros, movimentos, valor_movimentos, total_rendimento, total_deducao, total_liquido, arredondamento_positivo, avos_dt, avos_fp, data_aviso, devolucao, faltas, valor_faltas, user, folha, adicional_noturno, dsr, desc_auxilio_distancia) VALUES ('$id_clt', '$ajuda_custo', '$nome', '$idregiao', '$idprojeto', '$idcurso', '$data_entrada', '$data_demissao', NOW(), '$dias_trabalhados', '$um_ano', '$meses_ativo', '$dispensa', '$fator', '$aviso', '$aviso_previo_valorT', '$previo', '$data_fim_avprevio', '$fgts8_totalT', '$fgts4_totalT', '$anterior', '$cod_mov_fgts', '$cod_saque_fgts', '$UltSalT', '$saldo_de_salarioT', '$inss_saldo_salarioT', '$irrf_saldo_salarioT', '$terceiro_ssT', '$previ_ssT', '$valor_tdT', '$valor_td_inssT', '$valor_td_irrfT', '$previ_dtT', '$fv_valor_baseT', '$fv_um_tercoT', '$fp_valor_totalT', '$fp_um_tercoT', '$ferias_inssT', '$ferias_irrfT', '$valor_sal_familiaT', '$valor_sal_familia_totT', '$valor_adnoturnoT', '$valor_insalubridadeT', '$vale_refeicaoT', '$debito_vale_refeicaoT', '$art_479', '$valor_atrasoT', '$valor_comissaoT', '$valor_grativicacao', '$hora_extra', '$valor_outroT', '$a_rendimentos', '$a_rendimentosva', '$to_rendimentosT', '$to_descontosT', '$valor_rescisao_finalT', '$arredondamento_positivo', '$meses_ativo_dt', '$meses_ativo_fp', '$data_aviso', '$devolucaoT', '$faltas', '$valor_faltas', '$user', '0', '$adicional_noturno', '$dsr', '$desc_auxilio_distancia');\r\n";
...
$linkir   = str_replace('+', '--', encrypt("$regiao&$id_clt&$nome_arquivo"));
        ...
        <table width="50%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><a href="recisao_1.php?tela=4&enc=<?=$linkir?>" class="botao">Processar Rescis&atilde;o</a></td>
            <td><a href="recisao_1.php?tela=2&enc=<?=$linkvolt?>" class="botao">Voltar</a></td>
          </tr>
        </table>
        <p>&nbsp;</p>
        </td>
