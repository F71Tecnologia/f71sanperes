...
while($row_bolsista = mysql_fetch_array($result_bolsista)){
	...
	<TD align='center' $bord>R$ ".number_format($row_curso[salario],2,',','.')."</TD>
...
$result_clt = mysql_query("SELECT A.id_curso,A.id_curso,A.nome,A.campo3,A.cpf,A.rg,A.tipo_conta,A.agencia,A.conta,B.tipopg,B.campo1,C.nome as funcao, C.salario
FROM rh_clt AS A LEFT JOIN tipopg AS B ON (A.tipo_pagamento=B.id_tipopg) 
LEFT JOIN curso as C
ON C.id_curso = A.id_curso
WHERE A.banco = '$row_banco[id_banco]' AND A.status = '10' AND A.id_projeto =  '$projeto'
ORDER BY `A`.`nome`  ASC");
...
while($row_clt = mysql_fetch_array($result_clt)){
	...
	<TD $bord>R$ ".number_format($row_clt[salario],2,',','.')."</TD>
	<TD $bord align='center'>$contratacao</TD>
	<TD $bord align='center'>{$row_clt['tipopg']}</TD>
	</TR>";
	
	$valor_smpclt = str_replace(",",".",$row_clt['salario']);
...
<TD>$endereco</TD>
<TD align='center'>".number_format($row_curso2[salario],2,',','.')."</TD>
...
<TD align='center'>$telefone</TD>
<TD>$endereco</TD>
<TD align='center'>".number_format($row_cursoclt2[salario],2,',','.')."</TD>
