...
$movimentos_rescisao = $obj_rescisao->getMovimentosRescisaoArray($id_clt, $id_rescisao);

//$movimentos_rescisao['descicao']
//$movimentos_rescisao['id_mov']
//$movimentos_rescisao['valor']
//$movimentos_rescisao['campo_rescisao']
//$movimentos_rescisao['categoria']
//echo '<pre>';
//print_r($movimentos_rescisao);
//echo '</pre>';
//
//exit();
...
foreach ($movimentos_lancaveis_credito as $movimento) {
    $valor_movimento = $movimentos_rescisao[$movimento['id_mov']]['valor'];
    $total_rendimento += $valor_movimento;
    if (($cont % 3) == 0) {
        echo '<tr>';
    }
    ?>
                        <td><span class="numero"><?= $movimento['campo_rescisao'] ?></span>&nbsp;<?= $movimento['descicao'] ?></td>
                        <td>R$ <?= number_format( ($valor_movimento<=0) ? '0' : $valor_movimento  ,2,',','.'); ?></td>                        
                        <?php
                        if (count($movimentos_lancaveis_credito) == ($cont + 1)) {
                            ...
                            while (($sobra % 3) != 0) {
                                if ((($sobra + 1) % 3) != 0) {
                                    ...
                                    echo '<td class="secao">TOTAL RESCISÓRIO BRUTO</td><td class="secao"><div class="valor" id="total_rescisorio_bruto">R$ '.number_format($total_rendimento,2,',','.').'</div></td>';
                                }
                                $sobra++;
                            }
                        }
                        ...
                        if (count($movimentos_lancaveis_debito) == ($cont + 1)) {
                            ...
                            while (($sobra % 3) != 0) {
                                if ((($sobra + 1) % 3) != 0) {
                                    ...
                                    $total_liquido = ($total_rendimento - $total_deducoes);
                                    echo '<td class="secao">TOTAL DAS DEDU&Ccedil;&Otilde;ES</td><td class="secao"><div class="valor" id="total_deducoes">R$ '.number_format($total_deducoes,2,',','.').'</div></td>' .
                                    '<tr><td></td><td></td><td></td><td></td><td class="secao">VALOR RESCISÓRIO LÍQUIDO</td><td class="secao"><div class="valor" id="valor_rescisorio_liquido">R$ '.number_format($total_liquido,2,',','.').'</div></td></tr>';
                                }
                                $sobra++;
                            }
                        }
