...
if(isset($_REQUEST['method']) && !empty($_REQUEST['method'])){ 
     if($_REQUEST['method'] == "desprocessar_recisao"){
         ...
         $dados = $objRescisao->verificaSaidaPagaDeRecisao($_REQUEST['id_recisao'], $_REQUEST['id_regiao'], $_REQUEST['id_clt']);
         return $dados;
     }
}
...
if($_GET['voltar_aguardando'] == true){
    ...
    header("Location: recisao2_teste.php?regiao={$_GET['regiao']}");
}
				    ...
				    <td>AÇÃO</td>	
                                ...
                                while ($row_aguardo = mysql_fetch_array($qr_aguardo)) {
                                    ...
                                    ?>">
                                        ...
                                        <td><a href="recisao2_teste.php?tela=2&enc=<?= $link ?>"><?= $row_aguardo['nome'] ?></a></td>
                                        <td><?= $NomeProjeto ?></td>
                                        <td><?= $row_aguardo['locacao'] ?></td>
                                        <td><?= $NomeCurso ?></td>
                                        <td>
                                            <?php if ($ACOES->verifica_permissoes(82)) { ?>
                                                <a href="recisao2_teste.php?voltar_aguardando=true&id=<?php echo $row_aguardo[0]; ?>&regiao=<?php echo $_GET['regiao']; ?>&id_clt=<?php echo $row_aguardo[0]; ?>" title="Desprocessar Aguardando Demissão" onclick="return window.confirm('Você tem certeza que quer desprocessar aguardando demissão?');"><img src="../imagensrh/deletar.gif" /></a>
                                            <?php } ?>
                                        </td>
                                    </tr>
...
//                                echo $sql_rescisao_complementar;
                                ...
                                ?>">
                                    ...
                                    <td align="center">
                                        <?php if ($ACOES->verifica_permissoes(82)) { ?>
                                            <!--<a href="recisao2_teste.php?deletar=true&id=<?php echo $row_rescisao[0]; ?>&regiao=<?php echo $_GET['regiao']; ?>&id_clt=<?php echo $row_demissao[0]; ?>" title="Desprocessar Rescisão" onclick="return window.confirm('Você tem certeza que quer desprocessar esta rescisão?');"><img src="../imagensrh/deletar.gif" /></a>-->
                        ...
                        <form name="acao_recisao" id="acao_recisao" action="recisao2_teste.php">
...
// Calculando Saldo FGTS
                    ...
                    <form action="recisao2_teste.php" name="form1" method="post" onsubmit="return validaForm()">
                ...
                case 3:
                    ...
                    //Pegando informações do CLT
                    $dadosClt = $objClt->getDadosCltRescisao($id_clt, $dispensa);
                    
                    ///setando os dados da Rescisão
                    $objCalcRescisao->setClt($id_clt);
                    $objCalcRescisao->setMotivoRescisao($dispensa);
                    $objCalcRescisao->setTipoAviso($aviso);
                    
                    //Carregando as configurações da rescisão
                    $objCalcRescisao->getRescisaoConfig($dadosClt['um_ano']);
                    
                    
                    ///formatando datas para os calculos abaixo
                    $arrayDataDemissao  = $objCalcRescisao->getData($dadosClt['data_demi'], 2);
                    $salario_contratual = $dadosClt['salario'];
                    $periodoTrabalhado  = $objCalcRescisao->getPeriodoTrabalhado($dadosClt['data_entrada'], $dadosClt['data_demi']);
                    ...
                    //Insalubridade, mantendo a forma como já funciona na rescisão, não lança movimento, esse valor é gravado na tabela rh_recisao
                  ...
                  //  $salarioFamilia = $objCalcRescisao->getSalarioFamilia($salario_contratual, $diasTrab, $dados['id_projeto'], $dados['data_demi']); //Verificar na calculos.php                     
                    ...
                    //////////////////////////////////////////////////////////////////////////////                        
                    $saldo_salario      = $objCalcRescisao->getSaldoSalario($salario_contratual, $diasTrab);  //Retora o valor do saldo de salário
                    $movimentosLancados = $objCalcRescisao->getMovimentosRescisaoLancados(); //retorna um array com os movimentos que foram lançados pra a rescisão, e a incidencia de bases INSS e IRRF
                    ...
                    /////////////////////////////////////////////////
                    $decimoTerceiro = $objCalcRescisao->getDecimoTerceiroProporcional($salarioBaseCalc, $dadosClt['data_entrada'],$data_demissao); //Retorna um array com os valores de décimo e décimom indenizado
                    ...
                    $ferias = $objCalcFerias->getPeriodoFeriasRescisao($id_clt, $dadosClt['data_entrada'], $data_demissao);// PEga os períodos de férias que o clt tem direito,nesse método já está verificando se ele já gozou as férias
