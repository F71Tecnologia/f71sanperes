...
if($t_ss == 1){
	...
	$Calc -> MostraINSS($saldo_de_salario,$data_demissao);
	$inss_saldo_salario = $Calc -> valor;
        	         
	$base_irrf_saldo_salarios = ($saldo_de_salario - $inss_saldo_salario)  + $valor_insalubridade;	
...
if($t_ap == 1 and $aviso == 'indenizado') {
     ...
     for($d=1;$d <= (int)$diferenca_anos; $d++){	
            $valor_diario_3 += ($salario_base_limpo/$qnt_dias_mes) * 3;	
     }
     $valor_aviso_previo =   $salario_base_limpo + $valor_diario_3;
