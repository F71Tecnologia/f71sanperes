	...
	<?php } ?>
      ...
      <?php // Consulta de Clts que foram demitidos
		...
		$qr_rescisao_complementar    = mysql_query("SELECT * FROM rh_rescisao_complementar WHERE rescisao_rescisao = '$row_rescisao[0]'");
                ...
                if(substr($row_rescisao['data_proc'],0,10) >= '2013-04-01'){
...
$qr_clt = mysql_query("SELECT A.nome, A.campo3, A.data_demi, A.data_entrada, A.id_projeto, A.id_curso, A.id_regiao, A.id_curso,
                  ...
                  /*MESES TRABALHADOS*/
                        if(YEAR(data_entrada) = YEAR(data_demi), 
                        PERIOD_DIFF(CONCAT(YEAR(data_demi),'',MONTH(data_demi)),   CONCAT(YEAR(data_entrada),'',MONTH(data_entrada)) ), 
                        PERIOD_DIFF(CONCAT(YEAR(data_demi),'',MONTH(data_demi)),   CONCAT(YEAR(data_demi),'','01') ))
...
$qr_movimentos = mysql_query("select nome_movimento, id_mov, incidencia,
                            ...
                            WHERE id_clt = $id_clt   AND ((ano_mov = 2013 AND lancamento = 1) OR lancamento = 2) 
                            AND status !=0 AND incidencia = '5020,5021,5023'
                            AND id_mov NOT IN(200,56)
...
if($t_ss == 1){
        ...
        $valor_salario_dia_REND  =  $salario_base / $qnt_dias_mes;
        
        
        
        $saldo_de_salario  =  $valor_salario_dia * $dias_trabalhados;  
        $saldo_de_salario_REND  =  $valor_salario_dia_REND * $dias_trabalhados;  
        
        
    
	// Calculando INSS sobre Saldo de Salários
	$Calc -> MostraINSS($saldo_de_salario_REND,$data_demissao);
	$inss_saldo_salario = $Calc -> valor;
        	         
	$base_irrf_saldo_salarios = ($saldo_de_salario_REND - $inss_saldo_salario)  + $valor_insalubridade;	
        
       
	// Calculando IRRF sobre Saldo de Salários
	$Calc -> MostraIRRF($base_irrf_saldo_salarios,$id_clt,$idprojeto, $data_demissao);
...
if($t_13 == 1  and $verifica_13_folha == 0) {
        ...
        //Quantidade de mese
        $Calc->Calc_qnt_meses_13_ferias_rescisao($data_entrada, $data_demissao);
...
if($t_ap == 1 and $aviso == 'indenizado') {
     ...
     $valor_aviso_previo =   $salario_base + $valor_insalubridade_integral + $valor_diario_3;
     
     if($aviso == 'indenizado'){       
        
        // Décimo Terceiro Saldo de Salário (Indenizado)
        $qnt_13_indenizado   = 1;
        $valor_13_indenizado = $salario_base / 12;  
...
////TOTAIS ///
/////////////
$TOTAL_SALDO_SALARIO       = $saldo_de_salario_REND - $inss_saldo_salario-$irrf_saldo_salario;
$TOTAL_DECIMO_PROPORCIONAL = $valor_td-$valor_td_inss-$valor_td_irrf;
$TOTAL_FERIAS              = $fv_valor_base+$fv_um_terco+$fp_valor_total+$fp_um_terco;
$TOTAL_OUTROS_VENCIMENTOS  = $valor_sal_familia + $valor_atraso + $valor_outro + $valor_ap_recebido_trab+$valor_13_indenizado+$valor_insalubridade+$multa_fv+$fv_um_terco_dobro;
...
$to_rendimentos =   $saldo_de_salario_REND + $valor_td + $TOTAL_FERIAS + $TOTAL_OUTROS_VENCIMENTOS + $to_mov_rendimentos;
...
?>
  ...
  <form action="acao.php" method="post" name="Form" id="Form">
    <table cellpadding="0" cellspacing="0" style="background-color:#FFF; margin:0px auto; width:80%; border:0; line-height:24px;">
      ...
      <tr>
        <td class="secao" style=" color:   #999;">Saldo de sal&aacute;rio (<?=$dias_trabalhados?>/<?php echo $qnt_dias_mes; ?>):</td>
        <td style=" color:  #999;">
      ...
      <tr>
         <td class="secao">Saldo de sal&aacute;rio 
             com <br>os rendimentos (<?=$dias_trabalhados?>/<?php echo $qnt_dias_mes; ?>):</td>
        <td>
            R$ <?=formato_real($saldo_de_salario_REND)?> <?php if(!empty($faltas)) { echo '('.$faltas.' faltas)'; } ?>
        </td>
...
$campos_insert['sal_base']          =  $salario_base;
$campos_insert['saldo_salario']     =  $saldo_de_salario_REND;
...
if(sizeof($movimentos) >0){
    ...
    while($row_mov = mysql_fetch_assoc($qr_movimentos)){
        $conteudo .= "INSERT INTO rh_movimentos_rescisao (id_rescisao, id_mov, id_clt, nome_movimento, valor ) VALUES (ultimo_id_rescisao,'$row_mov[id_mov]', '$row_mov[id_clt]', '$row_mov[nome_movimento]', '$row_mov[valor_movimento]' ); \r\n";
    }
}
...
foreach($fp as $linha) {
	...
	$link = str_replace('+', '--', encrypt("$regiao&$id_clt&$idi[0]"));
        echo '<script>location.href="nova_rescisao_1.php?enc='.$link.'"</script>';
	exit();
