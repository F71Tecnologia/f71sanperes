...
// gamby
if ($dia_competencia) {
    $arr_admitidos = $dao->getAdmitidosDiario($id_master, $projeto);
    
    $relacao = $dao->carregaRelacao($arr_admitidos, $competencia);
} else {
    $dao->ano = $ano_competencia;
    $dao->mes = $mes_competencia;
    
    $arr_admitidos = $dao->getAdmitidos($id_master, $ano_competencia, $mes_competencia, $projeto);
    $arr_transferidos = $dao->getTransferidos($id_master, $ano_competencia, $mes_competencia, $projeto);
    $arr_demitidos = $dao->getDemitidos($id_master, $ano_competencia, $mes_competencia, $projeto);
    
    $relacao = $dao->carregaRelacao(array_merge($arr_admitidos, $arr_transferidos, $arr_demitidos), $competencia);
}
// gamby
//$arr_admitidos = $dao->getAdmitidos($id_master); // original

//print_array($relacao); exit();
...
$endereco_autorizado = $master['logradouro'];
...
$txt->dados($txt->completa($txt->nome(removeAcentosCaracteres($master['logradouro'])), 40)); // Endereço do Autorizado
...
$txt->dados($txt->completa(' ', 92)); // Filler 50 + 40
$txt->fechalinha();
...
foreach ($relacao as $cnpj => $arr_clt) {
    if (!empty($cnpj_projeto)) {
        if ($cnpj == $cnpj_projeto) {
            ...
            $qnt_primeiro_dia = $dao->getPrimeiroDiaNew($cnpj, $ano_competencia . '-' . $mes_competencia);                        
            ...
            foreach ($arr_clt as $clt) {
                ...
                if (empty($clt['id_curso']) || $clt['id_curso'] <= 0) {
                    $erros['CURSO'][$clt['id_clt']]['nome'] = utf8_encode('LINHA ' . $sequencia . ' - Função em branco ou zerado #' . $clt['id_clt'] . ' ' . $clt['nome_funcionario']);
                }
...
//                }
                ...
                if (empty($clt['escolaridade']) || $clt['escolaridade'] <= 0 || $clt['escolaridade'] > 14) {
                ...
                if (empty($clt['hora_semana']) || $clt['hora_semana'] <= 0 || $clt['hora_semana'] > 44) {
                    ...
                    $erros['HORA_SEMANA'][$clt['id_clt']]['nome'] = utf8_encode('LINHA ' . $sequencia . ' - Horas contratuais menores que 1 ou maiores que 44. FUNÇÃO: #' . $clt['id_curso'] . ' ' . $res_curso['nome_curso'] . '. PROJETO:' . $res_curso['nome_projeto'] . '. REGIÃO:' . $res_curso['nome_regiao'] . '. FUNCIONÁRIO: #' . $clt['id_clt'] . ' ' . $clt['nome_funcionario']);
                }
                ...
                if (empty($clt['cbo']) || $clt['cbo'] <= 0) {
                    $res_curso = $dao->getCurso($clt['id_curso']);
                    $erros['CBO'][$clt['id_clt']]['nome'] = utf8_encode('LINHA ' . $sequencia . ' - CBO branco ou zerado. FUNÇÃO: ' . $res_curso['nome_curso'] . ' na Região ' . $res_curso['nome_regiao'] . ' (#' . $clt['id_clt'] . ' ' . $clt['nome_funcionario'] . ')');
                }
        ...
        foreach ($arr_clt as $clt) {
            $clt['numero_ctps'] = trim($clt['numero_ctps']);
...
//            if (empty($clt['id_curso']) || $clt['id_curso'] <= 0) {
//                $erros['FUNCAO'][$clt['id_clt']]['nome'] = utf8_encode('LINHA ' . $sequencia . ' - Curso em branco ou zerado #' . $clt['id_clt'] . ' ' . $clt['nome_funcionario']);
//            }
            if (empty($clt['cbo']) || $clt['cbo'] <= 0) {
                $erros['CBO'][$clt['id_clt']]['nome'] = utf8_encode('LINHA ' . $sequencia . ' - CBO em branco ou zerado #' . $clt['id_clt'] . ' ' . $clt['nome_funcionario']);
            }
            if (!validaCPF($clt['cpf_limpo'])) {
                $erros['CPF'][$clt['id_clt']]['nome'] = utf8_encode('LINHA ' . $sequencia . ' - CPF: "' . $clt['cpf_limpo'] . '" #' . $clt['id_clt'] . ' ' . $clt['nome_funcionario']);
            }
...
if (isset($_GET['print'])) {
    ...
    $log->gravaLog('Relatório de Impostos', "Relatório Gerado: CAGED");
}
exit();
