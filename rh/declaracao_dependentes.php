...
$qr_estCivil = mysql_query("SELECT * FROM estado_civil WHERE id_estado_civil = '{$row_clt['id_estado_civil']}'");
$row_estCivil = mysql_fetch_assoc($qr_estCivil);


 $query_dep = "SELECT * FROM dependentes WHERE id_bolsista = '{$row_clt['id_clt']}'";
$result_dep = mysql_query($query_dep);
while ($row_dep = mysql_fetch_assoc($result_dep)) {
    $arr_dep[] = $row_dep;
}
...
//            });
    ...
    <body>
        ...
        <div class="pagina">
            ...
            <h4 class="text-center">DECLARAÇÃO DE DEPENDENTES PARA FINS <br> DE DESCONTO DO IMPOSTO DE RENDA</h4>
            <br>

            <p class="text-justify">Empregadora: <strong>INSTITUTO DE ATENÇÃO BÁSICA E AVANÇADA À SAÚDE</strong></p>
            <p class="text-justify">CNPJ: <strong>09.652.832.0003-38</strong></p>

            <br>

            <p class="text-justify">Nome do Declarante: <strong><?= $row_clt['nome'] ?></strong></p>
            <p class="text-justify">RG: <strong><?= $row_clt['rg'] ?></strong>	 CPF: <strong><?= $row_clt['cpf'] ?></strong>	 Estado Civil: <strong><?= $row_estCivil['nome_estado_civil'] ?></strong></p>
            <p class="text-justify">Endereço: <strong><?= $row_clt['endereco'] ?>, <?= $row_clt['numero'] ?></strong>, Complemento: <strong><?= $row_clt['complemento'] ?></strong></p>
            <p class="text-justify">Cidade: <strong><?= $row_clt['cidade'] ?></strong>	 Estado: <strong><?= $row_clt['uf'] ?></strong>	 CEP: <strong><?= $row_clt['cep'] ?></strong></p>
            <p class="text-justify">Para fins da Legislação do Imposto de Renda, declaro que são minhas dependentes as pessoas abaixo relacionadas:</p>
            <br>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php foreach ($arr_dep as $val) { ?>
                        <?php if (!empty($val['nome1'])) { ?>
                            <tr>
                                <td><?= $val['nome1'] ?></td>
                                <td>Filho(a)</td>
                                <td><?= $val['datas1'] ?></td>
            ...
            <br>
            <p class="text-justify">Cientes da proibição da dedução de um mesmo dependente por ambos os cônjuges, declaramos, sob pena da lei, que as informações aqui prestadas são verdadeiras e de nossa inteira responsabilidade, não cabendo à Empregadora nenhuma responsabilidade perante a fiscalização.</p>

            <br>
            <br>

            <p class="text-center">____________________________<br>
            Declarante</p>

            <br>
            <br>
            
            <p class="text-center">_________________________<br>
            Cônjuge</p>

            <br>

            <p class="text-justify">SAO PAULO, <?= $dia ?> de <?= $mes ?> de <?= $ano ?>.</p>
            <p class="text-justify">Observações: </p>
            <ul>
                <li>Sempre que ocorrer alteração, esta Declaração deverá ser renovada.</li>
                <li>No caso de dependentes comuns, Filho(a)s, esta Declaração deverá ser Assinada pelo Declarante e Conjugue. </li>   
            </ul>
