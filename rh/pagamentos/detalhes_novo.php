 ...
 */
$query_banco  = "SELECT A.id_banco_principal FROM projeto AS A WHERE A.id_projeto = '{$row_clt['id_projeto']}'";
...
//    $tipo_id = "170";
    $tipo_id = "34"; // na entrada e saida do IABAS,a multa FGTS é id/tipo 20
...
//    $valor = ($tipo_guia != 3 && $tipo_guia != 5) ? str_replace('.', ',', $_POST['valor']) : str_replace('.', '', $_POST['valor_multa']);
    ...
    $query_xxx = "SELECT A.*, IF(A.nome_banco = '', B.nome, A.nome_banco ) as banco FROM rh_clt as A
                           LEFT JOIN  bancos as B
                           ON A.banco = B.id_banco
                           WHERE A.id_clt = $id_clt";
    $qr_clt = mysql_query($query_xxx) or die($query_xxx."<br>".mysql_error());
    ...
    //CRIANDO A SAÍDA
    $sql = "INSERT INTO saida (id_regiao, id_projeto, id_banco,  id_user, nome, especifica,  tipo,  valor, data_proc, data_vencimento, status,comprovante, nosso_numero, tipo_boleto, cod_barra_gerais, id_referencia, id_tipo_pag_saida, entradaesaida_subgrupo_id, id_clt, descricao)
          VALUES ('$regiao', '$projeto', '$banco', '{$_COOKIE['logado']}', '$nome', '$nome', '$tipo', '$valor',NOW(), '$data',  '1', '2', '$nosso_numero', '2', '$cod_barra_gerais','1', '1', '$subgrupo', '$id_clt', '$descricao') ";
    ...
    if ($tipo_guia == 1) {
        ...
        $nomearquivo = '/../../rh/arquivos/ferias/ferias_' . $clt . '_' . $id_ferias . '.pdf';
        if(!file_exists($nomearquivo)){
            $nomearquivo = '../../rh_novaintra/ferias/arquivos/ferias_' . $clt . '_' . $id_ferias . '.pdf';
        }
        
        $arquivo_destino = '../../comprovantes/saida/'. $dataAno .'/'. $dataMes .'/'. $id_comprovante . '.' . $id_saida . '.pdf';

        //COPIANDO RECIBO DE FÉIRAS DO RH PARA O FINANCEIRO
        ...
        $arquivo_destino2 = '../../comprovantes/saida/'. $dataAno .'/'. $dataMes .'/' . $id_saida_files_2 . '.' . $id_saida . '.pdf';
    ...
    echo "<script> 
          setTimeout(function(){
            window.parent.location.href = 'formulario_para_solicitacao.php?id_saida='+$id_saida+'&id_clt='+$id_clt+'&tipo='+$tipo_guia;
    ...
    <head>
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('.date, .multa_dt').on('focusout', function () {
