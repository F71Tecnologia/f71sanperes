...
$qr_rpa = mysql_query("SELECT A.*,G.id_saida, B.nome,  C.nome as nome_projeto,C.id_projeto, B.banco, B.conta, B.conta_dv, B.agencia, B.agencia_dv, B.tipo_pagamento,B.nome_banco, B.cpf,B.pis,E.nome as funcao
...
//        echo '<a href="../../autonomo/arquivo_rpa_pdf/' . $row_rpa['id_rpa'] . '_' . $row_rpa['id_autonomo'] . '.pdf">link</a><br>';
        if ($zip->open($pathFile, ZipArchive::CREATE) === true) {
            $zip->addFile('../../estagiario/arquivo_rpa_pdf/' . $row_rpa['id_rpa_estagiario'] . '_' . $row_rpa['id_estagiario'] . '.pdf', normalizaNome($row_rpa['nome_projeto']) . '/' . normalizaNome($row_rpa['nome']) . '.pdf');
        }
    }
    $zip->close();
        ...
        <form action="" method="post" name="form1" action="<?php $_SERVER["PHP_SELF"]; ?>">
            ...
            <div id="content">
                ...
                if ($contagem != 0) {
                    ?>
                    <p class="controls"> 
                        <input type="submit" name="gerarRPA" value="Gerar PDF de RPEs" id="gerarRpa" class="button" />
                    ...
                    <table width="100%" cellspacing="0" cellpadding="0" class="grid" id="tableRpa">
                        ...
                        while ($row_rpa = mysql_fetch_assoc($qr_rpa)) {
                            ...
                            echo '<tr>';
                            echo '<td>'. ++$i  .'</td>';
                            echo '<td align="center">' . $row_rpa['id_saida'] . '</td>';
                            echo '<td align="center">' . $row_rpa['id_rpa_estagiario'] . '</td>';
                            ...
                            echo '<td align="center">' . $row_rpa['agencia'] . '</td>';
                            echo '<td align="center">' . $row_rpa['agencia_dv'] . '</td>';
                            echo '<td align="center">' . $row_rpa['conta'] . '</td>';
                            echo '<td align="center">' . $row_rpa['conta_dv'] . '</td>';
                            echo '<td align="center">' . $row_rpa['hora_mes'] . '</td>';
                            echo '<td align="center">' . number_format($row_rpa['valor'], 2, ',', '.') . '</td>';
                            echo '<td align="center">' . number_format($row_rpa['valor_vt'], 2, ',', '.') . '</td>';
                            echo '<td align="center">' . number_format($row_rpa['valor_vr'], 2, ',', '.') . '</td>';
                            echo '<td align="center">' . number_format($row_rpa['valor_va'], 2, ',', '.') . '</td>';
                            ...
                            $valor_vt += $row_rpa['valor_vt'];
                            $valor_vr += $row_rpa['valor_vr'];
                            $valor_va += $row_rpa['valor_va'];
                            $totalizador_liquido += $row_rpa['valor_liquido'];
                            $projetoAnt = $row_rpa['id_projeto'];
                        }
                        
                        echo'<tr height="40" style="background-color: #c8ebf9">
                                <td colspan="12" align="right" style="font-weight:bold;">QUANTIDADE DE RPES:</td>
                        ...
                        echo'<tr height="40" style="background-color: #c8ebf9">
                                ...
                                <td align="center"> R$ ' . number_format($subtotal_bruto, 2, ',', '.') . '</td>
                                <td align="center"> R$ ' . number_format($valor_vt, 2, ',', '.') . '</td>
                                <td align="center"> R$ ' . number_format($valor_vr, 2, ',', '.') . '</td>
                                <td align="center"> R$ ' . number_format($valor_va, 2, ',', '.') . '</td>
                                <td align="center"> R$ ' . number_format($subtotal_liquido, 2, ',', '.') . '</td>
