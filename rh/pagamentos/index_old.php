...
for($ano=2009; $ano<=date('Y'); $ano++):
	...
	foreach($meses as $nome_mes => $mes):
		...
		// COntrole
		$query_saida = mysql_query("SELECT id_saida FROM pagamentos_especifico WHERE mes = '$mes' AND ano = '$ano' AND  id_clt = '$row_recisao[id_clt]'");
		$num_saida = mysql_num_rows($query_saida);
		
		$id_saida = @mysql_result($query_saida,0);
		$query_controle = mysql_query("SELECT * FROM saida WHERE id_saida = '$id_saida'");
