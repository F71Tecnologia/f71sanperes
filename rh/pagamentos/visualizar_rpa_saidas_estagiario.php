...
$qr_rpa = mysql_query("SELECT *
                      ...
                      INNER JOIN estagiario AS B ON (A.id_estagiario = B.id_estagiario)
...
$nome_completo = "Recibo de Pagamento de Estágio - " . htmlentities($row_rpa['nome']) . " - $texto " . mesesArray($mes) . '/' . $ano;
$qr_saida = mysql_query("SELECT *, B.nome as descricao,C.nome as enviado_por, D.nome as pago_por,B.estorno,B.id_saida,
                         ...
                         WHERE A.id_rpa_estagiario = $id_rpa_estagiario AND tipo_vinculo = $tipo_guia");
...
$verifica_estorno = mysql_query("SELECT IF(B.estorno != 0 ,'estorno', B.status) as verifica_saida
                                 ...
                                 WHERE id_rpa_estagiario = '$id_rpa_estagiario' AND tipo_vinculo = $tipo_guia  ORDER BY data_proc DESC LIMIT 1");
...
if(isset($_REQUEST['excluir'])){
    ...
    $qr_saida = "SELECT status, id_estagiario FROM saida WHERE id_saida = $id_saida;";
...
//        $row_rpa = mysql_fetch_assoc($qr_rpa);
        $qr_rpaSaidaAssoc = mysql_query("SELECT B.id_saida, B.`status`, A.tipo_vinculo
                                FROM rpa_saida_assoc AS A
                                INNER JOIN saida AS B ON A.id_saida = B.id_saida
                                WHERE A.id_rpa_estagiario = {$id_rpa_estagiario} AND B.status != 0;") or die ("Erro ao consultar o rpa_saida_assoc");
        ...
        if($cont==0){
            ...
            echo "<script> 
                    setTimeout(function(){
                    window.parent.location.href = 'http://" . $_SERVER['HTTP_HOST'] . "/intranet/rh/pagamentos/index.php?id=1&regiao=$regiao&mes=$mes_consulta&ano=$ano_consulta&filtrar=1&tipo_pagamento=7&tipo_contrato=5';
        ...
        <script>
            $(function() {
                $("#editar").on("click", function() {
                    $("#form1").attr('action','editar_rpa_saidas_estagiario.php');
                    $("#form1").submit();
...
<?php } ?>
    ...
    if ($row_verifica['verifica_saida'] == 'estorno') {
        ...
        echo " <div id='message-box' class='message-blue txcenter' style='margin-top: 10px; width:600px;'>
                 <a href='cadastro_rpa_guias.php?id_rpa_estagiario=$id_rpa_estagiario&tipo_guia=$tipo_guia&id_autonomo=$id_autonomo&mes_consulta=$mes_consulta&ano_consulta=$ano_consulta'>REENVIAR GUIA</a>
               </div>";
    }
    ?>
</div>
