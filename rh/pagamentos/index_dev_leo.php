    ...
    switch ($tipoPagamento) {
        ...
        case 3: //"3" => "Pagamentos Férias"
            $query = "SELECT A.*,B.regiao as nome_regiao, C.nome as nome_projeto,  D.nome as nome_clt
                    ...
                    WHERE A.status = 1 AND B.id_master = $usuario[id_master] AND A.mes = $mes AND A.ano = $ano AND B.id_regiao = '{$regiao}' ORDER BY projeto;";
                                    ...
                                    while ($row_resci = mysql_fetch_assoc($result)) {
                                        ...
                                        while ($row1 = mysql_fetch_array($resc)) {
                                            $sql_resci = "SELECT B.id_rescisao, C.vinculo_id_rescisao, A.`status`
                                                        ...
                                                        WHERE A.id_clt = {$row_resci['clt']} AND C.`status` = 1 AND C.vinculo_id_rescisao IS NOT NULL AND B.id_rescisao = {$row1['id_recisao']}";
                                            ...
                                            <td>
                                                <div>
                                                    ...
                                                    foreach ($arrayR as $idClt => $rescisao) {
                                                        foreach ($rescisao as $idRescisao => $cor) {
                                                            ?>
                                                            <div class="<?= $cor; ?>" style="float:left; width:100%" align="center">
                                                                <?php if ($cor == '') {
                                                                    ?>
...
//                                            echo mysql_num_rows($query_controle);
                                            switch ($row_controle['sindicato']) {
                                                case '0':
                                                    $color = "cor-2";
                                                    $link_guias = 'cadastro_1_dev.php'; // depois alterar o nome para cadastro_1.php
                                                    break;
                                                case 1:
                                                    $color = "cor-1";
                                                    $link_guias = 'visualizar_guias_saidas_dev.php';
