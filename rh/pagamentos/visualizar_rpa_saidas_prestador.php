...
$nome_completo = "RPA - " . htmlentities($row_rpa['c_fantasia']) . " - $texto " . mesesArray($mes) . '/' . $ano;
...
if(isset($_REQUEST['excluir'])){
    ...
    $qr_saida = "SELECT status, id_prestador FROM saida WHERE id_saida = $id_saida;";
...
<?php } ?>
    ...
    if ($row_verifica['verifica_saida'] == 'estorno') {
        ...
        echo " <div id='message-box' class='message-blue txcenter' style='margin-top: 10px; width:600px;'>
                 <a href='cadastro_rpa_guias.php?id_rpa=$id_rpa&tipo_guia=$tipo_guia&id_autonomo=$id_prestador&mes_consulta=$mes_consulta&ano_consulta=$ano_consulta'>REENVIAR GUIA</a>
               </div>";
    }
    ?>
</div>
