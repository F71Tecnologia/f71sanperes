...
$id_rpa_estagiario         = $_REQUEST['id_rpa_estagiario'];
$tipo_guia      = $_REQUEST['tipo_guia']; /// TIPO: 2- GPS, 3 - IR
$id_estagiario    = $_REQUEST['id_estagiario'];
$usuario = carregaUsuario();

$qr_rpa = mysql_query("SELECT A.id_rpa_estagiario, B.nome, B.id_regiao, B.id_projeto, A.mes_competencia,  A.ano_competencia, A.valor_inss, A.valor_liquido, A.valor_ir
                        FROM rpa_estagiario  as A
                        INNER JOIN estagiario as B
                        ON A.id_estagiario = B.id_estagiario                   
                        WHERE A.id_rpa_estagiario = $id_rpa_estagiario;");
...
switch($tipo_guia){    
      case 1:
        $texto = "Recibo de Pagamento de Estágio";
        $tipo_saida = 260;       
        $subgrupo = 41;
...
$nome_completo = htmlentities($row_rpa[nome])." - $texto ".  mesesArray($mes).'/'.$ano;
  
if(isset($_REQUEST['acao'])){
    
        $id_rpa_estagiario           = $_POST['id_rpa_estagiario'];
...
//        $valor            = str_replace('.','',$_POST['valor']);
        ...
        $qr_rpa = mysql_query("SELECT A.id_rpa_estagiario, A.id_estagiario, A.valor_liquido , C.nome as nome_projeto, D.regiao as nome_regiao
                               FROM rpa_estagiario as A
                               INNER JOIN  estagiario as B
                               ON A.id_estagiario = B.id_estagiario
                               ...
                               WHERE A.id_rpa_estagiario = $id_rpa_estagiario") or die (mysql_error());
        $row_rpa = mysql_fetch_assoc($qr_rpa);
        //DADOS DO BANCO
        ...
        $sql = "INSERT INTO saida (id_regiao, id_projeto, id_banco, id_user, nome, id_nome, especifica, tipo, adicional, valor, data_proc, data_vencimento, status,comprovante, nosso_numero, tipo_boleto, cod_barra_gerais, id_referencia, id_tipo_pag_saida, entradaesaida_subgrupo_id, mes_competencia, ano_competencia,id_estagiario)
                VALUES ('$row_banco[id_regiao]', '$row_banco[id_projeto]', '$banco', '$_COOKIE[logado]', '$nome_completo','', '$especifica', '$tipo_saida', '', '$valor',NOW(), '$data',  '1', '2', '$nosso_numero', '2', '$cod_barra_gerais','1', '1', '$subgrupo', '$row_rpa[mes_competencia]', '$row_rpa[ano_competencia]','$id_estagiario') ";
        ...
        if(mysql_query($sql)){
           ...
           ///ASSOCIANDO O RPA A SAÍDA
            $qr_rpa_assoc = mysql_query("INSERT INTO rpa_saida_assoc (id_rpa_estagiario, id_saida, tipo_vinculo) VALUES ('$id_rpa_estagiario', '$id_saida', '$tipo_guia')");
            ...
            $nome_arquivo = $id_saida_files.'.'.$id_saida.'.pdf'; 
              ...
              if($tipo_guia == 1 ){
                  ...
                  $arquivo_origem  = '../../estagiario/arquivo_rpa_pdf/'.$id_rpa_estagiario."_".$id_estagiario.'.pdf';
                  $arquivo_destino = '../../comprovantes/saida/'.date('Y').'/'.date('m').'/'.$nome_arquivo;
                 ...
                 if(copy($arquivo_origem, $arquivo_destino)){
                       echo 'Envio concluído...';            
                                echo "<script> 
                                  setTimeout(function(){
                                    window.parent.location.href = 'http://".$_SERVER['HTTP_HOST']."/intranet/rh/pagamentos/index.php?id=1&regiao=$regiao&mes=$mes_consulta&ano=$ano_consulta&filtrar=1&tipo_pagamento=7';
                    ...
                    if(move_uploaded_file($arquivo['tmp_name'], '../../comprovantes/saida/'.date('Y').'/'.date('m').'/'.$nome_arquivo)){
...
<html>
    ...
    <body>
            <form action="cadastro_rpa_guias_estagiario.php" name="form1" id="form_rpa" method="post" enctype="multipart/form-data">

            <input type="hidden" name="id_rpa_estagiario" id="id_rpa_estagiario" value="<?php echo $id_rpa_estagiario ?>" />
            <input type="hidden" name="id_estagiario" id="id_estagiario" value="<?php echo $id_estagiario ?>" />
            ...
            <table width="90%" border="0" align="center" cellpadding="5" cellspacing="0">
                ...
                <tr>
                    ...
                    <td><label for="bancos"></label>
                        <select name="bancos" id="bancos" class="validate[required,custom[select]]">
                            ...
                            <?php 
                            $query_banco = mysql_query("SELECT * FROM bancos WHERE id_regiao IN (SELECT id_regiao FROM regioes WHERE id_master = '{$usuario['id_master']}') AND status_reg = '1'");
                ...
                <?php } else { ?>
                       <tr>
                            <td colspan="2" align="right" valign="midlle">Visualizar Recibo:</td>
                            <td> 
                                <a id="ver_rpa" href="http://<?php echo $_SERVER['HTTP_HOST'];?>/intranet/estagiario/arquivo_rpa_pdf/<?php echo $id_rpa_estagiario; ?>_<?php echo $id_estagiario;?>.pdf"  target="iframe">     <img border="0px" src="../folha/imagens/verfolha.gif" width="18" height="18" /></a>
                                <input type="hidden" name="arquivo" id="arquivo_rpa" value="<?php echo $id_rpa_estagiario; ?>_<?php echo $id_estagiario;?>.pdf" /> 
                            </td>
                        </tr>
                <?php } ?>  
