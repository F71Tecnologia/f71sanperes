...
if($tipo_folha == 'COOP'){
	$tabela_consulta = 'folhas';
        $banco_sql =  "OR  id_master = (SELECT id_master FROM regioes WHERE id_regiao = 47)"; 
}
...
$regioes = mysql_result(mysql_query(" SELECT  GROUP_CONCAT(id_regiao) as regioes FROM regioes 
                             WHERE id_master = (SELECT id_master FROM regioes WHERE id_regiao = $id_regiao)
                             $banco_sql;"),0);
...
$nome_completo = urldecode("$tipo_folha $texto $nome_mes/$ano $projeto Folha: $id_folha $decimo3- $nome_regiao");
?>
<style type="text/css">
p.botao {
			...
			$.post('actions/cadastra.php',
					{
						...
						tipo_contrato :$('#tipo_contrato_pg').val(),
						tipo 	: '<?=$tipo?>',
...
<form action="" method="post" enctype="multipart/form-data" name="form1" id="form">
    ...
    <input type="hidden" name="tipo_contrato_pg"  id="tipo_contrato_pg" value="<?php echo $tipo_contrato_pg;?>"/>
    <table width="90%" border="0" align="center" cellpadding="5" cellspacing="0">
        ...
        <tr>
          ...
          <td><label for="bancos"></label>
            <select name="bancos" id="bancos">
                ...
                $query_banco = mysql_query("SELECT * FROM bancos WHERE id_regiao = '{$usuario['id_regiao']}' AND id_projeto = '{$projeto}' AND status_reg = '1'");
                ?>
                <?php
        ...
        <?php } ?>
          <tr>
          <td colspan="4" align="center">
          <input type="hidden" name="folha_projeto" id="folha_projeto" value="<?php echo $row_folha['projeto']?>" />
          <input type="hidden" name="folha_regiao" id="folha_regiao"  value="<?php echo $row_folha['regiao']?>" />
