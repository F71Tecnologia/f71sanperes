...
if(isset($_REQUEST['id_autonomo'])){
    $id = $_REQUEST['id_autonomo'];
...
} elseif(isset($_REQUEST['id_prestador'])) {
    $id = $_REQUEST['id_prestador'];
    $qr_rpa = mysql_query("SELECT A.id_rpa, B.c_fantasia, B.id_regiao, B.id_projeto, A.mes_competencia,  A.ano_competencia, A.valor_inss, A.valor_liquido, A.valor_ir
                        FROM rpa_autonomo  as A
...
if(isset($_REQUEST['id_autonomo'])) {
     $nomeRpa = $row_rpa['nome'];
} elseif(isset($_REQUEST['id_prestador'])) {
     $nomeRpa = $row_rpa['c_fantasia'];
}
...
$nome_completo = "RPA - ".htmlentities($nomeRpa)." - $texto ".  mesesArray($mes).'/'.$ano;
...
//        $valor            = str_replace('.','',$_POST['valor']);
        ...
        if(isset($_POST['id_autonomo'])){
            
            $id = $_POST['id_autonomo'];
        ...
        } elseif(isset($_POST['id_prestador'])) {
            
            $id = $_POST['id_prestador'];
            $qr_rpa = mysql_query("SELECT A.id_rpa, A.id_prestador, A.valor_liquido , C.nome as nome_projeto, D.regiao as nome_regiao
                               FROM rpa_autonomo as A
                               INNER JOIN  prestadorservico as B
                               ON A.id_prestador = B.id_prestador
                               INNER JOIN projeto as C
        ...
        if(isset($_POST['id_autonomo'])){
             $sql = "INSERT INTO saida (id_regiao, id_projeto, id_banco, id_user, nome, id_nome, especifica, tipo, adicional, valor, data_proc, data_vencimento, status,comprovante, nosso_numero, tipo_boleto, cod_barra_gerais, id_referencia, id_tipo_pag_saida, entradaesaida_subgrupo_id, mes_competencia, ano_competencia,id_autonomo)
                VALUES ('$row_banco[id_regiao]', '$row_banco[id_projeto]', '$banco', '$_COOKIE[logado]', '$nome_completo','', '$especifica', '$tipo_saida', '', '$valor',NOW(), '$data',  '1', '2', '$nosso_numero', '2', '$cod_barra_gerais','1', '1', '$subgrupo', '$row_rpa[mes_competencia]', '$row_rpa[ano_competencia]','$id') ";
        } elseif(isset($_POST['id_prestador'])) {
             $sql = "INSERT INTO saida (id_regiao, id_projeto, id_banco, id_user, nome, id_nome, especifica, tipo, adicional, valor, data_proc, data_vencimento, status,comprovante, nosso_numero, tipo_boleto, cod_barra_gerais, id_referencia, id_tipo_pag_saida, entradaesaida_subgrupo_id, mes_competencia, ano_competencia,id_prestador)
                VALUES ('$row_banco[id_regiao]', '$row_banco[id_projeto]', '$banco', '$_COOKIE[logado]', '$nome_completo','', '$especifica', '$tipo_saida', '', '$valor',NOW(), '$data',  '1', '2', '$nosso_numero', '2', '$cod_barra_gerais','1', '1', '$subgrupo', '$row_rpa[mes_competencia]', '$row_rpa[ano_competencia]','$id') ";
        }
           ...
           ///ASSOCIANDO O RPA A SAÍDA
            ...
            $nome_arquivo = $id_saida_files.'.'.$id_saida.'.pdf'; 
              ...
              if($tipo_guia == 1 ){
                  ...
                  $arquivo_origem  = '../../autonomo/arquivo_rpa_pdf/'.$id_rpa."_".$id.'.pdf';
...
<html>
    ...
    <body>
            <form action="cadastro_rpa_guias.php" name="form1" id="form_rpa" method="post" enctype="multipart/form-data">

            <input type="hidden" name="id_rpa"      id="id_rpa"      value="<?php echo $id_rpa ?>" />
            <?php  if(isset($_REQUEST['id_autonomo'])){
                $nameID = 'id_autonomo';
            } elseif(isset($_REQUEST['id_prestador'])) {
                $nameID = 'id_prestador';
            } ?>
            <input type="hidden" name="<?=$nameID?>" id="<?=$nameID?>" value="<?php echo $id ?>" />
                ...
                <?php } else { ?>
                       <tr>
                            ...
                            <td> 
                                ...
                                if(isset($_REQUEST['id_autonomo'])) { ?>
                                    <a id="ver_rpa" href="http://<?php echo $_SERVER['HTTP_HOST'];?>/intranet/autonomo/arquivo_rpa_pdf/<?php echo $id_rpa; ?>_<?php echo $id;?>.pdf"  target="iframe">     <img border="0px" src="../folha/imagens/verfolha.gif" width="18" height="18" /></a>
                               <?php } elseif(isset($_REQUEST['id_prestador'])) { ?>
                                    <a id="ver_rpa" href="http://<?php echo $_SERVER['HTTP_HOST'];?>/intranet/autonomo/imprime_rpa_prestador.php?id_prestador=<?=$id?>&reg=<?=$regiao?>&pro=<?=$projeto?>&rpa=<?=$id_rpa?>"  target="iframe">     <img border="0px" src="../folha/imagens/verfolha.gif" width="18" height="18" /></a>
                               <?php } ?>
                                <input type="hidden" name="arquivo" id="arquivo_rpa" value="<?php echo $id_rpa; ?>_<?php echo $id;?>.pdf" /> 
                            </td>
                        </tr>
                <?php } ?>  
