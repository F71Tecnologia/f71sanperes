...
$qr_estagiario = mysql_query("SELECT CONCAT(A.id_estagiario,' - ',A.nome) as estagiario, B.regiao as nome_regiao, C.nome as nome_projeto
                        FROM estagiario as A
                       INNER JOIN regioes as B
                       ON B.id_regiao = A.id_regiao
                       INNER JOIN projeto as C
                       ON C.id_projeto = A.id_projeto
                      WHERE A.id_estagiario = '$estagiario'");
...
if ($tipo_guia == 1) {
    ...
    $tipo_nome = "FÉRIAS";
    $subgrupo = 1;
    $qr_saida = mysql_query("SELECT B.id_banco, B.id_saida, B.status, B.nome as descricao, B.especifica, B.valor, B.estorno,
                                ...
                                WHERE A.id_estagiario = '$estagiario' AND (B.tipo = 76 OR B.tipo = 8) AND A.ano = $ano;");
} elseif ($tipo_guia == 2) {
    ...
    $qr_saida = mysql_query("SELECT B.id_banco, PG.id_saida,B.nome, C.id_banco as id_banco, B.estorno,B.estorno_obs,
                                    ...
                                    LEFT JOIN bancos AS F 
                                    ON B.id_banco = F.id_banco
                                    WHERE B.status != '0' AND PG.mes = '$mes' AND PG.ano = '$ano' AND PG.id_estagiario = '$estagiario' AND (B.tipo = '51' OR B.tipo = '31') AND PG.id_rescisao = '$id_rescisao';
                                    ");
} elseif ($tipo_guia == 3) {
    ...
    $qr_saida = mysql_query("SELECT  B.id_banco, E.nome as nome_banco, E.id_banco as id_banco, DATE_FORMAT(B.data_proc, '%d/%m/%Y')  as processado,
                            ...
                            LEFT JOIN bancos AS F 
                            ON B.id_banco = F.id_banco
                            WHERE B.id_estagiario = '$estagiario' AND B.tipo IN(34) AND A.multa_rescisao = 1");
//                            WHERE B.id_estagiario = '$estagiario' AND B.tipo IN(167,31) AND A.multa_rescisao = 1");
...
//                            LEFT JOIN bancos AS F 
//                            ON B.id_banco = F.id_banco
//                            WHERE B.id_estagiario = '$estagiario' AND B.tipo IN(167,170) AND A.rescisao_complementar = 1";
//        echo $teste;
//        exit;
    $qr_saida = mysql_query("SELECT  B.id_banco, E.nome as nome_banco, E.id_banco as id_banco, DATE_FORMAT(B.data_proc, '%d/%m/%Y')  as processado,
                            DATE_FORMAT(B.data_vencimento, '%d/%m/%Y')  as data_vencimento,
                            ...
                            LEFT JOIN bancos AS F 
                            ON B.id_banco = F.id_banco
                            LEFT JOIN pagamentos_especifico_estagiario AS G ON (B.id_saida = G.id_saida)
                            WHERE B.id_estagiario = '$estagiario' AND B.tipo IN(167,31,170) AND A.rescisao_complementar = 1 AND G.id_rescisao = '$id_rescisao' AND A.multa_rescisao <> 2");
    ...
    $qr_saida = mysql_query("SELECT  B.id_banco, E.nome as nome_banco, E.id_banco as id_banco, DATE_FORMAT(B.data_proc, '%d/%m/%Y')  as processado,
                            ...
                            ON B.id_banco = F.id_banco
                            LEFT JOIN pagamentos_especifico_estagiario AS G ON (B.id_saida = G.id_saida)
                            WHERE B.id_estagiario = '$estagiario' AND B.tipo IN(167,31,170) AND A.rescisao_complementar = 0 AND G.id_rescisao = '$id_rescisao' AND A.multa_rescisao = 2 GROUP BY A.id_saida");
    ...
    if($row_saida['status']==1){
        switch ($tp_guia) {
            case 1: // FÉRIAS
                ...
                $qr_pgEspecifico = mysql_query("DELETE FROM pagamentos_especifico_estagiario WHERE id_saida = $id_saida LIMIT 1;") or die ("Erro ao excluir. (pagamentos_específico, saida $id_saida.");
            ...
            case 2: // RESCISAO
                $qr_buscaRescisao = mysql_query("SELECT C.id_rescisao
                                    FROM saida AS A
                                    LEFT JOIN rh_recisao AS B ON (A.id_estagiario = B.id_estagiario)
                                    LEFT JOIN pagamentos_especifico_estagiario AS C ON (B.id_recisao = C.id_rescisao)
                ...
                $qr_buscaResComplementar = mysql_query("SELECT B.id_saida
                                            ...
                                            LEFT JOIN pagamentos_especifico_estagiario AS C ON (A.id_recisao = C.id_rescisao)
...
<html>
    ...
    <body class="novaintra">
        <div id="content">   
            ...
            <h3><?php echo $row_estagiario['estagiario']; ?></h3>
            <p><strong><?php echo $tipo_nome; ?></strong></p>
            <p><strong>Região: </strong><?php echo $row_estagiario['nome_regiao']; ?></p>
            <p><strong>Projeto: </strong><?php echo $row_estagiario['nome_projeto']; ?></p>
...
//                    
    ?>
                        ...
                        <td>
                        <?php if ($reenvio) { ?>
                                <div id="message-box" class="message-blue txcenter" style="margin-top: 10px;">
                                    <a title="Duplicar saida" href="duplicar_saida.php?ID=<?= $row_saida['id_saida'] ?>&tipo=saida&id_rescisao=<?= $row_saida['id_rescisao'] ?>&mes=<?= $row_saida['mes'] ?>&ano=<?= $row_saida['ano'] ?>"  onclick="return hs.htmlExpand(this, {objectType: 'iframe'})">Duplicar Saída</a>
                                </div>
                            <?php } ?>
                        </td>
                        <?php if ($acoes->verifica_permissoes(89)){
                            ...
                            <td>
                                ...
                                <input type="hidden" value="<?php echo $id_estagiario;?>" name="id_estagiario"/>
                                <input type="hidden" value="<?php echo $mes;?>" name="mes_consulta"/>
                                <input type="hidden" value="<?php echo $ano;?>" name="ano_consulta"/>
                                <center><input type="submit" value="Excluir" name="excluir"/></center>
                            </td>
                        <?php }else{?>
                            <td>
                            <center><input type="button" value="Excluir" name="excluir" disabled="disabled" /></center>
                            </td>
                        <?php }
                        }?>
    ...
    if ($row_saida['estorno'] == 1 and ! empty($row_saida['estorno_obs']) != '') {
        echo '<td>' 
        . $row_saida['estorno_obs'] 
        . "<div id=\"message-box\" class=\"message-blue txcenter\" style=\"margin-top: 10px;\">"
        . "<a href=\"detalhes_novo_estagiario.php?id_estagiario={$estagiario}&amp;mes=$mes&amp;ano=$ano&amp;id_rescisao=$id_rescisao&amp;tipo=$tipo_guia&amp;keepThis=true&amp;TB_iframe=true&amp;width=850\" class=\"thickbox\">"
        . "Novo pagamento"
        . "</a>"
        . "</div>"
        . '</td>';
