...
//    $valor = str_replace('.', '', $_POST['valor']);
    ...
    $valorMulta = (isset($_REQUEST['valor_multa']) && !empty($_REQUEST['valor_multa']) ) ? str_replace(",", ".", str_replace(".", "", $_REQUEST['valor_multa'])) : 0;
...
//    $row_banco = mysql_fetch_assoc($qr_banco);
    ...
    //INSERINDO SAÍDA
    $sql = "INSERT INTO saida (id_regiao, id_projeto, id_banco, id_user, nome, id_nome, especifica, tipo, adicional, valor, valor_multa, data_proc, data_vencimento, status,comprovante, nosso_numero, tipo_boleto, cod_barra_gerais, id_referencia, id_tipo_pag_saida, entradaesaida_subgrupo_id,rh_sindicato)
           VALUES ('$row_banco[id_regiao]', '$row_banco[id_projeto]', '$banco', '$_COOKIE[logado]', '$nome','$id_nome', '$nome', '$tipo', '$adicional', '$valor','$valorMulta',NOW(), '$data',  '1', '2', '$nosso_numero', '2', '$cod_barra_gerais','1', '1', '$subgrupo','{$sindicato}') ";
    mysql_query($sql);
    $id_saida = mysql_insert_id();
    $log->gravaLog('Pagamentos', "Pagamento Emitido: ID{$id_saida}");

    //LANÇAMETO CONTABIL
    $array_lancamento = array('id_saida' => $id_saida, 'id_projeto' => $row_banco['id_projeto'], 'id_usuario' => $usuario['id_funcionario'], 'data_lancamento' => date("Y-m-d"), 'historico' => $nome);
    $id_lancamento = $objPlanoContas->inserirLancamento($array_lancamento, $mes_consulta, $ano_consulta);

//    $array_itens[] = array('id_lancamento' => $id_lancamento, 'id_tipo' => $tipo, 'valor' => $valor, 'documento' => $n_documento, 'tipo' => 2, 'id_projeto' => $row_banco['id_projeto']);
//    $array_itens[] = array('id_lancamento' => $id_lancamento, 'id_banco' => $banco, 'valor' => $valor, 'documento' => $n_documento, 'tipo' => 1);
//    $objPlanoContas->inserirItensLancamento($array_itens);
    ...
    if ($id_saida) {
        ...
        for ($i = 0; $i < count($_FILES['arquivo']['name']); $i++) {
            ...
            $nome_arquivo = explode('.', $_FILES['arquivo']['name'][$i]);
            ...
            if (!is_file($diretorio_destino . $nome_arquivo)) {
                if (move_uploaded_file($_FILES['arquivo']['tmp_name'][$i], $diretorio_destino . $nome_arquivo)) {
...
//        echo '<br>id saida: ' . $id_saida . '- id saida_files: ' . $id_saida_files[$x] . '- sql saida_files: ' . $sql_anexos . '<br><br>';
    ...
    ///INSERINDO ANEXO 
//    esse era o upload original do sistema
...
<html>
    <head>
        ...
        <script>
            $(function () {
                ...
                //////VALIDANDO
                $('#arquivo1,#arquivo2,.arquivo').live('change',function () {
                ...
                $('#enviar').click(function () {
                    ...
                    if ($('#tipo_guia').val() == 2 || $('#tipo_guia').val() == 7) {
                        ...
                        console.log(extensao_arquivo2, arquivo2);
    ...
    <body>
        <form name="form1" id="form1" action="cadastro_1.php" method="post" enctype="multipart/form-data">
            ...
            <table width="90%" border="0" align="center" cellpadding="5" cellspacing="0">
                ...
                <tr>
                    <td width="269">&nbsp;</td>
                    <td width="205" align="right"><span style="font-size:12px;">Valor de Multa: R$</span></td>
                    <td width="1081"><input name="valor_multa" type="text" id="valor_multa" size="13" /></td>
                    <td width="36">&nbsp;</td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
                    <td align="right"><span style="font-size:12px;">Data :</span></td>
                    <td><input name="data" type="text" id="data" size="13" class="inputPequeno validate[required]" /></td>
                    <td>&nbsp;</td>
                </tr>
                ...
                <tr>
                    <td colspan="2" align="right" valign="midlle">Selecione o(s) arquivo(s)</td>
                    <td>
                        <input name="arquivo[]" class="arquivo" id="arquivo1" type=file />
                        <br />
