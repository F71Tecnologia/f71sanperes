...
//    header("Content-type: application/xls");
    ...
    echo "  <head>";
    echo "  <title>RELAT”RIO DE PROVIS√O DE GASTOS</title>";
...
if($_COOKIE['debug']){
echo "SELECT A.*,A.id_unidade_pag as unidade_pag,G.id_saida, B.nome,  C.nome as nome_projeto,C.id_projeto, B.banco, B.conta, B.conta_dv, B.agencia, B.agencia_dv, B.tipo_pagamento,B.nome_banco, B.cpf,B.pis,E.nome as funcao, I.unidade
                        FROM  rpa_autonomo as A
                        LEFT JOIN autonomo as B ON (A.id_autonomo = B.id_autonomo)
                        LEFT JOIN projeto as C ON (C.id_projeto = IF(A.id_projeto_pag IS NULL, B.id_projeto, A.id_projeto_pag))
                        ...
                        WHERE A.mes_competencia = '$mes2d' AND A.ano_competencia = '$ano' AND A.id_projeto_pag = '{$usuario['id_regiao']}'
                        #GROUP BY B.id_autonomo
ORDER BY B.id_projeto,B.nome";}
$qr_rpa = mysql_query("SELECT A.*,A.id_unidade_pag as unidade_pag,G.id_saida, B.nome, CONCAT(B.endereco, ' ', B.numero, '', B.complemento, ', ', B.bairro, ' - ', B.cidade, ' ', B.uf, ' ', B.cep) AS endereco_completo, C.nome as nome_projeto,C.id_projeto, B.banco, B.conta, B.conta_dv, B.agencia, B.agencia_dv, B.tipo_pagamento,B.nome_banco, B.cpf,B.pis,E.nome as funcao, I.unidade
                        FROM  rpa_autonomo as A
                        LEFT JOIN autonomo as B ON (A.id_autonomo = B.id_autonomo)
                        LEFT JOIN projeto as C ON (C.id_projeto = IF(A.id_projeto_pag IS NULL, B.id_projeto, A.id_projeto_pag))
                        LEFT JOIN regioes as D ON (D.id_regiao = B.id_regiao)
 ...
 * 09/02/2017 14h16
 * by: Max
 * COMENTANDO O GROUP BY (#GROUP BY B.id_autonomo)
 * POIS TEVE UM CASO QUE A PESSOA
 * TEM 2 RPAS E ESTAVA VINDO S” 1
 */
                ...
                if ($contagem != 0) {
                    ...
                    <table width="100%" cellspacing="0" cellpadding="0" class="grid" id="tableRpa">
                        ...
                        while ($row_rpa = mysql_fetch_assoc($qr_rpa)) {
                            ...
                            echo '<td>' . $row_rpa['unidade'] . '</td>';
                            echo '<td>' . $row_rpa['nome'] . '</td>';
                            echo '<td>' . $row_rpa['cpf'] . '</td>';
                            echo '<td>' . $row_rpa['endereco_completo'] . '</td>';
                        ...
                        echo'<tr height="40" style="background-color: #c8ebf9">
                                <td colspan="13" align="right" style="font-weight:bold;">QUANTIDADE DE RPAS:</td>
                                <td align="center">'. $contagem .'</td>
                            </tr>';
                        
                        echo'<tr height="40" style="background-color: #c8ebf9">
                                <td colspan="13" align="right" style="font-weight:bold;">SUBTOTAIS:</td>
                                <td align="center"> R$ ' . number_format($subtotal_bruto, 2, ',', '.') . '</td>
                                <td align="center"> R$ ' . number_format($subtotal_inss, 2, ',', '.') . '</td>
                                <td align="center"> R$ ' . number_format($subtotal_irrf, 2, ',', '.') . '</td>
                                <td align="center"> R$ ' . number_format($subtotal_iss, 2, ',', '.') . '</td>
                        ...
                        echo'<tr height="40" style="background-color: #c8ebf9">
                                <td colspan="13" align="right" style="font-weight:bold;">TOTAIS:</td>
