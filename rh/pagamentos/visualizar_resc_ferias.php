...
if ($tipo_guia == 1) {
    ...
    $qr_saida = mysql_query("SELECT B.id_banco, B.id_saida, B.status, B.nome as descricao, B.especifica, B.valor, B.estorno,
                                ...
                                WHERE A.id_clt = '$clt' AND (B.tipo = 76 OR B.tipo = 8) AND A.ano = $ano;");
} elseif ($tipo_guia == 2) {
    ...
    $qr_saida = mysql_query("SELECT B.id_banco, PG.id_saida,B.nome, C.id_banco as id_banco, B.estorno,B.estorno_obs,
                                    ...
                                    LEFT JOIN bancos AS F 
                                    ON B.id_banco = F.id_banco
                                    WHERE B.status != '0' AND PG.mes = '$mes' AND PG.ano = '$ano' AND PG.id_clt = '$clt' AND (B.tipo = '51' OR B.tipo = '31') AND PG.id_rescisao = '$id_rescisao';
                                    ");
} elseif ($tipo_guia == 3) {
    ...
    $qr_saida = mysql_query("SELECT  B.id_banco, E.nome as nome_banco, E.id_banco as id_banco, DATE_FORMAT(B.data_proc, '%d/%m/%Y')  as processado,
                            ...
                            LEFT JOIN bancos AS F 
                            ON B.id_banco = F.id_banco
                            WHERE B.id_clt = '$clt' AND B.tipo IN(34) AND A.multa_rescisao = 1");
//                            WHERE B.id_clt = '$clt' AND B.tipo IN(167,31) AND A.multa_rescisao = 1");
...
//        exit;
    $qr_saida = mysql_query("SELECT  B.id_banco, E.nome as nome_banco, E.id_banco as id_banco, DATE_FORMAT(B.data_proc, '%d/%m/%Y')  as processado,
                            ...
                            WHERE B.id_clt = '$clt' AND B.tipo IN(167,31,170) AND A.rescisao_complementar = 1 AND G.id_rescisao = '$id_rescisao' AND A.multa_rescisao <> 2");
    ...
    $qr_saida = mysql_query("SELECT  B.id_banco, E.nome as nome_banco, E.id_banco as id_banco, DATE_FORMAT(B.data_proc, '%d/%m/%Y')  as processado,
                            ...
                            ON B.id_banco = F.id_banco
                            LEFT JOIN pagamentos_especifico AS G ON (B.id_saida = G.id_saida)
                            WHERE B.id_clt = '$clt' AND B.tipo IN(167,31,170) AND A.rescisao_complementar = 0 AND G.id_rescisao = '$id_rescisao' AND A.multa_rescisao = 2 GROUP BY A.id_saida");
...
//                    
    ...
    if ($row_saida['estorno'] == 1 and ! empty($row_saida['estorno_obs']) != '') {
        ...
        . "<div id=\"message-box\" class=\"message-blue txcenter\" style=\"margin-top: 10px;\">"
        . "<a href=\"detalhes_novo.php?id_clt={$clt}&amp;mes=$mes&amp;ano=$ano&amp;id_rescisao=$id_rescisao&amp;tipo=$tipo_guia&amp;keepThis=true&amp;TB_iframe=true&amp;width=850\" class=\"thickbox\">"
        . "Novo pagamento"
        . "</a>"
        . "</div>"
        . '</td>';
