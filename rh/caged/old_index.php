...
if (isset($_POST['verifica_envio'])) {
    ...
    $buscaTrabalhadores = $txt->consultaTrabalhadores($_POST['ids_clt']);
    ...
    while ($row_clt = mysql_fetch_assoc($buscaTrabalhadores)) {
       ...
       if (($ano < $anoCorrente) || ($ano == $anoCorrente && $mes < $mesCorrente)){
           $txt->montaRegTipoX($arquivo, $row_clt);
       }else{
                ...
                <form action="" method="post" name="form1" id="form">
                    ...
                    <table class="tabela_ramon" width="100%">
                        ...
                        while ($row_trab = mysql_fetch_assoc($lista)) {
                            ...
                            <tr <?php echo $class; ?>> 
                                <td align="center">
                                    <input type="checkbox" name="ids_clt[]" class="id" data_total ='<?php echo $total;?>' value="<?php echo $row_trab['id_clt'] ?>" <?= ($check_registro) ? ' checked="checked" ' : ''; ?>/>
                                </td>
                                <td><?php echo $row_trab['id_clt']; ?></td>
                                <td>
                                    <?php if (!empty($erro)) { ?>
                                        <a class="erros" title="<?= implode("<br>", $erro) ?>" href="Edita.php?ID=<?= $row_trab['id_clt'] ?>&<?= implode('&', array_keys($erro)) ?>" onclick="return hs.htmlExpand(this, {objectType: 'iframe'})" target="_blank">
                                            <?= $row_trab['nome_clt'] ?>
                                        </a>
                                        <?php
                                    } else {
