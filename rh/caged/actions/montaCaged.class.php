...
//          //    $result = print_r($qr_total_clt);
        
        $totalADM = mysql_query("SELECT COUNT(ADM.id_clt) AS total
                                FROM (SELECT IF(temp.total > 0,'tem','nao tem') AS status_transf, A.id_clt
                                      FROM rh_clt AS A
                                      LEFT JOIN (SELECT COUNT(T.id_clt) AS total, T.id_clt FROM rh_transferencias AS T GROUP BY T.id_clt) AS temp ON (A.id_clt = temp.id_clt)
                                      LEFT JOIN projeto AS C ON (A.id_projeto = C.id_projeto)
                                      WHERE A.data_entrada < '$this->data_referencia' AND (A.data_demi > '$this->data_referencia' OR A.data_demi IS NULL OR A.data_demi = '0000-00-00') 
                                      AND C.id_master = '$this->id_master' AND A.id_projeto IN($idsProjetoClt)
                                      GROUP BY A.id_clt
                                      HAVING status_transf = 'nao tem') AS ADM;
                                ");
        
        $totalTRANSF1 = mysql_query("SELECT COUNT(temp.id_clt) AS total
                                     FROM(SELECT B.id_clt
                                          FROM rh_transferencias AS A
                                          LEFT JOIN rh_clt AS B ON (A.id_clt = B.id_clt)
                                          LEFT JOIN projeto AS C ON (A.id_projeto_de = C.id_projeto)
                                          WHERE A.id_projeto_de IN($idsProjetoClt) AND A.data_proc < '$this->data_referencia' AND B.data_entrada < '$this->data_referencia' 
                                          AND (B.data_demi > '$this->data_referencia' OR B.data_demi IS NULL OR B.data_demi = '0000-00-00') 
                                          AND C.id_master = '$this->id_master'
                                          GROUP BY B.id_clt) AS temp;
                                         ");
        
        $totalTRANSF2 = mysql_query("SELECT COUNT(temp.id_clt) AS total
                                    FROM(SELECT A.id_clt, A.nome, A.data_entrada, B.id_projeto_de, A.data_demi, B.data_proc
                                        FROM rh_clt AS A 
                                        LEFT JOIN (SELECT * FROM rh_transferencias AS T GROUP BY T.id_clt ORDER BY T.data_proc ASC) AS B ON (A.id_clt = B.id_clt)
                                        LEFT JOIN projeto AS C ON (B.id_projeto_de = C.id_projeto)
                                        WHERE B.data_proc > '$this->data_referencia' AND A.data_entrada < '$this->data_referencia' 
                                        AND (A.data_demi > '$this->data_referencia' OR A.data_demi IS NULL OR A.data_demi = '0000-00-00') AND C.id_master = '$this->id_master' AND B.id_projeto_de IN($idsProjetoClt)
                                        ) AS temp;
                                          ");
        
        $row_totalADM = mysql_fetch_assoc($totalADM);
        $row_totalTRANSF1 = mysql_fetch_assoc($totalTRANSF1);
        $row_totalTRANSF2 = mysql_fetch_assoc($totalTRANSF2);
        
        $result = $row_totalADM['total']+$row_totalTRANSF1['total']+$row_totalTRANSF2['total'];
