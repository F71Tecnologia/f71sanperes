		...
		$desabilitado = "";
		  ...
		  switch ($row_clt['status']) {
			  ...
			  case 200:
				...
				if($exp_data_demi > $exp_data_ini and $exp_data_demi[1] != $exp_data_ini[1]){
					$ferias = "";
					$insere = "sim";
				}else{
					$ferias = "";
			  ...
			  break;
			  case 60 or 61 or 62 or 63 or 81:
			  if($row_folha['terceiro'] != 1 ){
                                       ...
                                       if($exp_data_demi > $exp_data_ini and $exp_data_demi[1] != $exp_data_ini[1]){
                                                ...
                                                #echo "$row_clt[nome]  $row_clt[status]  $row_clt[data_saida]  $row_clt[data_demi] ( $ferias = $dias_trab - nao)<br>";
...
function ValidaForm(){
	...
	if(Nocheck == 0){
      ...
      <br>
      <br>
     <?php if($_COOKIE['logado'] == 87 and $row_projeto['tipo_folha'] == 1) { ?>
     
     <a href="importar_ponto.php?enc=<?=$encmar?>" class="botao">CONTINUAR</a><br>
