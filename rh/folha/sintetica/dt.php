...
//        }
        ...
        if (($flagSindicato['insalubridade'] == 1 || $row_curso['id_curso'] == 4063) && ($tipo_terceiro == 2 || $tipo_terceiro == 3)) {
            ...
            if ($_COOKIE['debug'] == 666) {
                echo '//////////////////////////////VARIÁVEIS DE INSALUBRIDADE////////////////////////////////';
                echo '<pre>';
                echo $dias . '<br>';
...
//            }
    ...
    if (!empty($row_clt['pensao_alimenticia']) && $row_clt['pensao_alimenticia'] > 0) {
        ...
        $sqlVerificaPensao = mysql_query($queryVerificaPensao) or die("Erro ao selecionar favorecidos de pensão");
        ...
        for ($i = 0; $i < count($arrayFavorecidos); $i++) {
            ...
            if ($arrayFavorecidos[$i]['incide_13'] == 1) {
                ...
                if (!is_null($arrayFavorecidos[$i]['sobreSalLiquido']) ||
                    ...
                    /**
                     * QUANDO A PENSÃO FOR SOBRE O 
                     * SALARIO LIQUIDO
                     * ESSA É A OPÇÃO POR PADRÃO
                     */
                    if ($arrayFavorecidos[$i]['sobreSalLiquido'] == 1) {
                        $baseUsadaNaPensao = "Sobre Salário Líquido";
                        $totalFlagsMarcadas++;
                    ...
                    if ($arrayFavorecidos[$i]['sobreSalBruto'] == 1) {
                        $baseUsadaNaPensao = "Sobre Salário Bruto";
                        $totalFlagsMarcadas++;
                        $baseParaPensao = $base_inss - $total_vt_vr_va;
                    }

                    /**
                     * QUANDO A PENSÃO FOR SOBRE  
                     *  1/3 SOBRE O LIQUIDO
                     */
                    ...
                    if ($arrayFavorecidos[$i]['sobreSalMinimo'] == 1) {
                        ...
                        if (!is_null($arrayFavorecidos[$i]['quantSalMinimo'])) {
                            ...
                            $baseUsadaNaPensao = "Sobre {$arrayFavorecidos[$i]['quantSalMinimo']} Salario(s) Mínimo(s)";
                        } else {
                            $baseParaPensao = 0;
                            $msgErrorPensao .= " <br> A Quantidade de Salário Minimo esta Zerada <br> ";
                        }
                    }
                    ...
                    if ($arrayFavorecidos[$i]['umTercoSobreLiquido'] == 1) {
                         ...
                         * ARRAY DE PENSÕES
                         */
                        $nomeMov = "PENSÃO ALIMENTÍCIA 1/3 SOBRE O LIQUIDO ";
                    } else {
                        $valorParaPensao = $baseParaPensao * $arrayFavorecidos[$i]['aliquota'];
                        /**
                         * ARRAY DE PENSÕES
                         */
                        $nomeMov = "PENSÃO ALIMENTÍCIA " . ($arrayFavorecidos[$i]['aliquota'] * 100) . "% ";
                    }
                    ...
                    if ($arrayFavorecidos[$i]['valorfixo'] > 0) {
                        ...
                        $nomeMov = "PENSÃO ALIMENTÍCIA: {$arrayFavorecidos[$i]['favorecido']}";
                    }
                    ...
                    $msgErrorPensao .= " <br> Selecione uma Base de Calculo para Pensão no Formulário do CLT <br> ";
                    $valorParaPensao = $arrayFavorecidos[$i]['valorfixo'];
                }
                 ...
                 * DE UMA OPÇÃO NO CADASTRO DO CLT
                 */
                if ($totalFlagsMarcadas > 1) {
                    $msgErrorPensao .= " <br> Existem mais de uma Base para calculo de Pensão Alimentícia marcada para o CLT  <br> ";
                    $baseParaPensao = 0;
                    $valorParaPensao = 0;
                }
            }
