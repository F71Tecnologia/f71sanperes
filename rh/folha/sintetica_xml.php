              ...
              <td colspan="10">
                    ...
                    <table cellspacing="1" cellpadding="0" width="100%" id="folha">
                     <tr class="secao">
                      ...
                      <td class="nome"><?php if(!empty($decimo_terceiro)) { echo 'MESES'; } else { echo 'DIAS'; } ?></td>
                      <?php
...
///CARREGANDO DADOS PELO XML
$xml = simplexml_load_file("xml_horista/$folha.xml");

foreach($xml->clt as $xml_clt) {
		 ...
		 <tr class="linha_<?php if($linha++%2==0) { echo 'um'; } else { echo 'dois'; } ?> destaque">
            ...
            <td width="6%" class="dias" title="DIAS"><?php if(!empty($decimo_terceiro)) { echo $meses; } else { echo $dias; } ?> </td>
             
             <?php
			  if($projeto_tipo_folha == 1){
				  
				echo '<td align="center"><input type="text" value="'.$row_participante['hora_trabalhada'].'" style="background-color:transparent;border:0;" size="4" class="horas"/> </td>';  
			  }
			  ?>
        ...
        <!-----------LINHA CONTENDO OS VALORES DOS MOVIMENTOS PARA CÁLCULO VIA JAVASCRIPT QUANDO FOR FEITA A ALTERAÇÃO NO CAMPO "HORAS"-------------------->	
          ...
          $array_valores = array("salario" => formato_real($salario), 					
						          ...
						          "irrf_dt" 				=> formato_real($irrf_dt), 				
									...
									"base_inss_rat" 		=> formato_real(($base_inss * 0.058)),
		...
		<?php include('sintetica/update_participante_teste.php');
