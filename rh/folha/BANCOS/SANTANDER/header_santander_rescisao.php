...
/* DADOS DE CONTROLE */
$BANCO = '033'; //CÓDIGO DO BANCO NA COMPENSAÇÃO
...
$QUANT_REGISTROS = $this->numeroSequencial[s] = $this->numeroSequencial[c] = 1;
...
for($i=1;$i<=2;$i++){
    if($i == 1){
        $handle = fopen("../folha/BANCOS/SANTANDER/{$this->CAMINHO}_C.txt", "w+");
    }else if($i == 2){
        $handle = fopen("../folha/BANCOS/SANTANDER/{$this->CAMINHO}_S.txt", "w+");
    }
