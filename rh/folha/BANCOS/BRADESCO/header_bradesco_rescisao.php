...
//echo "../folha/BANCOS/BRADESCO/{$this->PASTA}/{$this->CONSTANTE}_{$this->DD}_{$this->MM}_2_{$this->TIPO}.txt";exit;
for($i=1;$i<=2;$i++){
    if($i == 1){
        $handle = fopen("../folha/BANCOS/BRADESCO/{$this->CAMINHO}_C.txt", "w");
    }else if($i == 2){
        $handle = fopen("../folha/BANCOS/BRADESCO/{$this->CAMINHO}_S.txt", "w");
    }
