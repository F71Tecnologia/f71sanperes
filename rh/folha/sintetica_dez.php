...
if($res_cltpresente !=  ''){   
    $sql_cltFin = mysql_query("SELECT * 
                FROM rh_clt
                WHERE id_regiao = {$regiao} AND id_projeto = {$row_folha['projeto']} AND data_entrada >= '{$data_folha_trv}' AND id_clt NOT IN({$res_cltpresente})") or die(mysql_error());
    ...
    $("body").on("click","#sim",function(){
        ...
        $.ajax({
           ...
           success: function(data){
               if(data.status){
                   window.location.href = "http://f71lagos.com/intranet/rh/folha/sintetica.php?enc="+enc+"&finaliza=" + link;
               }
           }
        });
    });
                  ...
                  <div>
                    ...
                    <table cellspacing="0" cellpadding="0" width="100%">
                     <tr class="secao">
                      ...
                      <td width="6%" align="left" style="padding-left:5px;">ADMISSÃO</td>
...
?>
   ...
   <td width="7%"><?php echo $row_participante['entrada']; ?> </td>
   <td width="7%"><?php echo number_format($row_participante['salario'],2,",","."); ?> </td>
   <td width="6%"><?php if(!empty($decimo_terceiro)) { echo $meses; } else { echo $dias; } ?> </td>
       ...
       <td width="10%"><?php echo formato_real($liquido);  ?></td>
   </tr>
...
<?php  }  ?>
    ...
    <?php include('sintetica/estatisticas_folha_dez.php'); ?>
</div>
