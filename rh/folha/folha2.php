...
//                exit();
		...
		$log->gravaLog('Folha de Pgto.', "Inserção de CLT's na Folha de 13º: ID{$folha}");
                
		print "<script>
		location.href=\"folha2.php?terceiro={$_REQUEST['terceiro']}&tipoterceiro={$_REQUEST['tipoterceiro']}&mes={$_REQUEST['mes']}&ano={$_REQUEST['ano']}&enc=$encinc\"
		</script>";
		exit;		
	}// FIM DÉCIMO TERCEIRO
        else {            
            ...
            $dia_15 = (cal_days_in_month(CAL_GREGORIAN, $row_folha['mes'], $row_folha['ano']) == 31 )? 16 :15;
            while($row_clt = mysql_fetch_array($result_clt)) {
                    // VERIFICANDO SE O CARA JA ESTÁ NA OUTRA FOLHA
                        $RE_verifica = mysql_query("SELECT * FROM rh_folha_proc a INNER JOIN rh_folha b ON a.id_folha = b.id_folha 
                                                    WHERE a.mes = '$row_folha[mes]' AND a.id_clt = '$row_clt[0]' AND a.ano = '$row_folha[ano]' 
                                                    AND a.status = '3' AND b.terceiro = '{$row_folha['terceiro']}'  AND b.tipo_terceiro = '{$row_folha['tipo_terceiro']}'");
...
//                    }else{
                            ...
                            $desabilitado = "";				
                              ...
                              //////////////////////////////////////////////                
                              $teste = $objFolha->verificaInsereFolha($row_clt['status'], $row_folha['data_inicio'], $row_folha['data_fim'], $row_clt['data_entrada'], $row_clt['data_demi'], $row_folha['terceiro']);
                              $insere = $teste['insere'];
                              $ferias = $teste['ferias']; 
                              $fora_da_folha = $teste['fora_da_folha']; 

                              if($insere == "sim" ){     
                                    $INSERT_SQL[] = " ('$row_clt[0]','$regiao','$row_folha[projeto]','$folha','$row_folha[data_proc]','$id_user','$row_folha[mes]','$row_folha[ano]',
                                                      '$row_clt[campo3]','$row_clt[nome]','$dias_trab','2','$row_clt[tipo_pagamento]','$ferias', '$row_clt[salario_outra_empresa]', '$row_clt[desconto_outra_empresa]', '$row_clt[valor_desconto_inss]','$row_clt[id_curso]','$row_clt[id_horario]')";
                                    $idsClt[] = $row_clt['id_clt'];
                                    $log->gravaLog('Folha de Pgto.', "Inserção de CLT's na Folha: ID{$folha}");
                              } else if($fora_da_folha == 'sim') {
                                  $array_fora_da_folha[] = "{$row_clt['id_clt']} - {$row_clt['nome']} | {$row_clt['status']}";
                              }

                              unset($dias_trab, $ferias);		
//                    }      
             }//Fim WHile CLT
        }
...
if(count($INSERT_SQL) >0 ){
    $_SESSION['fora_da_folha'] = $array_fora_da_folha;
}
if(count($INSERT_SQL) >0 ){      
    $INSERT_SQL = implode(',',$INSERT_SQL);
    $sql = "INSERT INTO rh_folha_proc(id_clt,id_regiao,id_projeto,id_folha,data_proc,user_proc,mes,ano,cod,nome,dias_trab,
             ...
             $INSERT_SQL ";        
    mysql_query($sql) or      die ("Erro <br><br>".mysql_error());

    //COLOCANDO STATUS 1 NOS MOVIMENTOS DOS CLTS SELECIONADOS PARA ESSA FOLHA
    $idsClt = implode(',',$idsClt);
    $sqlMov = "UPDATE rh_movimentos_clt SET status_folha=1 WHERE id_clt IN($idsClt) AND mes_mov='$row_folha[mes]' AND ano_mov='$row_folha[ano]'";      
    mysql_query($sqlMov) or die ("Erro <br><br>".mysql_error());
}
...
$encinc = encrypt("$row_folha[regiao]&$folha&2"); 
$encinc = str_replace("+","--",$encinc);        
header("Location: folha2.php?terceiro={$_REQUEST['terceiro']}&tipoterceiro={$_REQUEST['tipoterceiro']}&mes={$_REQUEST['mes']}&ano={$_REQUEST['ano']}&enc=$encinc");
exit;
/* ATÉ AKI */
...
if(!empty($_REQUEST['inclusao'])){
	...
	foreach ($aut as $id_autonomo){
        ...
        $log->gravaLog('Folha de Pgto.', "Emissão");
...
$qr_prelog = "SELECT A.*, DATE_fORMAT(B.data_entrada,'%d/%m/%Y') as data_admissao, 
    if(B.data_saida != '0000-00-00', DATE_FORMAT(data_saida,'%d/%m/%Y'),'') as data_demissao
    FROM rh_folha_proc as A 
    INNER JOIN rh_clt as B ON(A.id_clt = B.id_clt)
    WHERE A.id_folha = '$folha' AND A.status = '$st' ORDER BY nome ASC";

//$qr_prelog = "SELECT A.*, C.especifica, DATE_fORMAT(B.data_entrada,'%d/%m/%Y') as data_admissao, 
//    if(B.data_saida != '0000-00-00', DATE_FORMAT(data_saida,'%d/%m/%Y'),'') as data_demissao
//    FROM rh_folha_proc as A 
...
?><br>
      <table width="531" border="0" class="tabela_acoes">
        <tr>
          <td><a href="folha2.php?enc=<?=$encmar?>">Marcados</a></td>
          <td><a href="folha2.php?enc=<?=$encdes?>">Desmarcados</a></td>
          <td><a href="folha2.php?enc=<?=$encinc?>">Inclus&atilde;o</a></td>
     ...
     <span class="titulo_opcoes">Folha -
      ...
      <br />
  <?php if(count($_SESSION['fora_da_folha']) > 0) { 
      echo '<h3 style="color: red;">Pessoas que não entraram na folha:</h3><br><hr><br>';
     foreach ($_SESSION['fora_da_folha'] as $key => $value) {
         echo '<label style="color: red;">'.$value.'</label><br>';
     }
     echo '<hr>';
     unset($_SESSION['fora_da_folha']);
  } ?>
    <form action="" method="post" name="Form" onSubmit="return ValidaForm()">
      <table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
        <tr class="header_table">
          ...
          <th>Atividade</th>
          <th>Unidade</th>
          <th style="width: 100px; text-align:center">Data de Admissão</th>
          <th style="width: 100px; text-align:center">Data de Demissão</th>
       </tr>
       <?php
...
//                }
            ...
            $destaque = "";
            if($row_clt['especifica'] == "Aguardando Demissão"){
                $destaque = "'color: red;'";
            }else if($row_clt['especifica'] == "Atividade Normal"){
                $destaque = "'color: #666;'";
            }else{
                $destaque = "'color: rgb(219, 202, 28);'";
            }
            ...
            $chek =($row_clt['status'] == '1')? '':"checked";
            $aguardando_demissao = ($row_clt['especifica'] == "Aguardando Demissão") ? 1 : 0;
            print"
		  <tr class=\"novalinha $classcor\">
                    <td align='center'>$dvRE</td>
                    <td align='center'></td>
                    <td align='lefth'><label><input name='status_".$cont."' id='status_".$cont."' type='checkbox' class='removeUsuario' data-key='" . $row_clt['id_clt'] . "' value='$row_clt[0]' $chek $desabilitado $aj>$nomeT</label></td>
                    <td align='lefth'>$nomeC</td>
                    <td align='lefth'>$locacao</td>
                    <td align='center'>{$row_clt['data_admissao']}</td>
                    <td align='center'>{$row_clt['data_demissao']}</td>
		  </tr>";
...
function ValidaForm(){
	...
	if(Nocheck == 0){
      ...
      <?php // }else{ ?>
            <!--<a href="controllerMovimentos.php?enc=<?=$encmar?>" class="botao">CONTINUAR</a><br>-->
      <?php // } ?>
    <?php
