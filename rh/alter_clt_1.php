   ...
   <td>
	...
	<label><br />
	  <input name="trabalha_outra_empresa" type="radio" class="reset" onClick="document.getElementById('outra_empresa').style.display = 'none';" value="nao"
                  <?php if($row['trabalha_outra_empresa'] == 'nao') { echo 'checked'; } ?>>Não</label>
   </td>
  </tr>
...
if(empty($row_cont_vale)) {
	...
	or die ("Erro de digitação no INSERT dos vales query: ". mysql_error());
} else {
	...
	or die ("Erro de digitação no UPDATE dos vales query: ". mysql_error());
}
//
// DEPENDENTES
if(!empty($row['id_antigo'])) {
