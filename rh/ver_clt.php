...
//    
//    switch($type){
//        case 'Férias':$type = 3; break;
//        case 'Rescisão':$type = 4; break;
//        case 'Admissão':$type = 1; break;
...
<?php } ?>
                ...
                // AMANDA
                // MAX: FOI COLOCADO A EXCESSÃO DESSE ID_CLT 5395, POIS ELA SOLICITOU A DEMISSÃO, NESTE CASO SEGUNDO A REJANE, PODE SER DEMITIDO
                // O IDEAL É COLOCAR UMA FLAG
                $(".indResPosMat").click(function () {
                    var clt_prov = $("#clt_prov").val();
                    console.log(clt_prov);
                    if (clt_prov != 6562 || clt_prov != 5196 || clt_prov != 5199) {
                        if ($("#indicativo").val() == 'N') {
                            alert('Esta funcionária não pode ser demitida pois possui estabilidade de 30 dias após o término da licença maternidade.');
                            return false;
                        }
                    }
                });
                ...
                <table align="center" width="100%" cellspacing="0" cellpadding="12" style="font-size:13px; line-height:22px;">
                    ...
                    <tr>
                        ...
                        <td width="84%" bgcolor="#F3F3F3" valign="top">
                            ...
                            <br><i><?php echo 'Ultima Alteração feita por <b>' . $row_user2['nome'] . '<br></b> na data ' . $row['dataalter2'] . '</b> e horário ' . $ultim_atualizacao[1] ?></i><br>
                            <?php 
                        ...
                        <td colspan="2">
                            <div id="observacoes">
                                ...
                                if ($row['reintegracao']) {
                                    ...
                                    //VERIFICANDO SE EXISTE RESCISÃO COM STATUS 1, POR CONTA DE UMA REINTEGRAÇÃO
                                    ...
                                    echo "<a href='recisao/nova_rescisao_2.php?enc={$link}&reitegracao=1' class='visualizar_rescisao'>Termo Rescisório</a>";
                                }
                                ?>
                            </div>
                        </td>
...
<?php } ?>
                            ...
                            if ($ACOES->verifica_permissoes(17) && $ativo) {
                                    ...
                                    if ($ACOES->verifica_permissoes(20)) {
                                        ...
                                        <a href="../rh_novaintra/cracha_provisorio_imprimir.php?id_clt=<?= $row['0'] ?>" target="_blank" class="botao" style="font-size:12px;">Cracha Provisório</a>
                                        <?php
                                    }
                                    if ($ACOES->verifica_permissoes(20)) {
                                        ?>  
                                        <a href="../relatorios/gerar_relatorio.php?documento=contrato_experiencia&clt=<?= $row['0'] ?>" target="_blank" class="botao" style="font-size:12px;">Contrato de Experiência</a>
                                    ...
                                    if ($ACOES->verifica_permissoes(80) && $ativo) {
                                        ?>  
                                        <?php if ($_COOKIE['logado'] != 395) { ?>
                                        <a href="rh_transferencia.php?clt=<?= $row['0'] ?>" target="_blank" class="botao" style="font-size:12px;">Transferências</a>
                                        <?php
                                    } }
        ...
        <?php // if(!in_array($row['status'],array('60','61','62','63','64','65','66','81','101'))) {      ?>
                                    ...
                                    <?php if ($row['status'] == 10) { ?>
                                                ...
                                                <a href="docs/demissao.php?clt=<?= $row['0'] ?>&tab=bolsista<?= $id_pro ?>&pro=<?= $id_pro ?>&id_reg=<?= $id_reg ?>" target="_blank" class="botao indResPosMat">Demissão</a-->
                                        <?php if ($_COOKIE['logado'] != 395) { ?>
                                            <a href="docs/rescisao_configuracao.php?clt=<?= $row['0'] ?>" target="_blank" class="botao indResPosMat">Rescisão</a>
        ...
        <?php } ?>  
                                    ...
                                    if ($ACOES->verifica_permissoes(94)) {
                                        ?>  
                                    <?php if ($_COOKIE['logado'] != 395) { ?>
                                    <p><a href="estabilidade_provisoria/?pro=<?= $id_pro ?>&id_reg=<?= $id_reg ?>&id_clt=<?= $row['0'] ?>" target="_blank" class="botao" style="font-size:12px;">Estabilidade Provisória</a>
                                    <?php } ?>
                                        ...
                                        <a href="../relatorios/gerar_relatorio.php?documento=declaracao_dependentes&clt=<?= $row['0'] ?>" target="_blank" class="botao" style="font-size:10px;">Declaração de Dependentes</a></p>
        <?php
    }
    ?>

                                <p><a href="declaracao_jornada_semanal.php?pro=<?= $id_pro ?>&reg=<?= $id_reg ?>&clt=<?= $row['0'] ?>" target="_blank" class="botao" style="font-size:10px;">Declaração de Jornada Semanal</a>
                                <!--<a href="compensacao_horas_trabalho.php?pro=<?= $id_pro ?>&reg=<?= $id_reg ?>&clt=<?= $row['0'] ?>" target="_blank" class="botao" style="font-size:10px;">Compensação Horas de Trabalho</a>-->
                                    <a href="acordo_compensacao.php?pro=<?= $id_pro ?>&reg=<?= $id_reg ?>&clt=<?= $row['0'] ?>" target="_blank" class="botao" style="font-size:12px;">Acordo de Compensação</a>
...
<?php if ($ACOES->verifica_permissoes(90) && ($row['status'] >= 60 && $row['status'] != 200)) { ?>
                            ...
                            <?php if ($ACOES->verifica_permissoes(117) && ($row['status'] < 60 && $row['status'] != 200) && $_COOKIE['logado'] != 395) { ?>
                                <a href="alter_curso_clt.php?id_clt=<?= $row['0'] ?>" target="_blank" class="botao">EDITAR FUNÇÃO</a>
                                <a href="alter_curso_clt.php?id_clt=<?= $row['0'] ?>" target="_blank" class="botao" style="font-size:10px;">Ficha Estabelecimento Saúde</a>
                            <?php } ?>
                            <?php if ($ACOES->verifica_permissoes(90) && ($row['status'] >= 60 && $row['status'] != 200)) { ?>
                                <?php if ($_COOKIE['logado'] != 395) { ?>
                                <a href="reintegracaoclt.php?projeto=<?= $id_pro ?>&regiao=<?= $id_reg ?>&id=<?= $row['0'] ?>" target="_blank" class="botao">Reintegração</a>
                                <?php } ?>
                            <?php } ?>
                            <?php if ($ACOES->verifica_permissoes(117)) { ?>
                                <a href="retencao_inss.php?projeto=<?= $id_pro ?>&regiao=<?= $id_reg ?>&id=<?= $row['0'] ?>" target="_blank" class="botao">Retenção INSS</a>
                            <?php } ?>
                            <?php if ($ACOES->verifica_permissoes(117)){ ?>
                                <a href="ficha_anotacoes.php?projeto=<?= $id_pro ?>&regiao=<?= $id_reg ?>&id=<?= $row['0'] ?>" target="_blank" class="botao">Ficha de Anotações</a>
                            <?php } ?>
                                <a target="_blank" class="botao" href="domicilioProfissional.php?id=<?=$row['0']?>">Domicílio Profissional</a>
                        </td>
                    </tr>
...
if ($ACOES->verifica_permissoes(14)) {
    ...
    <?php foreach ($listaFeria['registros'] as $row_ferias) { ?>
                                ...
                                <!--                    <a href="/intranet/rh_novaintra/ferias/arquivos/ferias_<?= $row_ferias['id_clt'] ?>_<?= $row_ferias['id_ferias'] ?>.pdf" class="btn btn-default btn-xs" title="Ver PDF de Férias Novas" target="_blank">
                                                    img src="../../imagens/icons/att-pdf.png" style="width: 1.5em; height: 1.5em;" alt="Ver PDF"
        ...
        <?php if ($ACOES->verifica_permissoes(87)) { ?>
                                                <td class="text-center">
                                                    <!--<button type="button" class="btn btn-danger btn-xs desprocessar_ferias" data-ferias="<?= $row_ferias['id_ferias'] ?>" data-ferias="<?= $row_ferias['clt'] ?>" data-toggle="tooltip" title="Desprocessar Férias"><i class="fa fa-trash-o"></i></button>-->
                                                    <a href="../../rh/ferias/rh_ferias_desprocessar.php?clt=<?php echo $row_ferias['clt']; ?>&ferias=<?php echo $row_ferias['id_ferias']; ?>&tela=1" class="btn btn-danger btn-xs"  data-toggle="tooltip" title="Desprocessar Férias"><i class="fa fa-trash-o"></i></a>
