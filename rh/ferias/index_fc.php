...
// Tela 1 (Seleção de participante)
            switch ($tela) {
                case 1:
                    ...
                    <a href="ferias_em_lote.php" target="_blank" class="ferias_col">Férias Coletivas</a>
                                ...
                                while ($row_clt10 = mysql_fetch_array($REClts)) {
                                    ...
                                    ?>">
                                        ...
                                        <td>
                                            ...
                                            $linkFerias = 1;
                                            $licencaFerias = null;
                                            if ($row_clt10['status'] == '200') {$linkFerias = 0;
                                                $licencaFerias .= ' <span style="color:red; font-weight:bold;">(Aguardando Demissão)</span>';
                                            }
                                            //if($_COOKIE[logado] == 257){
                                                //echo "SELECT * FROM rh_eventos WHERE data <= NOW() AND data_retorno >= NOW() AND cod_status NOT IN(10) AND status = 1 AND id_clt = {$row_clt10[0]}";
                                                $sqlLicenca = mysql_query("SELECT * FROM rh_eventos WHERE data <= NOW() AND (data_retorno >= NOW() OR data_retorno = '0000-00-00') AND cod_status NOT IN(10,40) AND status = 1 AND id_clt = {$row_clt10[0]}");
                                                while($rowLicenca = mysql_fetch_assoc($sqlLicenca)){$linkFerias = 0;
                                                    $licencaFerias .= " <span style='color:green; font-weight:bold;'>({$rowLicenca['nome_status']})</span>";
                                                }
                                            //}
                                            if ($row_clt10['status'] == '40') {
                                                $licencaFerias .= ' <span style="color:#069; font-weight:bold;">(Em Férias)</span>';
                                                $linkFerias = 1;
                                            }
                                            //if($linkFerias){
                                                echo "<a href='index.php?enc=$link2'>{$row_clt10['nome']}</a>";
                                            /*}else{
                                                echo $row_clt10['nome'];
                                            }*/
                                            echo $licencaFerias;
...
// Tela 2 (Movimentos e Histórico de Férias)
                ...
                case 2:
                    ...
                    $periodos_disponiveis = $objCalcFerias->getPeriodoAquisitivo($data_entrada,$periodos_gozados);
                    $dadosFerias = $objCalcFerias->getFeriasPorClt();
                    ?>
                    <table cellpadding='8' cellspacing='8' align='center' style="border:1px solid #ddd; border-radius:10px; -moz-border-radius:10px; background-color:#f5f5f5;">
                        ...
                        <tr>   <td align="right"><?php include('../../reportar_erro.php'); ?></td>     </tr>
                        <tr>
                            <td>
                                <div id="tela2">
                                    ...
                                    <?php
                                    } // Fim das condições de subtelas                               

                                    if ($periodos_gozados  > 0) {
                                        ?>
                                        <div id="historico" style="border:1px solid #ddd; border-radius:10px; -moz-border-radius:10px; background-color:#eee; display:none; padding:10px;">
                                            <?php
                                             foreach($dadosFerias['registros'] as $historico){                                           
                                                $margem++;
                                                $id_ferias = $historico['id_ferias'];                                                 
                                                $titulo    =  '(' . $id_ferias . ') ' . $meses[$historico['mes']] . ' / ' . $historico['ano'];
                                                ...
                                                if ($margem != $dadosFerias['total_registro']) {
                                                ...
                                                ?>">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><?=  $historico['data_aquisitivo_iniBR']; ?> <i>à</i> <?= $historico['data_aquisitivo_fimBR']; ?></td>
                                                    </tr>
                                                    <tr>
                                                        <td width="110"><b>Período de Férias:</b></td>
                                                        <td><?= $historico['data_iniBR']; ?> <i>à</i> <?= $historico['data_fimBR'] ?></td>
                                                    </tr>
                                                </table>
                                            <?php } ?>
                                        </div>
                                ...
                                <div id="tela2">
                                    ...
                                    <?php
                                         ...
                                         <label style="font-weight:normal; margin-bottom:5px;">
                                           <input type="radio" name="periodo_aquisitivo" id="periodo_aquisitivo" value="<?=$periodo['inicio'].'/' .$periodo['fim']?>"
                                                   <?php
                                                   // Caso venha da Tela 4 vem selecionado o período aquisitivo
                                                   if ($_GET['periodo_aquisitivo'] == $periodo_aquisitivo) {echo " checked";}
                                                   ?>
                                                   />
                                                   <?php echo converteData($periodo['inicio'], 'd/m/Y'). ' - ' .converteData($periodo['fim'], 'd/m/Y'); ?>
                                        </label>
                                        <br />
                                    <?php 
                                        }
                                     ...
                                     /**   for ($a = 0; $a < $quantidade_anos; $a++) {
                                            ...
                                            <label style="font-weight:normal; margin-bottom:5px;">
                                                <input type="radio" name="periodo_aquisitivo" id="periodo_aquisitivo" value="<?= $periodo_aquisitivo ?>"
                                                       ...
                                                       <?php
                                                       // Caso venha da Tela 4 vem selecionado o período aquisitivo
                                                       if ($_GET['periodo_aquisitivo'] == $periodo_aquisitivo) {
                                                           echo " checked";
                                                       }
                                                       ?>
                                                       /> 
                                                           <?php echo implode('/', array_reverse(explode('-', $aquisitivo_inicio))) . ' - ' . date('d/m/Y', mktime('0', '0', '0', $mes_data_entrada, $dia_data_entrada - 1, $ano_data_entrada + $a + 1)); ?>
                                            </label>
                                            <br />
                                        <?php } //}   **/ ?>
                                        <p>&nbsp;</p>
                                        <?php
                                            if(in_array($_COOKIE['logado'], $func_f71)){
                                                echo "<label>Digite o (Salário Variável) que se encontra no recíbo de férias</label><br />";
                                                echo "<input type='text' name='sal_variavel' id='sal_variavel' value='' />";
                                            }
                                        ?>                                        
...
// Verificando o Período Aquisitivo
                    ...
                    $periodos_gozados     = $objCalcFerias->getPeriodosGozados();
                    $periodos_disponiveis = $objCalcFerias->getPeriodoAquisitivo($data_entrada,$periodos_gozados);
                    $periodo_concessivo   = $objCalcFerias->getPeriodoConcessivo($aquisitivo_end);
                    ...
                    $faltas = $objCalcFerias->getFaltasNoPeriodo($aquisitivo_ini, $aquisitivo_end);
                    echo '<pre>';
                        // print_r($faltas);
                    echo '</pre>';
                            ...
                            <td>
                                <div id="tela2">
                                    ...
                                    <form action="index.php?enc=<?= $link ?>" method="post" onSubmit="return verifica_tudo()">
                                        ...
                                        <?php 
                                        // verifica se tirou férias coletivas
                                        ...
                                        <?php
                                        // Considerarando Período de Faltas (Trabalhando Normalmente)
                                        if (!isset($_GET['despreza_faltas'])) {
                                            ?>
                                            ...
                                            <select name="quantidade_dias" >
                                                <?php
                                                // Pré-selecionado quando volta da tela 5
                                                ...
                                                for ($a = 1; $a <= $qnt_dias; $a++) {
                                                    ...
                                                    // Exibindo os Options
                                                    echo "<option value='" . sprintf('%02d', $a) . "'$selected$script>".$a - $qtdDias_fc."</option>";
                                            ...
                                            <?php
                                            // Se Desconsiderar Período de Faltas
                                        } else {
                                            ?>
                                            ...
                                            <?php
                                            // Terminando Seleção de Quantidade de Dias
                                        }
                                        ?>
                                        ...
                                        <br /><br />
                                        <?php if(in_array($_COOKIE['logado'], $func_f71)){
                                            echo "<input type='hidden' name='sal_variavel' id='sal_variavel' value='{$_REQUEST['sal_variavel']}' />";
                                        } ?>
                                        <input type="submit" value="Prosseguir" class="botao" style="margin:10px auto;"/>
...
//
                    ...
                    if($_COOKIE['logado'] == 87){
                       ...
                       $teste_media =  $objCalcFolha->getMediaMovimentos($clt, $mesE, $anoE, 12);
                        echo '<pre>';
                          // print_r($teste_media);
                        echo '</pre>';
                    }
...
// Base para INSS / IRRF / FGTS                 
                    $calc_inss_irrf_fgts = $remuneracao_calc;
// Verificando Faltas
...
// Buscando Pensão Alimenticia
                    ...
                    $qr_pensao1 = mysql_query("SELECT * FROM rh_movimentos_clt WHERE id_clt = '$clt' AND status = '1' AND status_ferias = '1' AND $limite_pensao1 AND ano_mov = ".date('Y')." AND id_mov IN('54','63', 223) ORDER BY id_movimento DESC");
                    $row_pensao1 = mysql_fetch_array($qr_pensao1);
                    $numero_pensao1 = mysql_num_rows($qr_pensao1);

                    $qr_pensao2 = mysql_query("SELECT * FROM rh_movimentos_clt WHERE id_clt = '$clt' AND status = '1' AND status_ferias = '1' AND $limite_pensao2 AND ano_mov = ".date('Y')." AND id_mov IN('54','63', 223) ORDER BY id_movimento DESC");
...
// Calculando Variáveis
                    $remuneracao_base = number_format($salario + $salario_variavel, 2, ".", "");
                                ...
                                <form action="<?= $_SERVER['PHP_SELF'] ?>" method="post" name="form1" id="form1">
                                    ...
                                    <table width="60%" cellspacing="0" cellpadding="2" style="border:solid 1px #ccc; line-height:24px;">
                                        ...
                                        <tr>
                                            ...
                                            <td width="23%">
                                                R$ <?= $salario_variavelT ?>  
                                                <a href="action.confere_movimentos.php?id_clt=<?php echo $clt; ?>&regiao=<?php echo $regiao; ?>&dt_referencia=<?php echo $dt_referencia;?>" onClick="return hs.htmlExpand(this, {objectType: 'iframe'})">ver</a>            
                                    ...
                                    <input type="hidden" name="data_inicio" value="<?= ConverteData($data_inicio) ?>"/>
...
('$id_clt','$nome','$regiao','$projeto','$mes','$ano','$data_aquisitivo_ini','$data_aquisitivo_fim','$data_inicio','$data_fim','$data_retorno','$salario','$salario_variavel','$remuneracao_base','$dias_ferias','$valor_dias_ferias','$valor_total_ferias','$umterco','$total_remuneracoes','$pensao_alimenticia','$inss','$inss_porcentagem','$ir','$fgts','$total_descontos','$total_liquido','$abono_pecuniario','$umterco_abono_pecuniario','$dias_abono_pecuniario','$faltas','$faltasano','$diasmes','$ferias_dobradas','$valor_total_ferias1','$valor_total_ferias2','$acrescimo_constitucional1','$acrescimo_constitucional2','$total_remuneracoes1','$total_remuneracoes2','$update_movimentos_clt','$user',NOW(),'$status', '$BASE_INSS','$BASE_IRRF', '$PERCENTUAL_IRRF', '$VALOR_DDIR', '$QNT_DEPENDENTES_IRRF', '$PARCELA_DEDUCAO_IRRF')") or die(mysql_error());
                    ...
                    $insert_ferias_proc = "INSERT INTO rh_ferias_log (id_ferias,id_usuario,data_mod, status_log_ferias, id_clt) VALUES ('$id_ferias','{$usuario['id_funcionario']}',NOW(), 1,$id_clt)";
                    mysql_query($insert_ferias_proc) or die(mysql_error('Erro no log'));

                    mysql_query("UPDATE rh_clt SET status='40' WHERE id_clt = '$id_clt'");

//                    mysql_query("INSERT INTO rh_eventos (id_clt, id_regiao, id_projeto, nome_status, cod_status, id_status, data, data_retorno, dias, obs, status, status_reg) VALUES ('$id_clt', '$regiao', '$projeto', 'Férias', '40', '1', '$data_inicio', '$data_retorno', '$dias_ferias', NULL, '1', '1')");
