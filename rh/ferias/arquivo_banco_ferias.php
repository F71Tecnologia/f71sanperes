...
if(isset($_GET['delArq'])){
    ...
    header("Location: arquivo_banco_ferias.php");exit;
}else if(isset($_GET['arq'])){
    $download = $ArquivoTxtBancoClass->downloadArquivo($_GET['arq']);
}

$arrayArquivos = $ArquivoTxtBancoClass->getRegistrosFerias($usuario['id_regiao']);
        ...
        <div id="content">
            <form  name="form" action="" id="form1" method="post" id="form">
                <div id="head">
                    ...
                    <div class="fleft">
                        <h2>Arquivos de Banco para Férias</h2>
                    </div>
                </div>
                <br class="clear">
                <br/>
                ...
                if(is_array($arrayArquivos)){
                    foreach ($arrayArquivos as $key => $value) {
                        ...
                        if($auxNomeArquivo != $value['nome_arquivo']){
                            ...
                            <thead>
                                <tr>
                                    <th colspan='4' class='arquivo' data-id='{$count}'>{$nomeArquivo[2]}</th>
                                    <th><a href='?arq={$key}'><img class='imgDown' src='../../img_menu_principal/relatorios_gestao.png'> Baixar</a></th>
