...
if (empty($_COOKIE['logado'])) {
    ...
    if(is_array($array)){
        for($i=0; $i<=count($array); $i++){
            if($i==0){
                $qr_alt_salario_or[] = "id_curso = {$array[$i]['id_curso_de']} AND data < '{$array[$i]['data_proc']}' AND data > '{$row['data']}' AND status = 1";
        ...
        $qr_alt_salario[] = "(SELECT 'A' ordem, salario_antigo, salario_novo, data FROM rh_salario WHERE  id_curso = {$row['id_curso']} AND data > '{$row['data']}' AND status = 1 $auxDemi GROUP BY DATE_FORMAT(data,'%Y-%m'))";
    }
