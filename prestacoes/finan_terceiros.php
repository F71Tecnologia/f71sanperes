...
//178 => MILTON, 208 => SUELLEN, 158 => RAMON
//71 => DANIELA, 137 => GEYSSA, 158 => RAMON
//277 => ANGÉLICA OLINTO
$funcionario_contabilidade = array(71, 158); //array(178, 208, 158);
$funcionario_financeiro = array(277, 158); //array(71,137, 158)
...
if (isset($_REQUEST['projeto'])) {
    ...
    if ((isset($_REQUEST['exportar']) && !empty($_REQUEST['exportar'])) || (isset($_REQUEST['filtrar']) && !empty($_REQUEST['filtrar']))) {
        ...
        $qr_verifica = PrestacaoContas::getQueryVerifica("terceiro", $dataMesRef, $dataMesIni, $usuarioW['id_master']);
    ...
    <body id="page-despesas" class="novaintra">
        <div id="content">
            <form action="" method="post" name="form1" id="form1">
                ...
                <h2>CONTRATOS DE SERVIÇOS TERCEIRIZADOS</h2>
                ...
                <?php if (!empty($result) && mysql_num_rows($result) > 0) { ?>
                    <br/>
                                            <p style="text-align: right; margin-top: 20px"><input type="button" onclick="tableToExcel('tbRelatorio', 'Servico de Terceiros')" value="Exportar para Excel" class="exportarExcel"></p>    
                    <br/>
                    <?php 
                    if ($btfinalizar) { 
                        $queryContrRecentes = "
                        ...
                        if(mysql_num_rows($queryContrRecentes)){?>
                            <div class="avisos_eventos">
                                ...
                                <h3 class="titulo_categoria">» LISTA DE CONTRATOS</h3>
                                <?php 
                    ...
                    <table id="tbRelatorio" border="0" cellpadding="0" cellspacing="0" width="100%" class="grid">
                        <thead>
                            <tr>
                                <th colspan="8">Unidade Gerenciada: <?php echo $projeto['nome'] ?></th>
                                <th><?php echo $mesShow ?></th>
                            </tr>
                            <tr>
                                <th colspan="9">O responsável: <?php echo $roMaster['nome'] ?></th>
                            </tr>
                            <tr>
                                <th colspan="9">CONTRATOS DE SERVIÇOS TERCEIRIZADOS</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="titulo">
                                <td rowspan="2">-</td>
                                <td rowspan="2">Serviço Contratado</td>
                                <td rowspan="2">Razão Social Contratado</td>
                                <td rowspan="2">CNPJ Contratado</td>
                                <td rowspan="2">Unidade de Medida</td>
                                <td colspan="2">Vigência do Contrato</td>
                                <td colspan="2">Valor Contrato (R$)</td>
                            </tr>
                            <tr class="titulo">
                                <td>Data Início</td>
                                <td>Data Término</td>
                                <!-- <td>Mês Anterior</td> -->
                                <td>Mensal Estimado</td>
                                <td><?php echo mesesArray($_REQUEST['mes']) ?></td>
                            </tr>
                <?php
                $totaldesc = 0;
                while ($row = mysql_fetch_assoc($result)) {
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="txright">R$ <?php echo number_format($total_mensal, 2, ",", ".") ?></td>
                        <td class="txright">R$ <?php echo number_format($valor_total, 2, ",", ".") ?></td>
                ...
                <?php } else { ?>
                    <?php if ($projetoR !== null) { ?>
                                        ...
                                        <div id='message-box' class='message-green'>
                                            <p>Nenhum pagamento efetuado a terceiros nessa competência</p>
                                        </div>
                    <?php } ?>
                <?php } ?>
                <?php if ($projetoR !== null) { ?>
                                        ...
                                        <?php //if ($erros == 0) { ?>
                                            <p class="controls"> 
                                                ...
                                                <?php }else if(in_array($_COOKIE['logado'], $funcionario_financeiro)){ ?>
                                                    <?php if($linhas_verifica_validacao == 0){ ?>
                                                        <input type="submit" class="button" value="Validar Prestação" name="validar" />
                                                    <?php }else{ ?>
                                                        <div id='message-box' class='message-yellow'>
                                                            <?php while($linha = mysql_fetch_assoc($result_verifica_validacao)){ ?>
                                                                <p>Prestação validada por <?php echo $linha['nome'] ?> às <?php echo $linha['data_criacao'] ?></p>
