...
if (isset($_REQUEST['gerar']) && !empty($_REQUEST['gerar'])) {
    ...
    $qr = mysql_query("SELECT C.id_entradasaida, C.nome, A.id_saida,B.id_saida_file,B.tipo_saida_file,A.especifica,A.nome AS nomesaida 
                        FROM saida as A 
                        LEFT JOIN saida_files AS B ON(A.id_saida = B.id_saida)
                        LEFT JOIN entradaesaida AS C ON(A.tipo = C.id_entradasaida)
                        WHERE MONTH(data_vencimento) = '{$_REQUEST['mes']}' AND YEAR(data_vencimento) = '{$_REQUEST['ano']}' ORDER BY A.tipo");
    ...
    foreach ($linkDownload as $k => $down) {
        ...
        if (substr_count(current($down), ".") >= 2)
            $d = "../comprovantes/";
        if (is_file($d . current($down)))
            $zip->addFile($d . current($down), $novoNome);
        else
