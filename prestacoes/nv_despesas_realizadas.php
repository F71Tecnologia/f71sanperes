...
if(isset($_REQUEST['projeto'])){        
    ...
    if(isset($_REQUEST['filtrar']) && !empty($_REQUEST['filtrar'])){
        ...
        $where = "A.id_banco={$_REQUEST['banco']} AND A.`status` = 2 AND D.cod != \"06.03.01\" AND A.estorno IN (0,2)";
...
if (isset($_REQUEST['finalizar']) && !empty($_REQUEST['finalizar'])) {
    ...
    while ($row = mysql_fetch_assoc($result)) {
        ...
        if ($cpf == "" && $cnpj == "" || str_replace(array(".","-"),"",$cpf) == "00000000000") {
            $arrayErros[] = "Saída ".$id_saida.". Empresa/CNPJ ou Funcionário/CPF contemplado pela saída errado ({$cnpj}-{$razao}) ({$cpf}-{$nome}) ({$row['id_clt']}-{$row['id_nome']}-{$row['id_prestador']}-{$row['id_banco']})";
        }else{
    ...
    <body id="page-despesas" class="novaintra">
        ...
        <div id="content">
            <form action="" method="post" name="form1" id="form1" class="form-horizontal top-margin1">
                ...
                <ul class="breadcrumb">
                    <li><a href="../">Home</a></li>
                    <li><a href="javascript:;" data-key="36" class="return_principal">Prestação de Contas</a></li>
                    <li class="active">Despesas Realizadas</li>
                </ul>
                ...
                <fieldset>
                    ...
                    <p style="text-align: right;">
                        <button type="button" onclick="tableToExcel('tabela', 'Despesas')" class="btn btn-success exportarExcel"><span class="fa fa-file-excel-o"></span>&nbsp;&nbsp;Exportar para Excel</button>
                    </p>              
                    
                    <div class="alert alert-dismissable alert-warning">                
                        <strong>Unidade Gerenciada: </strong> <?php echo $projeto['nome']; ?>
                        <strong class="borda_titulo">O responsável: </strong> <?php echo $masterNome; ?>
                        <strong class="borda_titulo">Mês Referente: </strong> <?php echo $mesShow; ?>
                    </div>
                    
                    <table id="tabela" class="grid table table-hover">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            while ($row = mysql_fetch_assoc($result)) {
                                ...
                                if($antesProjeto != $row['id_projeto']){
                                    ...
                                    echo "<tr class=\"active\"><td colspan='100%' style='text-align: center;'>Projeto: {$projetos[$row['id_projeto']]}</td></tr>";
                                }
                                
                                if ($antesGrupo != $row['id_grupo']) {
                                    $antesGrupo = $row['id_grupo'];
                                    echo "<tr class=\"active\"><td>0" . str_replace("0", "", $row['id_grupo']) . "</td><td>" . $row['nome_grupo'] . "</td><td class='txright'>" . number_format($totais[$row['id_grupo']], 2, ",", ".") . "</td></tr>";
                                }
                                if ($antesSubGrupo != $row['id_subgrupo']) {
                                    $antesSubGrupo = $row['id_subgrupo'];
                                    echo "<tr class=\"active\"><td><span class='artificio1'></span>" . $row['id_subgrupo'] . "</td><td>" . $row['subgrupo'] . "</td><td class='txright'>" . number_format($subtotais[$row['idsub']], 2, ",", ".") . "</td></tr>";
                                }
                        ...
                        <tfoot>
                            <tr class="info">
                                <td></td>
                                <td></td>
                                <td><strong>Total: </strong><?php echo number_format($row_totalfinal['total'], 2, ",", "."); ?></td>                                
                            </tr>                            
                        </tfoot>
                    </table>
                    ...
                    <?php if ($projetoR!==null) { ?>
                        <?php if ($btexportar) { ?>
                            ...
                            <div class="checkbox pull-right">
                                <label>
                                  <input type="checkbox" name="zip" id="zip" value="1"> Gerar arquivo ZIP
                                  <!--<input type="submit" class="button btn btn-primary" value="Exportar" name="exportar" />-->
                                  <button type="button" class="button btn btn-primary" name="exportar"><span class="fa fa-share-square-o"></span>&nbsp;&nbsp;Exportar</button>
                                </label>
                            </div>
                        ...
                        <?php if ($btfinalizar) { ?>
                            ...
                            <p class="controls pull-right"> 
                                <button type="button" class="button btn btn-warning" name="finalizar"><span class="fa fa-power-off"></span>&nbsp;&nbsp;Finalizar Prestação</button>
                            </p>
                            <div class="clear"></div>
                            <?php } else { ?>                            
