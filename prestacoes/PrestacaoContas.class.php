...
class PrestacaoContas {
    ...
    public function carregaBancos($projeto) {
        $qr_bancos = mysql_query("SELECT * FROM bancos WHERE id_projeto = '{$projeto}' AND status_reg=1");
     ...
     * 
     * @param type $tipo
     * @param type $dtReferencia
     * @param type $dtInicio
     */
    public static function getQueryVerifica($tipo,$dtReferencia,$dtInicio, $master = null ){
        ...
        $auxMaster = (!empty($master)) ? " AND A.id_master = $master " : null ;
        $qr = "SELECT B.id_prestacao,A.id_projeto,A.nome as projeto,DATE_FORMAT(B.gerado_em, '%d/%m/%Y') as gerado_embr, C.nome as funcionario, D.id_banco, D.agencia, D.conta, A.administracao
                        ...
                        WHERE A.inicio < '{$dtInicio}' AND A.prestacontas = 1 {$whereAdm} $auxMaster";
        return $qr;
    }
}