...
if (isset($_REQUEST['projeto'])) {
    ...
    if ($historico === false) {
        $qr = "SELECT A.id_autonomo, A.id_projeto, A.nome, A.cpf, A.conselho, B.id_rpa, B.valor, B.valor_inss, 
            ...
            DATE_FORMAT(G.data_pg, '%d/%m/%Y') as data_pgBR, B.id_rpa,G.id_saida
            FROM autonomo AS A
            ...
            LEFT JOIN (SELECT I.id_rpa,J.id_saida,J.data_pg FROM rpa_saida_assoc AS I
                        LEFT JOIN saida AS J ON (I.id_saida = J.id_saida)
                        WHERE J.`status` != 0) AS G ON (G.id_rpa = B.id_rpa)
            WHERE A.id_projeto = {$id_projeto} AND B.id_rpa IS NOT NULL AND B.mes_competencia = {$_REQUEST['mes']} AND B.ano_competencia = {$_REQUEST['ano']}
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="page-header box-contas-header">
                        <h2><span class="glyphicon glyphicon-book"></span> - PRESTAÇÃO DE CONTAS <small>- RH CONTRATADO POR RPA - Regime de Caixa</small></h2>
                    </div>
                </div>
            </div>
                ...
                <form action="" method="post" name="form1" id="form1" class="form form-horizontal">
                    ...
                    <table class="table table-striped table-condensed" id="tbRelatorio">
                        <thead class="text text-sm">
                            <tr>
                                <th colspan="11" class="text-center">Responsável: <?php echo $roMaster['nome'] ?></th>
                                <th class="text text-right"><?php echo $mesShow ?></th>
                            </tr>
                            <tr>
                                <th colspan="11" class="text-center">Unidade Gerenciada: <?php echo $projeto['nomeunidade'] ?></th>
                                <th></th>
                            </tr>
                            <tr>
                                <th colspan="11" class="text-center">Código Unidade Gerenciada: <?= $projeto['cnes']?></th>
                                <th></th>
                            </tr>
                            <tr>
                                <th colspan="11" class="text-center">RH CONTRATADO POR RPA - REGIME DE CAIXA</th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td colspan="12" class="info"></td>
                            </tr>
                        </tbody>
                        <tbody class="text text-uppercase">
                            <tr class="text-center fontreduz">
                                <td width="20%" >Nome</td>
                                <td width="10%">CPF</td>
                                <td width="5%">Nº do Conselho Profissional</td>
                                <td width="3%">Nº Recibo RPA</td>
                                <td width="14%">Categoria Profissional</td>
                                <td width="20%">Cargo</td>
                                <td width="6%">CBO</td>
                                <td width="2%">Qtd</td>
                                <td width="4%">Forma de Contratação</td>
                                <td width="4%">Carga Horaria Mensal</td>
                                <td width="6%">Data de Pagamento</td>
                                <td width="6%">Valor Líquido</td>
                            </tr>
                            <?php while ($row = mysql_fetch_assoc($result)) { $cpf = preg_replace('/[^[:digit:]]/', '', $row['cpf']); ?>   
                            <tr class="text-sm">
                                    <td><?php echo $row['nome']; ?></td>
                                    <td class="text-center"><?php echo preg_replace('/(\d{3})(\d{3})(\d{3})(\d{2})/', '$1.$2.$3-$4', $cpf); ?></td>
                                    <td class="text-center"><?php echo str_replace('.','',str_replace('-','',$row['numconselho'])); ?></td>
                                    <td class="text-center"><?php echo $row['id_rpa']; ?></td>
                                    <td class="text-center"><?php echo $row['nomeCbo']; ?></td>
                                    <td class="text-center"><?php echo $row['funcao']; ?></td>
                                    <td class="text-center"><?php echo $row['cod']; ?></td>
                                    <td class="text-center"><?php echo "1"; ?></td>
                                    <td class="text-center">RPA</td>
                                    <td class="text-center"><?php echo $row['hora_mensal_rpa']; ?></td>
                                    <td class="text-center"><?php echo $row['data_pgBR'];?></td>
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="10" class="text-right">Total de participantes:</td>
