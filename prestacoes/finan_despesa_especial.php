...
//ARRAY DE SAIDAS QUE N√O SERA ENVIADO O COMPROVANTE
...
/*-- IMPORTANTE -- ADICIONADO EM 24/11/2015 -- TIPOS DE SAÕDAS QUE S” TEM ANEXO, SEM COMPROVANTES --*/
$arrSemComprovantes = array(154,243,249,245,244); //154=REMESSA PGTO, 243 = TARIFA, 249 = RATEIO, 245 = TRANSFERENCIA, 244 = PGTO EMPRESTIMO
        ...
        if ($row['comprovante'] == 2) {
            ...
            if ($row['Atipo'] == '170') {
                ...
                if(in_array($id_saida, $saida_rpas)){
                    ...
                    $query_anexo = mysql_query("SELECT * FROM saida_files WHERE id_saida = '$id_saida'");
                    //echo "D :" . "SELECT * FROM saida_files WHERE id_saida = '$id_saida' " . "<br>";
                }
                ...
                } else if (mysql_num_rows($query_anexo) == 1) {
            ...
            $arrayArquivos[$id_saida] = " sem anexo";
            $arrayErros[] = $id_saida." sem anexo";
        }
        
        //VERIFICANDO SE O TIPO DE SAÕDA … OBRIGAT”RIO TER COMPROVANTE
        if(!in_array($row['Atipo'], $arrSemComprovantes)){
            //VERIFICANDO OS COMPROVANTES
            $qr_comp = mysql_query("SELECT * FROM saida_files_pg A INNER JOIN saida B ON (B.id_saida = A.id_saida) WHERE A.id_saida = {$id_saida}");
        ...
        <div id="content">
            <form action="" method="post" name="form1" id="form1">
                ...
                <fieldset>
                    ...
                    <p><label class="first">Projeto:</label> <?php echo montaSelect(array("-1" => "´ Selecione a Data ª"), null, "id='prestacao' name='prestacao' class='validate[custom[select]]'") ?></p>
                    <p class="controls">
