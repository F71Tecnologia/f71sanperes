...
class AssocProvisaoClass { 
    ...
    protected $QUERY;
    protected $SELECT = ' * ';
    protected $FROM = ' contabil_contas_assoc_provisao ';
    protected $WHERE;
    protected $GROUP;
    protected $HAVING;
    protected $ORDER;
    ...
    protected function limpaQuery() {
        $this->setQUERY('');
        $this->setSELECT(' * ');
        $this->setFROM(' contabil_contas_assoc_provisao ');
        $this->setWHERE('');
        $this->setGROUP('');
        $this->setHAVING('');
        $this->setORDER('');
    ...
    public function update() {
        ...
        $this->setQUERY("UPDATE contabil_contas_assoc_provisao SET " . implode(", ", ($camposUpdate)) . " WHERE id_provisao = {$this->getIdProvisao()} LIMIT 1;");
    ...
    public function insert() {
        ...
        $this->setQUERY("INSERT INTO contabil_contas_assoc_provisao ($keys) VALUES ('$values');");
        if ($this->setRs()) {
    ...
    public function inativa() {
        ...
        $this->setQUERY("UPDATE contabil_contas_assoc_provisao SET status = 0 WHERE id_provisao = {$this->getIdProvisao()} LIMIT 1;");
        if ($this->setRs()) {
            return 1;
        } else {
            die(mysql_error());
    ...
    public function deleta() {
        ...
        $this->setQUERY("DELETE FROM contabil_contas_assoc_provisao WHERE id_provisao = {$this->getIdProvisao()} LIMIT 1;");
        if ($this->setRs()) {
            return 1;
        } else {
            die(mysql_error());
    ...
    public function consulta(){
        ...
        $this->setWHERE("id_conta = '{$this->getIdConta()}' AND id_contabil_1 = '{$this->getIdContabil1()}' AND id_contabil_2 = '{$this->getIdContabil2()}' AND id_projeto = '{$this->getIdProjeto()}'");
        if ($this->setRs()) {
            return 1;
        } else {
            die(mysql_error());
