...
if($_REQUEST['method'] == 'carregaFornecedor') {
    $sqlPrestadorSemAssociassao = "SELECT id_prestador, c_razao 
                                FROM prestadorservico A
                                WHERE A.prestador_tipo = 4 AND (A.encerrado_em > CURDATE() OR A.encerrado_em IS NULL) AND A.id_projeto = {$_REQUEST['projeto']}
                                ORDER BY c_razao";
    ...
    while($rowPrestadorSemAssociassao = mysql_fetch_assoc($qryPrestadorSemAssociassao)){
        ...
        echo "<option value='{$rowPrestadorSemAssociassao['id_prestador']}' $selected >".utf8_encode($rowPrestadorSemAssociassao['c_razao'])."</option>";
    }
    exit;
}
...
if(isset($_REQUEST['prestador']) && isset($_REQUEST['filtrar']) && $_REQUEST['projeto'] >  0) { 
    ...
    $qryPlanos = "SELECT * FROM contabil_planodecontas WHERE id_projeto = '{$_REQUEST['projeto']}' ORDER BY classificador";
    $qryPlanos = mysql_query($qryPlanos) or die('Erro ao consultar Plano de Contas: ' . mysql_error());
    $arrayPlanosPassivo[] = $arrayPlanosDre[] = 'selecione';
    while ($rowPlanos = mysql_fetch_assoc($qryPlanos)) {
        $n = explode('.', $rowPlanos['classificador']);
        if($n[0] >= 1)
            $arrayPlanosPassivo[$rowPlanos['id_conta']] = $rowPlanos['classificador'] . ' - ' . $rowPlanos['descricao'];
        else if($n[0] >= 1)
            $arrayPlanosDre[$rowPlanos['id_conta']] = $rowPlanos['classificador'] . ' - ' . $rowPlanos['descricao'];
    }
//    print_array($arrayPlanos);
}
                        ...
                        <div class="panel panel-default">
                            <div class="panel-body">
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4"><?= montaSelect(getProjetos($usuario['id_regiao']), $projeto, "id='projeto' name='projeto' data-for='prestador' class='form-control input-sm validate[required,custom[select]]'") ?></div>
                                    <label for="" class="col-sm-1 text-sm control-label">Prestador</label>
                                    <div class="col-sm-4">
                        ...
                        <table id="tbRelatorio" class="table table-condensed table-hover text-sm valign-middle">
                            <thead>
                                <tr>
                                    ...
                                    <td class="text-left text-bold">DÉBITO</td>
                                    <td class="text-left text-bold">CRÉDITO</td>
                                </tr>
                                <tr>
                                    <td class="text-left text-bold">Conta Prestador</td>
                                    <td class="text-left text-bold"><?= montaSelect($arrayPlanosDre, $contaPrestador['id_conta_dre'], "id='conta' name='imposto[0][passivo]' class='input-sm validate[required,custom[select]] form-control'"); ?></td>
                                    <td class="text-left text-bold"><?= montaSelect($arrayPlanosPassivo, $contaPrestador['id_conta_passivo'], "id='conta' name='imposto[0][dre]' class='input-sm validate[required,custom[select]] form-control'"); ?></td>
                                </tr>
                                <tr>
                                    <td colspan="3" class="text-center text-bold">Impostos</td>
                                </tr>
                            </thead>
                            <tbody>
                                <?php while($rowImpostos = mysql_fetch_assoc($qryImpostos)){ ?>
                                <tr>
                                    <td class=""><?= $rowImpostos['sigla'] ?></td>
                                    <td class=""><?= montaSelect($arrayPlanosDre, $rowImpostos['id_conta_dre'], "id='conta' name='imposto[{$rowImpostos['id_imposto']}][passivo]' class='input-sm validate[required,custom[select]] form-control'"); ?></td>
                                    <td class=""><?= montaSelect($arrayPlanosPassivo, $rowImpostos['id_conta_passivo'], "id='conta' name='imposto[{$rowImpostos['id_imposto']}][dre]' class='input-sm validate[required,custom[select]] form-control'"); ?></td>
                                </tr>
                                <?php } ?>
                            </tbody>
                            <tfoot>
                                <tr>
                                    <td colspan="3" class="text-right">
                                        ...
                                        <button class="btn btn-primary" name="associar"><i class="fa fa-link"></i> Associar</button>
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
