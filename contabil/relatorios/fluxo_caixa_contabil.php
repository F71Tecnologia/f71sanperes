...
if(isset($_REQUEST['data_ini']) && isset($_REQUEST['data_fim']) && isset($_REQUEST['projeto']) && isset($_REQUEST['filtrar']) && $_REQUEST['projeto'] >  0) {
    ...
    $sqlSaldo = "SELECT
        SUM(CASE WHEN A.tipo = 1 THEN A.valor ELSE A.valor * -1 END) AS saldo
        FROM contabil_lancamento_itens A
        INNER JOIN contabil_lancamento B ON(B.id_lancamento = A.id_lancamento AND B.`status` != 0)
        INNER JOIN contabil_planodecontas C ON(C.id_conta = A.id_conta AND C.id_projeto = '{$projeto}')
        WHERE B.id_projeto = '{$projeto}' AND B.data_lancamento < '{$data_ini_bd}' AND A.`status` != 0 AND C.natureza = 'D'
        ORDER BY A.id_conta ASC";
    $saldo = number_format(mysql_result(mysql_query($sqlSaldo),0), 2, '.', '');
    
    $sql = "SELECT A.id_lancamento_itens, B.data_lancamento, C.descricao, A.valor, A.tipo, C.natureza
        FROM contabil_lancamento_itens A
        INNER JOIN contabil_lancamento B ON(B.id_lancamento = A.id_lancamento AND B.`status` != 0)
        INNER JOIN contabil_planodecontas C ON(C.id_conta = A.id_conta AND C.id_projeto = '{$projeto}' AND C.`status` != 0)
        WHERE B.id_projeto = '{$projeto}' AND (B.data_lancamento BETWEEN '{$data_ini_bd}' AND '{$data_fim_bd}') AND A.`status` != 0
        GROUP BY B.id_lancamento
        ORDER BY B.data_lancamento ASC";
    $qry = mysql_query($sql);
...
$nome_pagina = "Fluxo de Caixa Contábil";
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <table id="tbRelatorio" class="table table-condensed valign-middle">
                        ...
                        <tr class="active">
                            <td colspan="5" class="text-left text-slim text-uppercase">Fluxo de Caixa <?= $data_ini ?> até <?= $data_fim ?></td>
                        </tr>
                        <tr>
                        ...
                        <?php foreach ($arrayLancamentos as $key => $row_lanc) { 
                            $saldo += ($row_lanc['tipo'] == '1') ? $row_lanc['valor'] : -$row_lanc['valor']; ?>
                            <tr class="">
                                <td class="text-left"><?= implode('/', array_reverse(explode('-', $row_lanc['data_lancamento']))) ?></td>
                                <td class="text-left"><?= (!empty($row_lanc['descricao'])) ? $row_lanc['descricao'] : $row_lanc['descricao'] ?></td>
                                <td class="text-right"><?= ($row_lanc['tipo'] == '1' && $row_lanc['natureza'] == 'D') ? number_format($row_lanc['valor'], 2, ',', '.') : '0,00' ?></td>
                                <td class="text-right"><?= ($row_lanc['tipo'] == '2' && $row_lanc['natureza'] == 'D') ? number_format($row_lanc['valor'], 2, ',', '.') : '0,00' ?></td>
