...
$query = "SELECT * FROM rhempresa A WHERE A.id_projeto = '{$_REQUEST['projeto']}'";
...
if ($_REQUEST['filtrar'] == 'Imprimir') {
    ...
    class PDF extends FPDF {
        ...
        function Header() {
            ...
            $this->SetLineWidth(0);
            $this->Image("../../imagens/logomaster{$this->master['id_master']}.gif", 1, .85, 2);
            $this->Cell(3);
            $this->Cell(10, .3, $this->master['razao'], 0, 0, 'L');
            $this->Ln();
            $this->Cell(3);
            $this->Cell(10, .3, 'CNPJ ' . $this->master1['cnpj'], 0, 0, 'L');
            $this->Ln();
            $this->Cell(3);
            $this->Cell(10, .3, $this->master1['endereco'], 0, 'B', 'L');
            $this->SetFont('Arial', 'B', 8);
            $this->SetLineWidth(0);
            $this->Ln();
            $this->Cell(19, .4, $_REQUEST['data_ini'] . " a " . $_REQUEST['data_fim'], 0, 0, 'R');
            $this->SetFont('Arial', 'B', 8);
            $this->SetLineWidth(0);
            $this->cell(6, .3, $sintetico, 0, 0, 'R');
            $this->Ln();
            $this->Cell(19, .4, 'LIVRO DIÁRIO', 0, 0, 'C');
            $this->Ln();
            $this->SetFont('Arial', 'I', 6);
            $this->SetLineWidth(0);
            $this->Cell(19, .3, $this->master1['nome'], 0, 0, 'C');
            $this->Ln();
    ...
    foreach ($arrayLancamentos as $data => $row_data) {
        $pdf->SetFont('Arial', 'BIU', 7);
        $pdf->SetLineWidth(0);
        $pdf->cell(1.3, .35, $data, 0, 0);
        $pdf->Ln();
        $pdf->Ln();
        foreach ($row_data as $lanc => $row_lanc) {
            foreach ($row_lanc as $key => $row_itens_lanc) {
                    $pdf->SetFont('Arial', 'I', 4.5);
                    $pdf->SetLineWidth(0);
                    $pdf->Cell(2, .3, 'LANÇAMENTO / HISTÓRICO', 0, 'L');
                    $pdf->SetFont('Arial', '', 6);
                    $pdf->SetLineWidth(0);
                    $pdf->cell(1);
                    $pdf->cell(1, .3, $lanc, 0, 0);
                    $pdf->Cell(14, .3, $key, 0, 0);
                    $pdf->Ln();
                foreach ($row_itens_lanc as $value) {
                    $pdf->SetFont('Arial', 'I', 4.5);
                    $pdf->SetLineWidth(0);
                    $pdf->Cell(2, .3, ($value['tipo'] == 2) ? 'DEVEDORA' : 'CREDORA', 0, 'L');
                    $pdf->SetFont('Arial', '', 6);
                    $pdf->SetLineWidth(0);
                    $pdf->cell(1);
                    $pdf->Cell(12, .3, $value['classificador'] . ' - ' .$value['descricao'], 0, 'L');
                    $pdf->cell(4, .3, ($value['valor'] < 0) ? "(" . number_format($value['valor'] * -1, 2, ',', '.') . ")" : number_format($value['valor'], 2, ',', '.'), 0, 0, 'R');
                    $pdf->SetLineWidth(0);
                    $pdf->Ln();                    
                }
            }
                    ...
                    <div class="col-sm-12">
                        ...
                        <?php if (isset($_REQUEST['data_ini']) && isset($_REQUEST['data_fim']) && isset($_REQUEST['projeto']) && isset($_REQUEST['filtrar']) && $_REQUEST['projeto'] > 0) { ?>
                            ...
                            <table id="tbRelatorio" class="table table-condensed valign-middle">
                                ...
                                <tr class="active">
                                    <td colspan="4" class="text-bold text-center"><?= $master1['nome'].'<br>'.$master1['cnpj'] ?></td>
                                </tr>
                                <tr class="active">
