...
if (isset($_REQUEST['par']) && $_REQUEST['par'] == TRUE) {
    ...
    if (empty($projeto)) {
        ...
        $breadcrumb_pages = array("Lista Projetos" => "ver2.php");
        $nomePagina = "Visualizar Projeto";
    }
    
    $dadosHeader = montaCabecalhoNovo(getRegioes(), getMasters(), $usuario, __FILE__); //PREPARA VARIAVEIS PARA FUNCIONAMENTO DO CABEÇALHO (TROCA DE MASTER E DE REGIÕES)
    $breadcrumb_config = array("nivel"=>"../intranet", "key_btn"=>"3", "area"=>"Recursos Humanos", "id_form"=>"form1", "ativo"=>"$nomePagina");
            ...
            <div class="container">
                ...
                if (empty($projeto)) { ?>
                    <div class="row">
                        <div class="col-lg-12">
                            <legend>
                                <!--h4 class="up">LOCAIS CADASTRADOS (<?=$row_regiao['regiao']?>)</h4-->
                                <h4 class="up">PROJETOS</h4>
                            </legend>
                            <?php
                            ...
                            foreach ($status as $status_reg => $tipo) {
                                ...
                                if ($status_reg == 1) {
                                    $classe = "info"; $title = "ATIVO(S)";
                                } else {
                                    $classe = "danger";$title = "INATIVO(S)";
                                }
                                    ...
                                    <div class="panel panel-<?=$classe?>">
                                        ...
                                        <?php while ($row = mysql_fetch_array($qr_projetos)) { ?>
                                            <div class="col-lg-4 col-md-6 col-sm-6">
                                                <div class="info-box <?=$classe?> pointer no-padding-hr" data-proj="<?=$row['id_projeto']?>">
                    ...
                    if (empty($_REQUEST['id'])) { ?>
                        ...
                        <div id="myTabContent" class="tab-content">
                            <div class="tab-pane active projParticipante">
                                <div class="row">
                                    <div class="col-lg-6 col-xs-6 btn-group btn-group-vertical">
                                        <h4 class="text-danger">EDIÇÃO DE PARTICIPANTES</h4>
                                        <hr class="hr-danger">
                                        <div class="">
                                            <!--
                                            <div data-form="form1" data-url="bolsista2.php" data-projeto="<?=$projeto?>" class="link-sem-get btn btn-lg btn-block btn-primary margin_b5">Visualizar Participantes</div>
                                            <div data-form="form1" data-url="folha_ponto2.php" data-pro="<?=$projeto?>" data-id="1" data-caminho="1" class="link-sem-get btn btn-lg btn-block btn-default margin_b5">Gerar Apontamento</div>
                                            <?php if ($row['status_reg'] == 1) { ?>
                                            <a data-form="form1" data-url="rh/cadastroclt.php" data-regiao="<?=$regiao?>" data-projeto="<?=$projeto?>" class="link-sem-get btn btn-lg btn-block btn-warning margin_b5">Cadastrar CLT</a>
                                            <a data-form="form1" data-url="cooperativas/cadcooperado.php" data-regiao="<?= $regiao ?>" data-pro="<?= $projeto ?>" data-tipo="3" class="link-sem-get btn btn-lg btn-block btn-info margin_b5">Cadastrar Cooperado</a>
                                            <a data-form="form1" data-url="cadastro_bolsista.php" data-regiao="<?= $regiao ?>" data-pro="<?= $projeto ?>" class="link-sem-get btn btn-lg btn-block btn-success margin_b5">Cadastrar Aut&ocirc;nomo</a>
                                            <a data-form="form1" data-url="cooperativas/cadcooperado.php" data-regiao="<?= $regiao ?>" data-pro="<?= $projeto ?>" data-tipo="4" class="link-sem-get btn btn-lg btn-block btn-success margin_b5">Cadastrar Aut&ocirc;nomo / PJ</a>
                                            <a data-form="form1" data-url="terceirizado/cadterceiro.php" data-regiao="<?= $regiao ?>" data-pro="<?= $projeto ?>" data-tipo="4" class="link-sem-get btn btn-lg btn-block btn-success margin_b5">Cadastrar Terceirizado</a>
                                            <?php } ?>
                                            -->
                                            <a href="bolsista2.php?projeto=<?=$projeto?>" class="btn btn-lg btn-block btn-primary margin_b5">Visualizar Participantes</a>
                                            <a href="folha_ponto2.php?pro=<?=$projeto?>&id=1&caminho=1" class="btn btn-lg btn-block btn-default margin_b5">Gerar Apontamento</a>
                                            <?php if ($row['status_reg'] == 1) { ?>
                                            <a href="rh/cadastroclt.php?regiao=<?=$regiao?>&projeto=<?=$projeto?>" class="btn btn-lg btn-block btn-warning margin_b5">Cadastrar CLT</a>
                                            <a href="cooperativas/cadcooperado.php?regiao=<?= $regiao ?>&pro=<?= $projeto ?>&tipo=3" class="btn btn-lg btn-block btn-info margin_b5">Cadastrar Cooperado</a>
                                            <a href="cadastro_bolsista.php?regiao=<?= $regiao ?>&pro=<?= $projeto ?>" class="btn btn-lg btn-block btn-success margin_b5">Cadastrar Aut&ocirc;nomo</a>
                                            <a href="cooperativas/cadcooperado.php?regiao=<?= $regiao ?>&pro=<?= $projeto ?>&tipo=4" class="btn btn-lg btn-block btn-success margin_b5">Cadastrar Aut&ocirc;nomo / PJ</a>
                                            <a href="terceirizado/cadterceiro.php?regiao=<?= $regiao ?>&pro=<?= $projeto ?>&tipo=4" class="btn btn-lg btn-block btn-success margin_b5">Cadastrar Terceirizado</a>
                                            <?php } ?>
                                        </div>
                                    </div>
                                                ...
                                                <div class="stat-rows">
                                                    <div class="stat-row">
                                                        <a href="javascript:void(0);" class="stat-cell bg-danger padding-sm valign-middle">
                                                            <?=$num_clt_inativos?> CLTs Inativos
                                                            <!--i class="fa fa-users pull-right"></i-->
                                                        </a>
                                                    </div>
                                                    <div class="stat-row">
                                                        <a href="javascript:void(0);" class="stat-cell bg-danger darken padding-sm valign-middle">
                                                            <?=$num_autonomo_inativos?> Aut&ocirc;nomos Inativos
                                                            <!--i class="fa fa-users pull-right"></i-->
                                                        </a>
                                                    </div>
                                                    <div class="stat-row">
                                                        <a href="javascript:void(0);" class="stat-cell bg-danger darker padding-sm valign-middle">
                                                            <?=$num_cooperado_inativos?> Cooperados Inativos
                                                            <!--i class="fa fa-users pull-right"></i-->
                                                        </a>
                                                    </div>
                                                    <div class="stat-row">
                                                        <a href="javascript:void(0);" class="stat-cell bg-danger darker padding-sm valign-middle">
                                                            <?=$num_autonomo_pj_inativos?> Aut&ocirc;nomos / PJ Inativos
                                                            <!--i class="fa fa-users pull-right"></i-->
                                                        </a>
                                                    </div>
                                                </div> <!-- /.stat-rows -->
                            ...
                            <div class="tab-pane projRelatorio">
                                ...
                                <?php if(file_exists('relatorios2.php'))
                                    require('relatorios2.php'); ?>
                            </div>
                        </div>
                    <?php } else {
