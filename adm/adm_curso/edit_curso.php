...
if(isset($_REQUEST['method']) && !empty($_REQUEST['method']) && $_REQUEST['method']=="alteraSalario"){
    ...
    $motivo = mysql_real_escape_string(utf8_decode(trim($_REQUEST['motivo'])));
    
    mysql_query("INSERT INTO rh_salario (id_curso,data,salario_antigo,salario_novo,diferenca,user_cad,status,motivo) VALUES 
    ('$id_curso','$data_cad','$salario_antigo','$salario_novo','$diferenca','$id_usuario','1','$motivo')") or die (mysql_error());
            ...
            $(function(){
                ...
                $(".bt-image").on("click", function() {
                    ...
                    if (action === "salario") {
                        ...
                        thickBoxModal("Alteração Salarial", "#box_salario", "300", "360", null, null).css({display: "block"});
                    }
                });
                ...
                $(".check_not").live('click', function (){
                    var hide_noturno = $(this).parent().next();
                ...
                <div class="form_funcoes">
                    ...
                    <fieldset id="func1">
                        ...
                        <p>
                            ...
                            <input type="text" name="nome_funcao" id="nome_funcao" size="93" value="<?php echo $row['nome_funcao']; ?>" class="validate[required]" />
                        </p>
                        <p>
                            <label class='first'>Área:</label>
                            <input type="text" name="area" id="area" size="93" value="<?php echo $row['area']; ?>" class="validate[required]" />
                        </p>
                        <p>
                            <label class='first'>Departamento:</label>
                            <?php echo montaSelect($arrayDepartamentos, $row['id_departamento'], 'name="departamento" id="departamento" class="departamento"'); ?>
                        </p>
                        <p>
                            <label class='first'>CBO:</label>
                            <input type="text" name="cbo" id="cbo" size="93" value="<?php echo $row['nome_cbo']; ?>" class="validate[required]" />
                        </p>
                        <p>
                            <label class='first'>Local:</label>
                            <input type="text" name="local" id="local" size="93" value="<?php echo $row['local']; ?>" />
                        </p>
                        ...
                        <div id="direita">
                            ...
                            <?php }else{ ?>
                            <p>
                                <label class='first'>Valor Hora:</label>
                                <input type="text" name="valor_hora_cooperado" class="money" id="valor_hora" value="<?php echo $row['valor_hora']; ?>" />
                            </p>
                            <?php } ?>
                        </div>
                    ...
                    if($numRows>0){
                        ...
                        <fieldset id="func2" class="horario" data-position="<?php echo $posicao; ?>">
                            ...
                            <p>
                                ...
                                <input type="text" name="nome_horario[]" id="nome_horario" size="93" class="nome_horario validate[required] limpa" value="<?php echo $rst['nome']; ?>" />
                            </p>
                            <p>
                                <label class='first'>Observações:</label>
                                <input type="text" name="obs[]" id="obs" class="obs limpa" size="93" value="<?php echo $rst['obs']; ?>" />
                            </p>
                            <p class="remove">
                                <label class='first'>Preenchimento:</label>
                                Entrada <input type="text" name="entrada[]" id="entrada" size="10" class="entrada preenchimento validate[required] limpa" value="<?php echo $rst['entrada_1']; ?>" />
                                Saída Almoço <input type="text" name="ida_almoco[]" id="ida_almoco" size="10" class="ida_almoco preenchimento validate[required] limpa" value="<?php echo $rst['saida_1']; ?>" />
                                Retorno Almoço <input type="text" name="volta_almoco[]" id="volta_almoco" size="10" class="volta_almoco preenchimento validate[required] limpa" value="<?php echo $rst['entrada_2']; ?>" />
                                Saída <input type="text" name="saida[]" id="saida" size="10" class="saida preenchimento validate[required] limpa" value="<?php echo $rst['saida_2']; ?>" />
                            </p>
                            <div id="esquerda" class="esquerda">
                                <p>
                                    <label class='first'>Horas Mês:</label>
                                    <input type="text" name="horas_mes[]" id="horas_mes" size="30" maxlength="4" value="<?php echo $rst['horas_mes']; ?>" class="horas_mes limpa" />
                                </p>
                                <p>
                                    <label class='first'>Dias Mês:</label>
                                    <input type="text" name="dias_mes[]" id="dias_mes" size="30" maxlength="4" value="<?php echo $rst['dias_mes']; ?>" class="dias_mes limpa" />
                                </p>
                            </div>
                            ...
                            <div id="direita" class="direita">
                                <p>
                                  ...
                                  <input type="text" name="horas_semana[]" id="horas_semana" size="30" maxlength="4" value="<?php echo $rst['horas_semanais']; ?>" class="horas_semana limpa" />
                                </p>
                                <p>
                                    <label class='first'>Dias Semana:</label>
                                    <input type="text" name="dias_semana[]" id="dias_semana" size="30" maxlength="4" value="<?php echo $rst['dias_semana']; ?>" class="dias_semana limpa" />
                                </p>
                                <p>
                            ...
                            <p>
                                ...
                                <input type="radio" name="noturno[<?=$posicao?>]" value="1" id="n_sim" class="n_sim check_not" 
                                <?php 
                                if($rst['adicional_noturno'] == '1'){
                                    echo "checked";
                                }
                                ?> />
                                Sim
                                <input type="radio" name="noturno[<?=$posicao?>]" value="0" id="n_nao" class="n_nao check_not" 
                                <?php 
                                if($rst['adicional_noturno'] != '1'){
                                    echo "checked";
                                }
                            ...
                            <p id="hide_noturno" class="hide_noturno">
                                <label class='first'>Horas Noturno:</label>
                                <input type="text" name="horas_noturno[]" id="horas_noturno" class="horas_noturno" size="30" maxlength="4" value="<?php echo $rst['horas_noturnas']; ?>" />
                            </p>
                        </fieldset>                                                
...
?>                                        
            ...
            <div id="box_salario">
                ...
                <form action="" method="post" name="form2" id="form2" autocomplete="off">
                    ...
                    <p>
                        <label class="first">Motivo:</label>
                        <textarea rows="3" name="motivo" id="motivo"></textarea>
                    </p>
                    <p class="controls">
