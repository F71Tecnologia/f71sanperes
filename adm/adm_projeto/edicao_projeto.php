...
UPDATE projeto SET nome = 
		...
		data_atualizacao=NOW()
 		...
 		WHERE id_projeto = '$projeto' LIMIT 1")
								  or die(mysql_error());
  ...
  <tr>
    ...
    <table cellpadding="0" cellspacing="1" class="secao">
      ...
      <tr>
        ...
        <td colspan="3"><input name="local" size="50" type="text" id="local" value="<?php echo $local; ?>" /></td>
      </tr>
      <tr>
        <td class="secao">Regi&atilde;o:</td>
        <td>
            <?php $qr_regioes = mysql_query("SELECT * FROM regioes WHERE status = '1' AND id_regiao = '$row[id_regiao]'"); 
                  $row_regiao = mysql_fetch_array($qr_regioes);
                  echo $row_regiao['id_regiao'].' - '.$row_regiao['regiao']; ?>
        </td>
        <td class="secao">
        UF:
        </td>
       ...
       <select name="estado" id="estado" class="validate[required]">  
                   ...
                   <option value="<?php if(empty($row['estado'])) { echo '';} else { echo $row['estado'].' selected="selected"';} ?>" >
				   <?php if(empty($row['estado']))  { echo 'Nenhuma UF selecionada';} else { echo $row['estado']; } ?>
    ...
    <?php endif;?>
		...
		if($row_regiao['id_regiao']=='15' or $row_regiao['id_regiao']=='36' or $row_regiao['id_regiao']=='37') continue; // 	
			...
			if($result_qr ==0) continue;
               ...
               <option value="<?=$row_banco['id_banco'];?>" <?=$selected?> > <?=$row_banco['id_banco'];?> - <?=$row_banco['nome']?> - <?=$row_banco['endereco']?>
                 </option>
    ...
    <table cellpadding="0" cellspacing="1" class="secao">
      ...
      <tr>
        ...
        <td valign="top">
        	<input name="total_participantes_clt" type="text" class="total_participantes" size="10" value="<?php if($tipo_clt!=0 ){echo $tipo_clt; }?>" /><br>
            <input name="total_participantes_autonomo" type="text" class="total_participantes" size="10" value="<?php if($tipo_autonomo!=0) {echo $tipo_autonomo;} ?>" /><br>
            <input name="total_participantes_cooperado" type="text" class="total_participantes" size="10" value="<?php if($tipo_cooperado!=0) {echo $tipo_cooperado; } ?>" /><br>
            <input name="total_participantes_autonomo_pj" type="text" class="total_participantes" size="10" value="<?php if($tipo_autonomo_pj!=0 ){echo $tipo_autonomo_pj; }  ?>" />
        </td>
