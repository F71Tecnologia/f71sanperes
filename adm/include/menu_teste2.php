											 ...
											 { 
												$dif=($data_termino-$data_hoje)/86400;
                                                            ...
                                                            <tr class="linha_um">
                                                                ...
                                                                <td><?php echo '<span style="color:#09C;">Renovação expira em '.$dif.' dias!</span>';?> </td>
                                                            </tr>
...
<?php	
$tipos = array( 1 => 'Qualificação', 2 => 'Alvará de Funcionamento', 3 => 'Balanço Patrimonial', 4 => 'Declaração de Isenção de IR', 5 => 'Cartão CNPJ' , 6 => 'CRF', 7 => 'CND', 8 => 'Estatuto',9 => 'CCN-RFB/PGFN', 10 =>'Procuração', 11 =>'Ofícios Enviados', 12 => 'Ata',13 => 'Ofícios Recebidos',14 =>'Certidão de Distribuições Cíveis', 15 => 'CNDT');
...
//asort($tipos);	
	...
	for($i=1; $i<=15;$i++):
				 ...
				 $cont=0;
			   $qr_oscip = mysql_query("SELECT * FROM obrigacoes_oscip WHERE id_master = '$Master' AND status = 1 AND tipo_oscip = '$tipos[$i]' AND periodo != 'indeterminado' ORDER BY data_publicacao DESC" ) or die (mysql_error());
		...
		endwhile;
				...
				if($tipo_anterior != $i and $ok != 1 and $expirando !=1 and $indeterminado != 1 and !empty($ids_expirado)) {
							...
							if($row_oscip2['data_publicacao'] != '0000-00-00') {
									...
									echo '<tr '.$class.' height="35">									
											 <td width="25%">'.$row_oscip2['tipo_oscip'].'</td>
											 <td width="20%">'.implode('/',array_reverse(explode('-',$row_oscip2['data_publicacao']))).'
											 <td>'.$row_oscip2['numero_periodo'].' '.$row_oscip2['periodo'].'</td>
											 <td width="50%">'.$row_oscip2['descricao'].'</td>'; 
									echo '   <td><span  style="color:#F00;">Expirado</span></td>
										 </tr>';
							}
