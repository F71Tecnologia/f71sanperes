		...
		while($row_regiao = mysql_fetch_assoc($qr_regiao)):
						...
						while($row_projeto = mysql_fetch_assoc($qr_projeto)):
								 ...
								 if(($prazo_renovacao<=$data_hoje) and($data_termino>$data_hoje))
									...
									{	 
												//verifica se exite subprojetos  exbibe caso exista e esteja expirando ou expirado
												$verifica_ok=0;		 
												$qr_subprojeto=mysql_query("SELECT * FROM subprojeto WHERE id_projeto='$row_projeto[id_projeto]' AND status_reg='1' ORDER BY termino DESC");
												$verifica = mysql_num_rows($qr_subprojeto);
											 ...
											 if(($prazo_renovacao<=$data_hoje) and($data_termino>$data_hoje)) //subprojetos expirando
											 { 
											 ...
											 elseif($data_hoje>$data_termino ) //SUBprojetos expirado
											 {
												 ?>
                                                <tr class="linha_um">
                                                    <td align="left"><?php echo $row_projeto['id_projeto'].' - '.$row_projeto['nome'];?> </td>
...
<?php	
$tipos = array( 1 => 'Qualificação', 2 => 'Alvará de Funcionamento', 3 => 'Balanço Patrimonial', 4 => 'Declaração de Isenção de IR', 5 => 'Cartão CNPJ' , 6 => 'CRF', 7 => 'CND', 8 => 'Estatuto',9 => 'CCN-RFB/PGFN', 10 =>'Procuração', 11 =>'Ofícios Enviados', 12 => 'Ata',13 => 'Ofícios Recebidos');
...
///publicações do Anexo 1
  ...
  <div class="letra" style="clear:left;">  
