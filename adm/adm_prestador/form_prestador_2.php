...
//$teste = 1;
//if ($teste = 1) {
//    $usuario['id_master'] = 1; //varteste
////$_REQUEST['prestador'] = 1139; //varteste
//    $_REQUEST['prestador'] = 1167; //varteste
//}


$medicos_nome = isset($_POST['nome_medico']) ? $_POST['nome_medico'] : array();


$medicos_tel = isset($_POST['tel_medico']) ? $_POST['tel_medico'] : array();
$medicos_cpf = isset($_POST['cpf_medico']) ? $_POST['cpf_medico'] : array();
$medicos_crm = isset($_POST['crm_medico']) ? $_POST['crm_medico'] : array();
$arr_especialidades_medico = isset($_POST['especialidade_medico']) ? $_POST['especialidade_medico'] : array();
$medicos_principal = isset($_POST['principal']) ? $_POST['principal'] : array();
$medicos_id = isset($_POST['id_medico']) ? $_POST['id_medico'] : array();
//$prestador = isset($_POST['prestador']) ? $_POST['prestador'] : NULL;
$especialidade = isset($_POST['c_especialidade']) ? $_POST['c_especialidade'] : NULL;
$arr_status_medico = isset($_POST['status_medico']) ? $_POST['status_medico'] : NULL;


//pegando tipos de especialidade médica
$sql_medico_especialidade = "SELECT A.*, B.nome AS nome_tipo FROM medico_especialidade AS A LEFT JOIN medico_especialidade_tipo AS B ON(A.tipo=B.id_medico_especialidade_tipo)";
$result = mysql_query($sql_medico_especialidade);
while ($row = mysql_fetch_array($result)) {
    $arr_especialidades[strtoupper($row['nome_tipo'])][$row['id_medico_especialidade']]['nome'] = $row['nome'];
    $arr_especialidades[strtoupper($row['nome_tipo'])][$row['id_medico_especialidade']]['valor'] = $row['valor'];
}
...
//if (isset($_REQUEST['acao']) && ($_REQUEST['acao']=='get_especialidades')) {
//
//    $arrTiposE = array();
//    foreach ($arr_especialidades as $key => $especialidade) {
//        $arrTiposE[$key] = $especialidade['nome'] . ' (R$ ' . number_format($especialidade['valor'], 2, ',', '.') . ')';
//    }
//    echo montaSelect($arrTiposE, NULL, "id='prestador_tipo' name='prestador_tipo'");
//    exit();
//}
...
if (isset($_REQUEST['prestador']) && !empty($_REQUEST['prestador'])) {
    ...
    $medicos = montaQuery("prestador_medico", "*", "id_prestador = {$id_prestador}", "id_medico DESC");
...
if (isset($_REQUEST['cadastrar'])) {
    ...
    // cadastrando médicos
    for ($cont = 0; $cont <= count($medicos_nome); $cont++) {

        if (!empty($medicos_nome[$cont])) {
            $nome = $medicos_nome[$cont];
            $tel = $medicos_tel[$cont];
            $cpf = $medicos_cpf[$cont];
            $crm = $medicos_crm[$cont];
            $especialidade_medico = $arr_especialidades_medico[$cont];
            $status_medico = $arr_status_medico[$cont];
//            $principal = ($medicos_principal[$cont] == 'on') ? '1' : '0';
            $principal = '1';
            $sql = "INSERT INTO prestador_medico(nome, tel, cpf, crm, especialidade, id_prestador, principal,`status`) VALUES ('$nome', '$tel', '$cpf', '$crm','$especialidade_medico', $id_prestador, '$principal', '$status_medico')";
            mysql_query($sql);
        }
    }
...
if (isset($_REQUEST['editar'])) {
    ...
    // editando médicos
    $sql_prestador_medico = "SELECT * FROM prestador_medico WHERE id_prestador=$id_prestador";
//    echo $sql_prestador_medico."<br>";
    $result = mysql_query($sql_prestador_medico);
    ...
    if (mysql_num_rows($result) <= 0) {
        ...
        for ($cont = 0; $cont <= count($medicos_nome); $cont++) {
            if (!empty($medicos_nome[$cont])) {
                ...
                $principal = '1';
                $status_medico = $arr_status_medico[$cont];
                $sql = "UPDATE prestador_medico SET nome='$nome', tel='$tel', cpf='$cpf', crm='$crm', especialidade='$especialidade_medico', principal='$principal', `status`='$status_medico' WHERE id_medico='$id' LIMIT 1";
    ...
    <head>
        ...
        <script>
            ...
            $(function() {
                ...
                $("#adicionar_medicos").click(function() {
                    var clone = $("#medico1").clone();
                    conts++;
                    clone.attr("id", "medicos" + conts);
                    clone.find("input").val("");

                    $(clone).appendTo("#medicos");
                    $(".tel").unmask("(99)9999-9999?9");
                    $(".cpf").unmask("999.999.999-99");
                    $(".tel").mask("(99)9999-9999?9");
                    $(".cpf").mask("999.999.999-99");
...
<?php if (!isset($_GET['dev'])) { ?>
                    $("#cad").click(function() {
                        ...
                        //valida data inicial/final
                        var dataIni = $("#contratado_em").val().split('/');
                        var dataFim = $("#encerrado_em").val().split('/');
                        var calcDataIni = dataIni[2] + '-' + dataIni[1] + '-' + dataIni[0];
                        var calcDataFim = dataFim[2] + '-' + dataFim[1] + '-' + dataFim[0];

                        // VALIDAÇÃO PROVISÓRIA DE CNPJ, CPF, ETC, O IDEAL É ADD A FUNÇÃO JS NO VALIDATION ENGINE
                        if (Date.parse(calcDataIni) > Date.parse(calcDataFim)) {
                            $(".dados_projeto").css({display: 'block', margin: '20px 0 0 0'});
                            $(".dados_projeto").html('DATA INÍCIO não pode ser maior que DATA FINAL');
                            $("#contratado_em").focus();
                            return false;
                        } else if (Date.parse(calcDataIni) == Date.parse(calcDataFim)) {
                            $(".dados_projeto").css({display: 'block', margin: '20px 0 0 0'});
                            $(".dados_projeto").html('DATA INÍCIO não pode ser igual a DATA FINAL');
                            $("#contratado_em").focus();
                            return false;
                        }
                        ...
                        if (!cnpj) {
                            ...
                            $(".contratante").html('CNPJ Inválido');
                            $("#cnpj").focus();
                            return false;
                        } else {
...
//                options_unidade_medida = $('.unidade_medida_a').children('option')
                ...
                $('#prestador_tipo').change(function(e) {
                    console.log($(this).val());
                    if ($(this).val() == '9') { // 9 = Pessoa juridica tipo médica
                        $('.box-medicos').show();
//                        $('.valor_a').attr('readonly','readonly');
//                        $('.unidade_medida_a').html($('.unidade_medida_a').children('option[selected=selected]'));
                    } else {
                        $('.box-medicos').hide();
//                        $('.valor_a').removeAttr('readonly');
//                        $('.unidade_medida_a').html(options_unidade_medida);
                    }
                });
                if ($('#prestador_tipo').val() == '9') { // 9 = Pessoa juridica tipo médica
                    $('.box-medicos').show();
//                    $('.valor_a').attr('readonly','readonly');
//                    $('.unidade_medida_a').html($('.unidade_medida_a').children('option[selected=selected]'));
                } else {
        ...
        <div id="content" style="width: 850px;">
            <form action="" method="post" name="form1" id="form1" enctype="multipart/form-data" >
                ...
                <fieldset>
                    ...
                    <p><label class='first'>Contratante: </label><input type="text" size="108" name="contratante" readonly="readonly" value="<?php echo $dados_prestador['contratante'] ?>" /></p>
                    <p><label class='first'>Endereço:</label><input type="text" size="108" name="endereco" readonly="readonly" value="<?php echo $dados_prestador['endereco'] ?>" /></p>
                    <div class="colEsq">
                ...
                <fieldset>
                    ...
                    <p><label class='first'>Razão Social:</label><input type="text" name="c_razao" id="c_razao" value="<?php echo $prestador['c_razao'] ?>" size="108" class="validate[required]" /></p>
                    <p><label class='first'>Endereço:</label><input type="text" name="c_endereco" id="c_endereco" value="<?php echo $prestador['c_endereco'] ?>" size="108" /></p>
                    ...
                    <p class="clear valid_email"><label class='first'>E-mail:</label><input type="text" name="c_email" id="c_email" value="<?php echo $prestador['c_email'] ?>" size="108" class="validate[custom[email]]" /></p>
                    ...
                    <p class="clear"><label class='first'>Site:</label><input type="text" name="c_site" id="c_site" value="<?php echo $prestador['c_site'] ?>" size="108" class="validate[custom[url]]" /></p>
                </fieldset>
                ...
                <fieldset>
                    ...
                    <p class="clear valid_email"><label class='first'>Email:</label><input type="text" name="co_email" id="co_email" value="<?php echo $prestador['co_email'] ?>" size="108" class="validate[custom[email]]" /></p>
                    <div class="colEsq">
...
//irá criar e adicionar campos com as informações do dependente
                ...
                <div class="box-medicos">
                    <div class='message-box message-red medicos'></div>
                    <fieldset >
                        <legend>Médico</legend>
                        <input style="margin-left: 10px;" type="button" id="adicionar_medicos" name="adicionar_medicos" value="Adicionar Médico"/>
                        <!--<input style="margin-left: 10px;" type="button" id="adicionar_medicos" name="adicionar_medicos" value="Gerenciar Especialidades"/>-->
                        <table cellpadding="0" cellspacing="0" border="0" class="grid" style="width: 98%; margin: 10px;">
                            <thead>
                                <tr>
                                    <th>Nome</th>
                            ...
                            <tbody id="medicos">
                                <?php
                                $cont_medicos = (count($medicos) <= 0) ? 1 : count($medicos);
                                for ($cont = 1; $cont <= $cont_medicos; $cont++) {
                                    ?>
                                    <tr id="medico<?php echo $cont; ?>">
                                        <td><input type="text" name="nome_medico[]" id="nome_medico1" value="<?php echo isset($medicos[$cont]['nome']) ? $medicos[$cont]['nome'] : ''; ?>" /></td>
                                        <td><input type="text" name="tel_medico[]" style="width: 110px;" value="<?php echo isset($medicos[$cont]['tel']) ? $medicos[$cont]['tel'] : ''; ?>" class="tel" /></td>
                                        <td><input type="text" name="cpf_medico[]" style="width: 110px;" value="<?php echo isset($medicos[$cont]['cpf']) ? $medicos[$cont]['cpf'] : ''; ?>" class="cpf" /></td>
                                        <td><input type="text" name="crm_medico[]" style="width: 80px;" value="<?php echo isset($medicos[$cont]['crm']) ? $medicos[$cont]['crm'] : ''; ?>" class="crm" /></td>
                                        <td class="center" >
                                            <?php
                                            $select_especialidades = '<select class="especialidade_medico" name="especialidade_medico[]">';
                                            foreach ($arr_especialidades as $label => $especialidades) {
                                                $select_especialidades .= '<optgroup label="' . $label . '">';
                                                foreach ($especialidades as $key => $especialidade) {

                                                    $checked = ($medicos[$cont]['especialidade'] == $key) ? ' selected="selected" ' : '';

                                                    $select_especialidades .= '<option value="' . $key . '" ' . $checked . '>' . $especialidade['nome'] . ' (R$ ' . number_format($especialidade['valor'], 2, ',', '.') . ')</option>';
                                                }
                                                $select_especialidades .= ' </optgroup>';
                                            }
                                            $select_especialidades .= '</select>';
                                            echo $select_especialidades;
                                            ?>

                                        </td>
                                        <td><?php echo montaSelect(array('1' => 'Ativo', '0' => 'Inativo'), $medicos[$cont]['status'], " name='status_medico[]' class='required[custom[select]]'") ?></td>
                                        <?php if (isset($_REQUEST['prestador']) && !empty($_REQUEST['prestador'])) { ?>
                                    <input type="hidden" name="id_medico[]" value="<?php echo isset($medicos[$cont]['id_medico']) ? $medicos[$cont]['id_medico'] : ''; ?>"/>
                                <?php } ?>
                                </tr>
                            <?php } ?>
                            </tbody>
