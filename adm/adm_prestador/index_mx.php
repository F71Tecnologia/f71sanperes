...
if ((isset($_REQUEST['filtrar']) && !empty($_REQUEST['filtrar'])) || (isset($_SESSION['voltarPrestador']))) {
    ...
    $rs = montaQuery("prestadorservico", "*,DATE_FORMAT(contratado_em, '%d/%m/%Y') AS contratado_embr, DATE_FORMAT(encerrado_em, '%d/%m/%Y') AS encerrado_embr ", "id_regiao = {$_REQUEST['regiao']} AND status = 1 AND id_projeto = {$_REQUEST['projeto']}", "prestador_tipo,c_razao", null, null, false);
        ...
        <script>
            $(function() {
                ...
                $(".bt-image").on("click", function() {
                    ...
                    if (action === "docs") {
                        thickBoxIframe(emp, "actions_mx.php", {prestador: key, method: "getDocs"}, "625-not", "500");
