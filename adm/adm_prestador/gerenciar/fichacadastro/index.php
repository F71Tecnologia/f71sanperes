...
if (empty($_COOKIE['logado'])) {
    ...
    $qr_regiao = mysql_query("SELECT * FROM regioes WHERE id_regiao = '$reg'");
...
//                                WHERE A.id_clt = $clt");
    ...
    $query_bol = "SELECT c_responsavel AS nome,c_tel AS tel,c_fax AS tel_rec,c_endereco,c_civil AS civil,c_nacionalidade AS nacionalidade,c_formacao AS formacao,c_rg AS rg,c_cpf AS cpf
FROM prestadorservico WHERE id_prestador = $id";
//    echo $query_bol;
    $result_bol = mysql_query($query_bol);
...
//                                    WHERE id_clt = {$clt} AND B.nome <> C.nome");
    ...
    if(is_array($array)){
        ...
        $qr_alt_salario[] = "(SELECT 'A' ordem, salario_antigo, salario_novo, data FROM rh_salario WHERE ".implode(" OR ",$qr_alt_salario_or)." GROUP BY DATE_FORMAT(data,'%Y-%m'))";
    }else{
        $qr_alt_salario[] = "(SELECT 'A' ordem, salario_antigo, salario_novo, data FROM rh_salario WHERE  id_curso = {$row['id_curso']} AND data > '$data_entrada' AND status = 1 $auxDemi GROUP BY DATE_FORMAT(data,'%Y-%m'))";
    }
    $qr_alt_salario[] = "(SELECT 'B' ordem, null, sallimpo salario_novo, (SELECT MAX(data_proc) FROM rh_transferencias WHERE id_clt = rh_folha_proc.id_clt AND MONTH(ADDDATE(data_proc, INTERVAL 1 MONTH)) = rh_folha_proc.mes AND YEAR(ADDDATE(data_proc, INTERVAL 1 MONTH)) = rh_folha_proc.ano AND id_curso_de <> id_curso_para) data FROM rh_folha_proc WHERE id_clt = {$row['id_clt']} AND status = 3 GROUP BY sallimpo ORDER BY data_proc LIMIT 1,100)";
            ...
            <div id="content">
                <div id="empresa" class="box">
                    ...
                    <table border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            <td colspan="2" class="txcenter"><p><strong>REGISTRO DE PRESTADOR</strong></p></td>
                            <td>
                ...
                <div id="pessoal" class="box">
                    ...
                    <div id="foto">
                        <?php
                        if ($row['foto'] == "1") {
                            $nome_imagem = $reg . "_" . $pro . "_" . $row['0'] . ".gif";
                        } else {
