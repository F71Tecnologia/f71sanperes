...
$(function() {
    ...
    $(".modal-bt").on("click", function() {
        ...
        //VER OU LISTAR
        if (action === "ver") {
            $.post('actions_mx.php', {id_doc: doc, id_prestador: prest, method: "showDocs"}, function(data) {
        ...
        if (action === "anexar") {
            $.post('actions_mx.php', {id_doc: doc, id_prestador: prest, method: "novoDoc"}, function(data) {
                $("#modal_lado2 div:first").html(data);
                $("#modal_geral").animate({left: "-590px"});
                acoesTela2();
            });
...
var acoesTela2 = function() {
    ...
    $(".modal-bt-det").click(function() {
        ...
        if (acao === "editar") {
            ...
            if (confirm("Essa acao e irreversivel, deseja realmente apagar esse documento?")) {
                $.post("actions_mx.php", {id: id, method: "excluirDoc"}, function(data) {
                    if (data.status == "1") {
    ...
    $("#bt-salvar").click(function() {
        ...
        $.post("actions_mx.php", {id: idEd, valor: novaDt, method: "editaDataDoc"}, function(data) {
            if (data.status != "1") {
