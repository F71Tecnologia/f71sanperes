...
$id_permitidos = array(5, 9, 24, 87, 204);
    ...
    <body>	
        <div id="corpo">
            ...
            <div id="conteudo" style="text-transform:uppercase;">
                <h1><span>OBRIGAÇÕES DA INSTITUIÇÃO</span></h1>
                <?php
                ...
                while ($row_tipo = mysql_fetch_assoc($qr_tipo_oscip)):
                    ...
                    //if($_COOKIE['logado'] == 204){die("SELECT * FROM obrigacoes_oscip WHERE  status='1'  AND tipo_oscip='$tipo_nome' AND id_master='$Master' ORDER BY data_publicacao DESC");}
                    ...
                    if ($verifica != 0) {
                        ...
                        if ($tipo_id == 10 or $tipo_id == 19) {
                            ...
                            $qr_verifica_expiracao = mysql_query("SELECT *, DATEDIFF(data_final, NOW()) as dias_expiracao 
                                        FROM
                                       (SELECT  *,
                                       IF(periodo = 'Período', oscip_data_termino, 
                                              IF(periodo = 'Anos', DATE_ADD(data_publicacao, INTERVAL numero_periodo YEAR),
                                                                        IF(periodo = 'Meses', DATE_ADD(data_publicacao, INTERVAL numero_periodo MONTH ), 
                                                                                      IF(periodo = 'Dias', DATE_ADD(data_publicacao, INTERVAL numero_periodo DAY), 
                                                                                              IF(periodo = 'Indeterminado', periodo,'' ))))) as data_final														  

                                                FROM obrigacoes_oscip 
                                         WHERE id_master = 6 AND status = 1) as folha 				
                                         WHERE tipo_oscip = '$tipo_nome' 
                                        ORDER BY dias_expiracao DESC");
                            while ($row_verifica = mysql_fetch_assoc($qr_verifica_expiracao)) {

                                if ($row_verifica['dias_expiracao'] <= 30 and $row_verifica['periodo'] != 'Indeterminado') {
                                    $expira++;
                                }
                            }
                            $expira_oscip = $expira;
                            ...
                            <a class="show seta_fechado" id="<?php echo $status; ?>" href=".<?php echo $status; ?>" onClick="return false"> <span style="text-transform:uppercase"><?php echo $tipo_nome; ?> </span> </a>      

                            <table cellspacing="1" cellpadding="4" class="relacao" width="100%" style="display:none;">

                                <tr class="secao">
                                    <?php
                                    if ($tipo_nome == 'Ofícios Enviados' OR $tipo_nome == 'Ofícios Recebidos') {
                                ...
                                while ($row_oscip = mysql_fetch_assoc($qr_oscip)):
                                    ...
                                    ?>>                     
                                        ...
                                        <td align="center">
                                            ...
                                            if ($tipo_nome == 'Ofícios Enviados' OR $tipo_nome == 'Ofícios Recebidos') {
                                                if ($tipo_nome == 'Ofícios Enviados') {
                                                    $oficio = 'Ofícios Recebidos';
                                                } else if ($tipo_nome == 'Ofícios Recebidos') {
                                                ...
                                                if (mysql_num_rows($sqlResposta) > 0) {
                                                    ...
                                                    echo "Respondido em $oficio {$rowResposta['id_oscip']}";
                                                }
                                            }
                                            ?>               
                                        </td>
