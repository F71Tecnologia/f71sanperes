...
include('../../funcoes.php');
// GAMBI PARA BURLAR O STATUS CODE 302
// PARA RENOVAR_OSCIP NÃO POSSO TER ESSE INCLUDE 
if(empty($_REQUEST['r'])){
...
if(isset($_GET['nome'])){
    ...
    $nome = $_GET['nome'];
    $update = mysql_query("UPDATE obrigacoes_oscip_anexos SET nome_diretoria = '$nome' WHERE id_anexo = '$id_anexo'") or die(mysql_error());
    if($update){
        echo 1;
    } else { 
        echo 0;
...
<script type="text/javascript">
	...
	////NOME
	$('.muda_nome').live('change',function(){
            console.log('b ');
		var nome = $(this).val();
		var id_anexo = $(this).attr('alt');		
		$.ajax({
			'url': 'cadastro_oscip2.php?nome='+nome+'&id_anexo='+id_anexo,
	...
	function prencheItens(event, ID, fileObj, response, data){
		...
		if(retorno.erro == false){
			...
			<?php if($row_oscip['tipo_oscip'] == 'Documentos da diretoria') {?>
			<br>Nome: <input type="text" name="muda_nome" class="muda_nome" alt="'+retorno.ID+'" value=""/>\n\
			<?php } ?>
			</td>\n\
			</tr>\n\
      ...
      <!-- tabela com os radios-->
       ...
       $qr_anexos = mysql_query("SELECT * FROM  obrigacoes_oscip_anexos WHERE id_oscip = '$id_oscip' AND tipo_anexo = '1' AND   status = '1' ORDER BY anexo_ordem ASC");
           ...
           if($row_anexos['extensao']=='pdf')
		   {
               ...
               <a target="_blank" href="../../adm/adm_contratos/paginas_oscip.php?m=<?=$link_master?>&id=<?=$row_anexos['id_oscip']?>&pg=<?=$row_anexos['anexo_ordem']?>&tp=<?=$row_anexos['tipo_anexo']?>" title="Visualizar anexo">        
            ...
            <tr>
            <td>
            	Ordem: <input type="text" name="muda_ordem" class="muda_ordem_publicacao" value="<?=$row_anexos['anexo_ordem']?>" alt="<?=$row_anexos['id_anexo']?>"/>
            </td>
            <td>
       ...
       $qr_anexos = mysql_query("SELECT * FROM  obrigacoes_oscip_anexos WHERE id_oscip = '$id_oscip' AND tipo_anexo = '2' AND   status = '1' ORDER BY anexo_ordem ASC ");
		   ...
		   {
			   ?>
			   	   <a target="_blank" href="anexos_oscip/<?=$row_anexos['id_anexo'].'.'.$row_anexos['extensao']?>" title="Visualizar anexo"> 
            ...
            <tr>
            <td>
            	Ordem: <input type="text" name="muda_ordem" class="muda_ordem_documento" value="<?=$row_anexos['anexo_ordem']?>" alt="<?=$row_anexos['id_anexo']?>"/>
                
                <?php 				
				if($row_oscip['tipo_oscip'] == 'Documentos da diretoria') {?>
                	<br> Nome: &nbsp;&nbsp;<input type="text" name="muda_nome" class="muda_nome" value="<?=$row_anexos['nome_diretoria']?>" alt="<?=$row_anexos['id_anexo']?>"/>                
                <?php } ?>
            </td>
            <td>
            	<a href="#" rel="<?=$row_anexos['id_anexo']?>" class="btn_del" onClick="return false"><img src="../../uploadfy/cancel.png" /></a>
