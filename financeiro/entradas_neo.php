...
<tr>
  ...
  <tr>
    ...
    <form action="entradas_neo.php" method="post" enctype="multipart/form-data" name='form1' onsubmit="return validaForm()" id="form1"> 
...
?></td>
      ...
      <tr class="bloco_notas" bordercolor="#FFFFFF" bgcolor="#FFFFFF" style="display : none;" >
        ...
        <tr class="bloco_nota" bordercolor="#FFFFFF" bgcolor="#FFFFFF" style="display : none;" >
		<td height="30"  align="right" valign="middle" bgcolor="#EBEBEB" class="linhaspeq"><strong><span class="style16">Projeto :</span></strong></td>
		<td height="30" align="left" valign="middle" bgcolor="#F6F6F6" class="linhaspeq">&nbsp;
                    <?php 
                    $result_projentrada = mysql_query("SELECT * FROM projeto where id_regiao = '$regiao' AND status_reg = '1'");
                    print "<select name='projeto_entrada'>";
                        print "<option value=''>Selecione</option>";
                        while($row_projeto = mysql_fetch_array($result_projentrada)){
                            print "<option value=$row_projeto[0]>$row_projeto[id_projeto] - $row_projeto[nome] </option>";
                        }
                    print "</select>";?>
		</td>
...
<?php 
	...
	*/
	...
	$adicional          = str_replace(".","", $adicional);
        $projeto_entrada    = $_REQUEST['projeto_entrada'];
...
if($_COOKIE['logado'] == 87){
	...
	if(!empty($_REQUEST['radio_nota'])){
        ...
        if(!empty($_REQUEST['projeto_entrada'])){
            $id_entrada = $_REQUEST['projeto_entrada'];
            $id_ultima_entrada = mysql_insert_id();
            if(!empty($id_entrada)){
                $id_entrada = $_REQUEST['projeto_entrada'];
                mysql_query("INSERT INTO entrada_complemento (id_entrada, valor, id_tipo, id_projeto) VALUES ('$id_ultima_entrada','$valor','$tipo','$projeto_entrada')");
